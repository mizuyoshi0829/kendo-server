...
//exit;
        ...
        $file_name = $_SESSION['auth']['year'] . 'allnaganoJuniornKojinnTounamentResults_' . $mw . '.xlsx';
        ...
        for( $player = 0; $player < $tournament_player_num; $player++ ){
            ...
            if( $player < $tournament_player_num2 ){
                ...
                $sheet->setCellValueByColumnAndRow( $col+1, $row, '('.$tournament_data['data'][0]['player'][$player]['belonging_to_name'].')' );
        ...
        $sheet->getColumnDimension(PHPExcel_Cell::stringFromColumnIndex(2))->setWidth(12.0+0.71+0.17);
        ...
        $sheet->getColumnDimension(PHPExcel_Cell::stringFromColumnIndex(4+$match_level*2+2))->setWidth(12.0+0.71+0.17);
...
*/
        ...
        if( $tournament_data['data'][0]['match'][1]['winner'] == 1 ){
            $sheet->setCellValueByColumnAndRow( 2, $row, $tournament_data['data'][0]['match'][1]['player2_name'] );
            $sheet->setCellValueByColumnAndRow( 5, $row, '('.$tournament_data['data'][0]['match'][1]['player2_belonging_to_name'].')' );
        } else if( $tournament_data['data'][0]['match'][1]['winner'] == 2 ){
            $sheet->setCellValueByColumnAndRow( 2, $row, $tournament_data['data'][0]['match'][1]['player1_name'] );
            $sheet->setCellValueByColumnAndRow( 5, $row, '('.$tournament_data['data'][0]['match'][1]['player1_belonging_to_name'].')' );
        }
        $sheet->getStyle('A'.($row).':F'.$row)->getFont()->setSize(12);
        $sheet->getRowDimension($row)->setRowHeight(16);
        $row++;
        $sheet->setCellValueByColumnAndRow( 1, $row, '３位' );
        if( $tournament_data['data'][0]['match'][2]['winner'] == 1 ){
            $sheet->setCellValueByColumnAndRow( 2, $row, $tournament_data['data'][0]['match'][2]['player2_name'] );
            $sheet->setCellValueByColumnAndRow( 5, $row, '('.$tournament_data['data'][0]['match'][2]['player2_belonging_to_name'].')' );
        } else if( $tournament_data['data'][0]['match'][2]['winner'] == 2 ){
            $sheet->setCellValueByColumnAndRow( 2, $row, $tournament_data['data'][0]['match'][2]['player1_name'] );
            $sheet->setCellValueByColumnAndRow( 5, $row, '('.$tournament_data['data'][0]['match'][2]['player1_belonging_to_name'].')' );
        }
        $sheet->getStyle('A'.($row).':F'.$row)->getFont()->setSize(12);
        $sheet->getRowDimension($row)->setRowHeight(16);
        $row++;
...
//print_r($tournament_data);
		...
		for( $tournament_team = 0; $tournament_team < $team_num; $tournament_team++ ){
			...
			$pref = $tournament_data['data'][0]['player'][$tournament_team]['belonging_to_name'];
...
//print_r($match_tbl);
		...
		for(;;){
			...
			if( $line == $line2-1 ){
				...
				$pdf .= '<td height="'.$table_height.'" class="div_border_br'.$win.' div_result_one_tournament">１２３</td>';
				$win = '';
				if( $tournament_data['data'][0]['match'][0]['winner'] == 2 ){
					$win = '_win';
				}
				$pdf .= '<td height="'.$table_height.'" class="div_border_b'.$win.' div_result_one_tournament">１２３</td>';
...
//exit;
	...
	function __output_entry_data_list_all_1_excel22_23( $sheet, $series, $series_mw, $start_pos, $series_name )
	{
		...
		foreach( $list as $lv ){
			...
			for( $player = 1; $player <= 1; $player++ ){
				$sheet->setCellValueByColumnAndRow( $col++, $pos, $fields['kojin_'.$series_mw.$player.'_sei'].' '.$fields['kojin_'.$series_mw.$player.'_mei'] );
				$sheet->setCellValueByColumnAndRow( $col++, $pos, $fields['kojin_'.$series_mw.$player.'_sei'] );
				$sheet->setCellValueByColumnAndRow( $col++, $pos, $fields['kojin_'.$series_mw.$player.'_mei'] );
			}
			$pos++;
		}
		db_close( $dbs );
	...
	function output_entry_data_list_all_1_excel22( $sheet )
	{
		...
		__output_entry_data_list_all_1_excel22_23( $sheet, 23, 'w', $pos+1, '個人戦女子' );
		return false;
	}
