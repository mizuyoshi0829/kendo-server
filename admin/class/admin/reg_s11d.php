<?php
    require_once dirname(__FILE__).'/reg_s9_11d.php';

	function __get_league_parameter_39_40( $series_mw )
	{
		$param = array(
			'mw' => $series_mw,
			'team_num' => 3,
	...
	function get_league_parameter_39()
	{
        return __get_league_parameter_39_40( 'm' );
	}

	function get_league_parameter_40()
	{
        return __get_league_parameter_39_40( 'w' );
	}

	function __get_tournament_parameter_39_40()
	{
		$param = array(
			'mw' => $series_mw,
			'team_num' => 8,
	...
	function get_tournament_parameter_39()
	{
		__get_tournament_parameter_39_40( 'm' );
	}

	function get_tournament_parameter_40()
	{
		__get_tournament_parameter_39_40( 'w' );
	}

	function __get_entry_data_39_40_list_for_PDF( $series, $series_mw, $kaisai_rev )
	{
		$c = new common();
...
*/
	...
	function get_entry_data_39_list_for_PDF()
	{
		return __get_entry_data_39_40_list_for_PDF( 16, 'm', 1 );
	}

	function get_entry_data_40_list_for_PDF()
	{
		return __get_entry_data_39_40_list_for_PDF( 17, 'w', 1 );
	}


	function get_entry_data_list_39_sql( $mw )
	{
	...
	function output_league_39_for_HTML_2( $league_list, $entry_list, $mv )
	{
//print_r($league_list);
		if( $mv == 'm' ){
			$mvstr = '男子';
...
//							$html .= $league_list[$league_data]['match'][$match_no_index-1]['hon2'].'-'.$league_list[$league_data]['match'][$match_no_index-1]['win2'];
	...
	function __output_league_39_40_for_HTML( $navi_info, $league_param, $league_list, $entry_list, $mw )
	{
//echo $path;
//print_r($league_list);
		if( $mw == 'm' ){
...
//							$html .= $league_list[$league_data]['match'][$match_no_index-1]['hon2'].'-'.$league_list[$league_data]['match'][$match_no_index-1]['win2'];
	...
	function output_league_39_for_HTML( $navi_info, $league_param, $league_list, $entry_list )
	{
        $objPage = new form_page();
//print_r($league_list);
        $objPage->output_league_for_HTML( $navi_info, $league_param, $league_list, $entry_list, 'm', false );
		//__output_league_39_40_for_HTML( $path, get_league_parameter_39(), $league_list, $entry_list, 'm' );
	}

	function output_league_40_for_HTML( $navi_info, $league_param, $league_list, $entry_list )
	{
        $objPage = new form_page();
        $objPage->output_league_for_HTML( $navi_info, $league_param, $league_list, $entry_list, 'w', false );
		//__output_league_39_40_for_HTML( $path, get_league_parameter_40(), $league_list, $entry_list, 'w' );
	}
	...
	function __output_league_39_40_for_Excel_one( $objPage, $sheet, $col, $row, $league_param, $league_list, $entry_list, $mv )
	{
//print_r($league_list);
		if( $mv == 'm' ){
			$mvstr = '男子';
	...
	function __output_league_39_40_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, $mw )
	{
//print_r($league_list);
		if( $mw == 'm' ){
			$mwstr = '男子';
		...
		$file_name = 'H29hokushinEtsuJuniorDantaiLeagueResults_' . $mw . '.xls';
...
//echo "1:",$mg_range1,':',$mg_range2,'<br />';
			...
			for( $dantai_index_row=0; $dantai_index_row<$league_list[$league_data]['team_num']; $dantai_index_row++ ){
                ...
                if( $league_list[$league_data]['extra_match_exists'] == 1 && $dantai_index_row == 1 ){
    				$sheet->setCellValueByColumnAndRow(
	    				$colscore+9, $row+$dantai_index_row*2+2, '代'
			    	);
                }
	...
	function output_league_39_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_39_40_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, 'm' );
	}

	function output_league_40_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_39_40_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, 'w' );
	}

	//---------------------------------------------------------------
	//
	//---------------------------------------------------------------
	function __output_league_match_for_HTML2_39_40( $objPage, $path, $league_list, $entry_list, $mv )
	{
		if( $mv == 'm' ){
			$mvstr = '男子';
		} else {
	...
	function output_league_match_for_HTML2_39( $navi_info, $league_param, $league_list, $entry_list )
	{
		//__output_league_match_for_HTML2_39_40( $objPage, $path, $league_list, $entry_list, 'm' );
        $objPage = new form_page();
        $objPage->output_league_match_for_HTML2( $navi_info, $league_param, $league_list, $entry_list, 'm' );
	}

	function output_league_match_for_HTML2_40( $navi_info, $league_param, $league_list, $entry_list )
	{
		//__output_league_match_for_HTML2_39_40( $objPage, $path, $league_list, $entry_list, 'w' );
        $objPage = new form_page();
	...
	function __output_league_match_for_excel_39_40( $objPage, $series_info, $league_param, $league_list, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//print_r($league_list);
//exit;
		$file_name = 'H29hokushinEtsuJuniorDantaiLeagueMatchResults_'.$mw.'.xls';
		$file_path = $series_info['output_path'] . '/' . $file_name;
	...
	function output_league_match_for_excel_39( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_match_for_excel_39_40( $objPage, $series_info, $league_param, $league_list, $entry_list, 'm' );
	}

	function output_league_match_for_excel_40( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_match_for_excel_39_40( $objPage, $series_info, $league_param, $league_list, $entry_list, 'w' );
	}
	...
	function __output_tournament_match_39_40_for_excel( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
		...
		require_once dirname(dirname(dirname(__FILE__))).'/phpExcel/Classes/PHPExcel.php';
		require_once dirname(dirname(dirname(__FILE__))).'/phpExcel/Classes/PHPExcel/IOFactory.php';
		$file_name = 'H29hokushinEtsuJuniorDantaiTournamentMatchResults_' . $mw . '.xls';
		$file_path = $series_info['output_path'] . '/' . $file_name;
		...
		$row = 6;
		for( $i1 = 1; $i1 <= 4; $i1++ ){
            $objPage->output_one_match_for_excel( $sheet, $col, $row, $series_info, $tournament_data['match'][$i1+2], $entry_list, $mv, 46, 46 );
            $row += 6;
        }
		for( $i1 = 1; $i1 <= 2; $i1++ ){
        ...
        $row += 6;
//        $objPage->output_one_match_for_excel( $sheet, $col, $row, $series_info, $tournament_data['match'][7], $entry_list, $mv, 46, 46 );
	...
	function output_tournament_match_39_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_match_39_40_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'm' );
	}

	function output_tournament_match_40_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_match_39_40_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'w' );
	}
	...
	function __output_tournament_39_40_for_excel( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//exit;
        $file_name = 'H29allnaganoJuniorDantaiTounamentResults_' . $mw . '.xls';
        $file_path = $series_info['output_path'] . '/' . $file_name;
        $excel = new PHPExcel();
        $excel->setActiveSheetIndex( 0 );        //何番目のシートに有効にするか
        $sheet = $excel->getActiveSheet();    //有効になっているシートを取得
	...
	function __output_tournament_39_40_for_excel2( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw, $series_m, $series_w )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//print_r($tournament_data);
//exit;
        $file_name = 'H29hokushinEtsuJuniorDantaiTounamentResults.xlsx';
        $file_path = $series_info['output_path'] . '/' . $file_name;
        $excel = new PHPExcel();
        $reader = PHPExcel_IOFactory::createReader('Excel2007');
        $excel = $reader->load(dirname(__FILE__).'/hokushinEtsuJuniorDantaiTournament.xlsx');
        $excel->setActiveSheetIndex( 0 );        //何番目のシートに有効にするか
        $sheet = $excel->getActiveSheet();    //有効になっているシートを取得
        $sheet->getDefaultStyle()->getFont()->setName('ＭＳ Ｐゴシック');
        $sheet->getDefaultStyle()->getFont()->setSize(9);
        ...
        // 決勝
        $sheet->getStyle( 'P6:P8' )->applyFromArray( $styleArrayVR );
        if( $tournament_data['match'][0]['winner'] == 1 ){
            $sheet->setCellValue( 'N4', $tournament_data['match'][0]['team1_name'] );
        } else if( $tournament_data['match'][0]['winner'] == 2 ){
            $sheet->setCellValue( 'N4', $tournament_data['match'][0]['team2_name'] );
        }

        // 3決
        if( $tournament_data['match'][1]['winner'] == 1 ){
            $sheet->setCellValue( 'AH13', $tournament_data['match'][1]['team2_name'] );
        } else if( $tournament_data['match'][1]['winner'] == 2 ){
            $sheet->setCellValue( 'AH13', $tournament_data['match'][1]['team1_name'] );
        }
        if( $tournament_data['match'][2]['winner'] == 1 ){
            $sheet->setCellValue( 'AL13', $tournament_data['match'][2]['team2_name'] );
        } else if( $tournament_data['match'][2]['winner'] == 2 ){
            $sheet->setCellValue( 'AL13', $tournament_data['match'][2]['team1_name'] );
        }
        if( $tournament_data['match'][7]['winner'] == 1 ){
            $sheet->getStyle( 'AH9:AH11' )->applyFromArray( $styleArrayVR );
            $sheet->getStyle( 'AI9:AJ9' )->applyFromArray( $styleArrayHR );
            $sheet->getStyle( 'AJ7:AJ8' )->applyFromArray( $styleArrayVR );
        } else if( $tournament_data['match'][7]['winner'] == 2 ){
            $sheet->getStyle( 'AL9:AL11' )->applyFromArray( $styleArrayVR );
            $sheet->getStyle( 'AK9:AL9' )->applyFromArray( $styleArrayHR );
            $sheet->getStyle( 'AJ7:AJ8' )->applyFromArray( $styleArrayVR );
        }
        $winstr = $objPage->get_tounament_chart_winstring_for_excel( $tournament_data['match'][$tournament_team_num-1] );
        $sheet->setCellValue( 'AH8', $winstr[1][1].' '.$winstr[1][2] );
        $sheet->setCellValue( 'AM8', $winstr[2][2].' '.$winstr[2][1] );

        if( $tournament_data['match'][0]['winner'] == 1 ){
            $sheet->setCellValue( 'A29', $tournament_data['match'][0]['team1_name'] );
        } else if( $tournament_data['match'][0]['winner'] == 2 ){
            $sheet->setCellValue( 'A29', $tournament_data['match'][0]['team2_name'] );
        }
        if( $tournament_data['match'][0]['winner'] == 1 ){
            $sheet->setCellValue( 'I29', $tournament_data['match'][0]['team2_name'] );
        } else if( $tournament_data['match'][0]['winner'] == 2 ){
            $sheet->setCellValue( 'I29', $tournament_data['match'][0]['team1_name'] );
        }
        if( $tournament_data['match'][7]['winner'] == 1 ){
            $sheet->setCellValue( 'Q29', $tournament_data['match'][7]['team1_name'] );
        } else if( $tournament_data['match'][7]['winner'] == 2 ){
            $sheet->setCellValue( 'Q29', $tournament_data['match'][7]['team2_name'] );
        }
        if( $tournament_data['match'][7]['winner'] == 1 ){
            $sheet->setCellValue( 'Y29', $tournament_data['match'][7]['team2_name'] );
        } else if( $tournament_data['match'][7]['winner'] == 2 ){
            $sheet->setCellValue( 'Y29', $tournament_data['match'][7]['team1_name'] );
        }
        ...
        // 決勝
        $sheet->getStyle( 'P37:P39' )->applyFromArray( $styleArrayVR );
        if( $tournament_data['match'][0]['winner'] == 1 ){
            $sheet->setCellValue( 'N35', $tournament_data['match'][0]['team1_name'] );
        } else if( $tournament_data['match'][0]['winner'] == 2 ){
            $sheet->setCellValue( 'N35', $tournament_data['match'][0]['team2_name'] );
        }

        // 3決
        if( $tournament_data['match'][1]['winner'] == 1 ){
            $sheet->setCellValue( 'AH44', $tournament_data['match'][1]['team2_name'] );
        } else if( $tournament_data['match'][1]['winner'] == 2 ){
            $sheet->setCellValue( 'AH44', $tournament_data['match'][1]['team1_name'] );
        }
        if( $tournament_data['match'][2]['winner'] == 1 ){
            $sheet->setCellValue( 'AL44', $tournament_data['match'][2]['team2_name'] );
        } else if( $tournament_data['match'][2]['winner'] == 2 ){
            $sheet->setCellValue( 'AL44', $tournament_data['match'][2]['team1_name'] );
        }
        if( $tournament_data['match'][7]['winner'] == 1 ){
            $sheet->getStyle( 'AH40:AH42' )->applyFromArray( $styleArrayVR );
            $sheet->getStyle( 'AI40:AJ40' )->applyFromArray( $styleArrayHR );
            $sheet->getStyle( 'AJ38:AJ39' )->applyFromArray( $styleArrayVR );
        } else if( $tournament_data['match'][7]['winner'] == 2 ){
            $sheet->getStyle( 'AL40:AL42' )->applyFromArray( $styleArrayVR );
            $sheet->getStyle( 'AK40:AL40' )->applyFromArray( $styleArrayHR );
            $sheet->getStyle( 'AJ38:AJ39' )->applyFromArray( $styleArrayVR );
        }
        $winstr = $objPage->get_tounament_chart_winstring_for_excel( $tournament_data['match'][$tournament_team_num-1] );
        $sheet->setCellValue( 'AH39', $winstr[1][1].' '.$winstr[1][2] );
        $sheet->setCellValue( 'AM39', $winstr[2][2].' '.$winstr[2][1] );

        if( $tournament_data['match'][0]['winner'] == 1 ){
            $sheet->setCellValue( 'A60', $tournament_data['match'][0]['team1_name'] );
        } else if( $tournament_data['match'][0]['winner'] == 2 ){
            $sheet->setCellValue( 'A60', $tournament_data['match'][0]['team2_name'] );
        }
        if( $tournament_data['match'][0]['winner'] == 1 ){
            $sheet->setCellValue( 'I60', $tournament_data['match'][0]['team2_name'] );
        } else if( $tournament_data['match'][0]['winner'] == 2 ){
            $sheet->setCellValue( 'I60', $tournament_data['match'][0]['team1_name'] );
        }
        if( $tournament_data['match'][7]['winner'] == 1 ){
            $sheet->setCellValue( 'Q60', $tournament_data['match'][7]['team1_name'] );
        } else if( $tournament_data['match'][7]['winner'] == 2 ){
            $sheet->setCellValue( 'Q60', $tournament_data['match'][7]['team2_name'] );
        }
        if( $tournament_data['match'][7]['winner'] == 1 ){
            $sheet->setCellValue( 'Y60', $tournament_data['match'][7]['team2_name'] );
        } else if( $tournament_data['match'][7]['winner'] == 2 ){
            $sheet->setCellValue( 'Y60', $tournament_data['match'][7]['team1_name'] );
        }
	...
	function output_tournament_39_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_39_40_for_excel2( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'm', 39, 40 );
	}

	function output_tournament_40_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_39_40_for_excel2( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'w', 39, 40 );
	}
	...
	function __output_tournament_chart_39_40_for_excel( $objPage, $series_info, $tournament_list, $league_data, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
	...
	function output_tournament_chart_39_for_excel( $objPage, $series_info, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_chart_39_40_for_excel( $objPage, $series_info, $tournament_data, $league_data, $entry_list, 'm' );
	}

	function output_tournament_chart_40_for_excel( $objPage, $series_info, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_chart_39_40_for_excel( $objPage, $series_info, $tournament_data, $league_data, $entry_list, 'w' );
	}
	...
	function __output_tournament_match_for_HTML2_39_40( $series_info, $tournament_list, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
		...
		//$html .= '<h3>団体順位決定戦</h3>';
		//$html .= $objPage->output_one_match_for_HTML2( $series_info, $tournament_list[0]['match'][7], $entry_list, $mw );
		$html .= $footer;
        $file = $series_info['result_path'] . '/dtm_' . $series_info['result_prefix'] . $mw . $tindex.'.html';
		$fp = fopen( $file, 'w' );
		fwrite( $fp, $html );
		fclose( $fp );
	}

	function output_tournament_match_for_HTML2_39( $series_info, $tournament_list, $entry_list )
	{
		__output_tournament_match_for_HTML2_39_40( $series_info, $tournament_list, $entry_list, 'm' );
	}

	function output_tournament_match_for_HTML2_40( $series_info, $tournament_list, $entry_list )
	{
		__output_tournament_match_for_HTML2_39_40( $series_info, $tournament_list, $entry_list, 'w' );
	}
	...
	function output_tournament_40_for_PDF( $tournament_data, $entry_list, $mv )
	{
	}

	function output_tournament_39_for_PDF( $tournament_data, $entry_list, $mv )
	{
//print_r($tournament_data);
		if( $mv == 'm' ){
			$mvstr = '男子';
...
exit;
	...
	function __output_tournament_39_40_for_HTML( $series_info, $tournament_list, $entry_list, $mv )
	{
//print_r($tournament_data);
//print_r($entry_list);
		if( $mv == 'm' ){
...
//echo $pdf;
//exit;
        $file = $series_info['result_path'] . '/dt_' . $series_info['result_prefix'] . $mv . $tindex.'.html';
		$fp = fopen( $file, 'w' );
		fwrite( $fp, $pdf );
		fclose( $fp );
	...
	function output_tournament_39_for_HTML( $series_info, $tournament_data, $entry_list )
	{
		__output_tournament_39_40_for_HTML( $series_info, $tournament_data, $entry_list, 'm' );
	}

	function output_tournament_40_for_HTML( $series_info, $tournament_data, $entry_list )
	{
		__output_tournament_39_40_for_HTML( $series_info, $tournament_data, $entry_list, 'w' );
	}
	...
	function __output_entry_data_list_all_1_excel39_40( $sheet, $series, $mv, $start_pos, $series_name, $kaisai_rev )
	{
		$c = new common();
		...
		foreach( $list as $lv ){
			...
			$pref_name = $c->get_pref_name( $preftbl, $fields['responsible_address_pref'] );
			$sheet->setCellValueByColumnAndRow( $col++, $pos, $pref_name );
			$sheet->setCellValueByColumnAndRow( $col++, $pos, $fields['responsible'] );
			$sheet->setCellValueByColumnAndRow( $col++, $pos, $fields['responsible_address_zip1'].'-'.$fields['responsible_address_zip2'] );
			$sheet->setCellValueByColumnAndRow( $col++, $pos, $pref_name.$fields['responsible_address'] );
			...
			for( $player = 1; $player <= 7; $player++ ){
				...
				$sheet->setCellValueByColumnAndRow( $col++, $pos, $fields['player'.$player.'_sei'] );
				$sheet->setCellValueByColumnAndRow( $col++, $pos, $fields['player'.$player.'_mei'] );
				if( $fields['player'.$player.'_add'] == '1' ){
					$sheet->setCellValueByColumnAndRow( $col, $pos, 'あり' );
				}
				$col++;
				$sheet->setCellValueByColumnAndRow( $col++, $pos, $fields['player'.$player.'_dan'] );
			}
			$pos++;
		}
		db_close( $dbs );
	...
	function output_entry_data_list_all_1_excel39( $sheet )
	{
		$pos = __output_entry_data_list_all_1_excel39_40( $sheet, 36, 'm', 4, '小学生の部', 1 );
		$pos = __output_entry_data_list_all_1_excel39_40( $sheet, 37, 'm', $pos+1, '中学生男子の部', 1 );
		$pos = __output_entry_data_list_all_1_excel39_40( $sheet, 38, 'w', $pos+1, '中学生女子の部', 1 );
		$pos = __output_entry_data_list_all_1_excel39_40( $sheet, 39, 'm', $pos+1, '成年男子の部', 1 );
		$pos = __output_entry_data_list_all_1_excel39_40( $sheet, 40, 'w', $pos+1, '成年女子の部', 1 );
		return false;
	}

	function output_entry_data_list_all_1_excel40( $sheet )
	{
		$pos = __output_entry_data_list_all_1_excel39_40( $sheet, 36, 'm', 4, '小学生の部', 1 );
		$pos = __output_entry_data_list_all_1_excel39_40( $sheet, 37, 'm', $pos+1, '中学生男子の部', 1 );
		$pos = __output_entry_data_list_all_1_excel39_40( $sheet, 38, 'w', $pos+1, '中学生女子の部', 1 );
	...
	function __get_entry_data_list2_39_40( $series )
	{
		$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
		$sql = 'select * from `entry_info`'
			.' where `series`='.$series        //.' and `year`='.$_SESSION['auth']['year']
	...
	function get_entry_data_list2_39()
	{
		return __get_entry_data_list2_39_40( 39 );
	}

	function get_entry_data_list2_40()
	{
		return __get_entry_data_list2_39_40( 40 );
	}

	//--------------------------------------------------------------

	function __get_entry_data_for_draw_csv_39_40( $list, $series )
	{
		$c = new common();
	...
	function get_entry_data_for_draw_csv_39( $list )
	{
		return array(
			'csv' => __get_entry_data_for_draw_csv_39_40( $list, 16 ),
			'file' => 'dantai_m.csv'
		);
	}

	function get_entry_data_for_draw_csv_40( $list )
	{
		return array(
			'csv' => __get_entry_data_for_draw_csv_39_40( $list, 17 ),
			'file' => 'dantai_w.csv'
		);
	}
	...
	function __output_realtime_html_for_one_board_39_40( $place, $place_match_no )
	{
		global $navi_info;
...
*/
	...
	function output_realtime_html_for_one_board_39( $place, $place_match_no )
	{
		//return __output_realtime_html_for_one_board_39_40( $place, $place_match_no );
		return '';
	}

	function output_realtime_html_for_one_board_40( $place, $place_match_no )
	{
		//return __output_realtime_html_for_one_board_39_40( $place, $place_match_no );
		return '';
	}
	...
	//
	//---------------------------------------------------------------
	function draw_entry_data_list_39( $series_info )
	{
        require_once dirname(dirname(dirname(__FILE__))).'/phpExcel/Classes/PHPExcel.php';
        require_once dirname(dirname(dirname(__FILE__))).'/phpExcel/Classes/PHPExcel/IOFactory.php';
        $file_name = 'draw' . date('YmdHis') . sprintf("%04d",microtime()*1000) . '.xls';
		$file_path = $series_info['output_path'] . '/' . $file_name;
        $sheet_index = 0;
        $excel = new PHPExcel();
		$sheet_index = __draw_entry_data_list_36_40( $excel, $sheet_index, 36, 3, '小学生の部', '級位', 'reg_36_2017_data.csv' );
		$sheet_index = __draw_entry_data_list_36_40( $excel, $sheet_index, 37, 3, '中学生男子の部', '段位', 'reg_37_2017_data.csv' );
		$sheet_index = __draw_entry_data_list_36_40( $excel, $sheet_index, 38, 3, '中学生女子の部', '段位', 'reg_38_2017_data.csv' );
		$sheet_index = __draw_entry_data_list_36_40( $excel, $sheet_index, 39, 3, '成年男子の部', '段位', 'reg_39_2017_data.csv' );
		$sheet_index = __draw_entry_data_list_36_40( $excel, $sheet_index, 40, 3, '成年女子の部', '段位', 'reg_40_2017_data.csv' );
        $writer = PHPExcel_IOFactory::createWriter( $excel, 'Excel5' );
        $writer->save( $file_path );
	...
	function draw_entry_data_list_40( $series_info )
	{
        require_once dirname(dirname(dirname(__FILE__))).'/phpExcel/Classes/PHPExcel.php';
        require_once dirname(dirname(dirname(__FILE__))).'/phpExcel/Classes/PHPExcel/IOFactory.php';
        $file_name = 'draw' . date('YmdHis') . sprintf("%04d",microtime()*1000) . '.xls';
