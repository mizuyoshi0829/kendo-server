<?php
	...
	function __get_league_parameter_12_13( $series_mw )
	{
		$param = array(
			'mw' => $series_mw,
			'team_num' => 3,
	...
	function get_league_parameter_12()
	{
		$param = array(
			'mw' => 'm',
			'team_num' => 5,
			'match_num' => 10,
			'match_info' => array(
				array( 0, 1 ), array( 0, 2 ), array( 0, 3 ), array( 0, 4 ), array( 1, 2 ),
				array( 1, 3 ), array( 1, 4 ), array( 2, 3 ), array( 2, 4 ), array( 3, 4 )
			),
			'place_num' => 4,
			'place_group_num' => 2,
			'place_match_info' => array(
				array( 1, 3, 5 ), array( 2, 4, 6 )
			),
			'group_num' => 1,
			'chart_tbl' => array(
				array(0,1,2,3,4), array(1,0,5,6,7), array(2,5,0,8,9),
				array(3,6,8,0,10), array(4,7,9,10,0)
			),
			'chart_team_tbl' => array(
				array( 0, 1, 1, 1, 1 ), array( 2, 0, 1, 1, 1 ), array( 2, 2, 0, 1, 1 ),
				array( 2, 2, 2, 0, 1 ), array( 2, 2, 2, 2, 0 )
			)
		);
		return $param;
	}

	function get_league_parameter_13()
	{
		$param = array(
			'mw' => 'w',
			'team_num' => 4,
			...
			'group_num' => 1,
			'chart_tbl' => array(
				array( 0, 1, 2, 3 ), array( 1, 0, 4, 5 ),
				array( 2, 4, 0, 6 ), array( 3, 5, 6, 0 )
			),
			'chart_team_tbl' => array(
				array( 0, 1, 1, 1 ), array( 2, 0, 1, 1 ),
				array( 2, 2, 0, 1 ), array( 2, 2, 2, 0 )
			)
		);
		return $param;
	}

	function __get_tournament_parameter_12_13()
	{
		$param = array(
			'mw' => 'w',
			'team_num' => 16,
	...
	function get_tournament_parameter_12()
	{
		__get_tournament_parameter_12_13();
	}

	function get_tournament_parameter_13()
	{
		__get_tournament_parameter_12_13();
	}

	function init_entry_post_data_from_def_12_shumoku_dantai_m( &$data, $def, $add1, $add2 )
	{
	...
	function get_entry_post_data_from_def_12_shumoku_dantai_m( $def, $add1, $add2 )
	{
	...
	function get_entry_db_data_from_def_12_shumoku_dantai_m( $data, $list, $def, $add1, $add2 )
	{
	...
	function update_entry_data_12_shumoku_dantai_m( $def, $dbs )
	{
		$data = array(
	...
	function init_entry_post_data_from_def_12_shumoku_dantai_w( &$data, $def, $add1, $add2 )
	{
	...
	function get_entry_post_data_from_def_12_shumoku_dantai_w( $def, $add1, $add2 )
	{
	...
	function get_entry_db_data_from_def_12_shumoku_dantai_w( $data, $list, $def, $add1, $add2 )
	{
	...
	function update_entry_data_12_shumoku_dantai_w( $def, $dbs )
	{
		$data = array(
	...
	function init_entry_post_data_from_def_12_shumoku_dantai( &$data, $def, $add1, $add2 )
	{
	...
	function get_entry_post_data_from_def_12_shumoku_dantai( $def, $add1, $add2 )
	{
	...
	function get_entry_db_data_from_def_12_shumoku_dantai( $data, $list, $def, $add1, $add2 )
	{
	...
	function update_entry_data_12_shumoku_dantai( $def, $dbs )
	{
		$data = array(
	...
	function is_entry_data_12_join_shumoku_dantai( $data )
	{
	...
	function __get_entry_data_12_13_list_for_PDF( $series, $series_mw, $kaisai_rev )
	{
		$c = new common();
...
*/
	...
	function get_entry_data_12_list_for_PDF()
	{
		return __get_entry_data_12_13_list_for_PDF( 12, 'm', 1 );
	}

	function get_entry_data_13_list_for_PDF()
	{
		return __get_entry_data_12_13_list_for_PDF( 13, 'w', 1 );
	}


	function get_entry_data_list_12_sql( $mw )
	{
	...
	function output_league_12_for_HTML_2( $league_list, $entry_list, $mv )
	{
//print_r($league_list);
		if( $mv == 'm' ){
			$mvstr = '男子';
...
//							$html .= $league_list[$league_data]['match'][$match_no_index-1]['hon2'].'-'.$league_list[$league_data]['match'][$match_no_index-1]['win2'];
	...
	function __output_league_12_13_for_HTML( $path, $league_param, $league_list, $entry_list, $mv )
	{
//echo $path;
//print_r($league_list);
		if( $mv == 'm' ){
...
//							$html .= $league_list[$league_data]['match'][$match_no_index-1]['hon2'].'-'.$league_list[$league_data]['match'][$match_no_index-1]['win2'];
	...
	function output_league_12_for_HTML( $path, $league_list, $entry_list )
	{
		__output_league_12_13_for_HTML( $path, get_league_parameter_12(), $league_list, $entry_list, 'm' );
	}

	function output_league_13_for_HTML( $path, $league_list, $entry_list )
	{
		__output_league_12_13_for_HTML( $path, get_league_parameter_13(), $league_list, $entry_list, 'w' );
	}
	...
	function __output_league_12_13_for_Excel_one( $objPage, $sheet, $col, $row, $league_param, $league_list, $entry_list, $mv )
	{
//print_r($league_list);
		if( $mv == 'm' ){
			$mvstr = '男子';
	...
	function __output_league_12_13_for_Excel( $objPage, $path, $league_param, $league_list, $entry_list, $mv )
	{
//print_r($league_list);
		if( $mv == 'm' ){
			$mvstr = '男子';
		...
		$file_path = $path . '/H28shimoinaLeagueResults.xls';
		$file_name = 'H28shimoinaLeagueResults.xls';

		$reader = PHPExcel_IOFactory::createReader('Excel5');
		$excel = $reader->load(dirname(__FILE__).'/H28shimoinaLeagueResultsBase.xls');
		$excel->setActiveSheetIndex( 0 );		//何番目のシートに有効にするか
		$sheet = $excel->getActiveSheet();	//有効になっているシートを取得

		$league_param_m = get_league_parameter_12();
		$league_data_m = $objPage->get_dantai_league_list( 12, 'm' );
		$entry_list_m = $objPage->get_entry_data_list3( 12, 'm' );
		__output_league_12_13_for_Excel_one( $objPage, $sheet, 2, 6, $league_param_m, $league_data_m, $entry_list_m, 'm' );

		$league_param_w = get_league_parameter_13();
		$league_data_w = $objPage->get_dantai_league_list( 13, 'w' );
		$entry_list_w = $objPage->get_entry_data_list3( 13, 'w' );
		__output_league_12_13_for_Excel_one( $objPage, $sheet, 5, 24, $league_param_w, $league_data_w, $entry_list_w, 'w' );
	...
	function output_league_12_for_Excel( $objPage, $path, $league_list, $entry_list )
	{
		return __output_league_12_13_for_Excel( $objPage, $path, get_league_parameter_12(), $league_list, $entry_list, 'm' );
	}

	function output_league_13_for_Excel( $objPage, $path, $league_list, $entry_list )
	{
		return __output_league_12_13_for_Excel( $objPage, $path, get_league_parameter_13(), $league_list, $entry_list, 'w' );
	}

	//---------------------------------------------------------------
	//
	//---------------------------------------------------------------
	function __output_league_match_for_HTML2_12_13( $objPage, $path, $league_list, $entry_list, $mv )
	{
		if( $mv == 'm' ){
			$mvstr = '男子';
		} else {
	...
	function output_league_match_for_HTML2_12( $objPage, $path, $league_list, $entry_list )
	{
		__output_league_match_for_HTML2_12_13( $objPage, $path, $league_list, $entry_list, 'm' );
	}

	function output_league_match_for_HTML2_13( $objPage, $path, $league_list, $entry_list )
	{
		__output_league_match_for_HTML2_12_13( $objPage, $path, $league_list, $entry_list, 'w' );
	}
	...
	function __output_league_match_for_excel_12_13( $objPage, $path, $league_list, $entry_list, $mv )
	{
		if( $mv == 'm' ){
			$mvstr = '男子';
		} else {
		...
		$file_path = $path . '/H28shimoinaLeagueMatchResults_'.$mv.'.xls';
		$file_name = 'H28shimoinaLeagueMatchResults_'.$mv.'.xls';
		$reader = PHPExcel_IOFactory::createReader('Excel5');
		$excel = $reader->load(dirname(__FILE__).'/H28shimoinaLeagueMatchResultsBase_'.$mv.'.xls');
		$excel->setActiveSheetIndex( 0 );		//何番目のシートに有効にするか
		$sheet = $excel->getActiveSheet();	//有効になっているシートを取得
		//$sheet->setCellValueByColumnAndRow( 0, 1, '平成28年度全国中学校体育大会 第46回全国中学校剣道大会' );
		//$sheet->setCellValueByColumnAndRow( 0, 1, '第46回全国中学校剣道大会' );
		//$sheet->setCellValueByColumnAndRow( 0, 2, $mvstr.'団体戦　予選リーグ結果' );
		$col = 0;
		$row = 5;
		$colStr = 'Q';
		for( $league_data = 0; $league_data < count( $league_data ); $league_data++ ){
			for( $match = 0; $match < $league_list[$league_data]['match_num']; $match++ ){
				$objPage->output_one_match_for_excel( $sheet, $col, $row, $league_list[$league_data]['match'][$match], $entry_list, $mv, 46, 26 );
				$row += 6;
			}
		}
		$writer = PHPExcel_IOFactory::createWriter( $excel, 'Excel5' );
	...
	function output_league_match_for_excel_12( $objPage, $path, $league_list, $entry_list )
	{
		return __output_league_match_for_excel_12_13( $objPage, $path, $league_list, $entry_list, 'm' );
	}

	function output_league_match_for_excel_13( $objPage, $path, $league_list, $entry_list )
	{
		return __output_league_match_for_excel_12_13( $objPage, $path, $league_list, $entry_list, 'w' );
	}
	...
	function __output_tournament_12_13_for_excel( $objPage, $path, $tournament_data, $entry_list, $mv )
	{
		if( $mv == 'm' ){
			$mvstr = '男子';
		} else {
	...
	function output_tournament_12_for_excel( $objPage, $path, $tournament_data, $entry_list, $mv )
	{
		return __output_tournament_12_13_for_excel( $objPage, $path, $tournament_data, $entry_list, 'm' );
	}

	function output_tournament_13_for_excel( $objPage, $path, $tournament_data, $entry_list, $mv )
	{
		return __output_tournament_12_13_for_excel( $objPage, $path, $tournament_data, $entry_list, 'w' );
	}
	...
	function __output_tournament_chart_12_13_for_excel( $objPage, $path, $tournament_data, $league_data, $entry_list, $mv )
	{
		if( $mv == 'm' ){
			$mvstr = '男子';
		} else {
	...
	function output_tournament_chart_12_for_excel( $objPage, $path, $tournament_data, $league_data, $entry_list, $mv )
	{
		return __output_tournament_chart_12_13_for_excel( $objPage, $path, $tournament_data, $league_data, $entry_list, 'm' );
	}

	function output_tournament_chart_13_for_excel( $objPage, $path, $tournament_data, $league_data, $entry_list, $mv )
	{
		return __output_tournament_chart_12_13_for_excel( $objPage, $path, $tournament_data, $league_data, $entry_list, 'w' );
	}
	...
	function __output_tournament_match_for_HTML2_12_13( $objPage, $path, $tournament_list, $entry_list, $mv )
	{
		if( $mv == 'm' ){
			$mvstr = '男子';
		} else {
	...
	function output_tournament_match_for_HTML2_12( $objPage, $path, $tournament_list, $entry_list )
	{
		//__output_tournament_match_for_HTML2_12_13( $objPage, $path, $tournament_list, $entry_list, 'm' );
	}

	function output_tournament_match_for_HTML2_13( $objPage, $path, $tournament_list, $entry_list )
	{
		//__output_tournament_match_for_HTML2_12_13( $objPage, $path, $tournament_list, $entry_list, 'w' );
	}
	...
	function output_tournament_13_for_PDF( $tournament_data, $entry_list, $mv )
	{
	}

	function output_tournament_12_for_PDF( $tournament_data, $entry_list, $mv )
	{
...
exit;
	...
	function __output_tournament_12_13_for_HTML( $path, $tournament_data, $entry_list, $mv )
	{
//print_r($tournament_data);
//print_r($entry_list);
		if( $mv == 'm' ){
...
//exit;
	...
	function output_tournament_12_for_HTML( $path, $tournament_data, $entry_list )
	{
		//__output_tournament_12_13_for_HTML( $path, $tournament_data, $entry_list, 'm' );
	}

	function output_tournament_13_for_HTML( $path, $tournament_data, $entry_list )
	{
		//__output_tournament_12_13_for_HTML( $path, $tournament_data, $entry_list, 'w' );
	}
	...
	function __output_entry_data_list_all_1_excel12_13( $sheet, $series, $mv, $start_pos, $series_name, $kaisai_rev )
	{
		$c = new common();
	...
	function output_entry_data_list_all_1_excel12( $sheet )
	{
		__output_entry_data_list_all_1_excel12_13( $sheet, 12, 'm', 4, '団体戦男子', 1 );
		__output_entry_data_list_all_1_excel12_13( $sheet, 13, 'w', 4+48, '団体戦女子', 1 );
		return false;
	}

	function output_entry_data_list_all_1_excel13( $sheet )
	{
		__output_entry_data_list_all_1_excel12_13( $sheet, 12, 'm', 4, '団体戦男子', 1 );
		__output_entry_data_list_all_1_excel12_13( $sheet, 13, 'w', 4+48, '団体戦女子', 1 );
		return false;
	...
	function __get_entry_data_list2_12_13( $series )
	{
		$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
		$sql = 'select * from `entry_info`'
			.' where `series`='.$series.' and `year`='.__REG_SERIES_YEAR__
	...
	function get_entry_data_list2_12()
	{
		return __get_entry_data_list2_12_13( 12 );
	}

	function get_entry_data_list2_13()
	{
		return __get_entry_data_list2_12_13( 13 );
	}

	//--------------------------------------------------------------

	function __get_entry_data_for_draw_csv_12_13( $list, $series )
	{
		$c = new common();
	...
	function get_entry_data_for_draw_csv_12( $list )
	{
		return array(
			'csv' => __get_entry_data_for_draw_csv_12_13( $list, 12 ),
			'file' => 'dantai_m.csv'
		);
	}

	function get_entry_data_for_draw_csv_13( $list )
	{
		return array(
			'csv' => __get_entry_data_for_draw_csv_12_13( $list, 13 ),
			'file' => 'dantai_w.csv'
		);
	}
	...
	function __output_realtime_html_for_one_board_12_13( $place, $place_match_no )
	{
		global $navi_info;
...
*/
	...
	function output_realtime_html_for_one_board_12( $place, $place_match_no )
	{
		//return __output_realtime_html_for_one_board_12_13( $place, $place_match_no );
		return '';
	}

	function output_realtime_html_for_one_board_13( $place, $place_match_no )
	{
		//return __output_realtime_html_for_one_board_12_13( $place, $place_match_no );
		return '';
	}
