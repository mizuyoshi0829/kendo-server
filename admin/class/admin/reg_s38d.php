<?php
//--------------------------------------------------------------
    function __get_entry_data_list2_109_110( $series )
	{
	...
	function get_entry_data_list2_109()
	{
		return __get_entry_data_list2_109_110( 109 );
	}

	function get_entry_data_list2_110()
	{
		return __get_entry_data_list2_109_110( 110 );
	}
	//---------------------------------------------------------------
	//
	//---------------------------------------------------------------

	function __output_tournament_109_110_for_HTML( $series_info, $tournament_list, $entry_list, $mv, $cssparam )
	{
...
//print_r($entry_list);
	...
	//	$pdf = '' . "\n"
		...
		//    . '    position: absolute;' . "\n"
			...
			. '}' . "\n"
			. '</style>' . "\n";
		$pdf = $pdf_header . '<H1 style="border-bottom: solid 1px #000000;" lang="ja">'.$tournament_data['tournament_name'] . '&nbsp;結果</H1>' . "\n"
...
//exit;
	...
	function output_tournament_109_for_HTML( $series_info, $tournament_data, $entry_list )
	{
        $objPage = new form_page();
        ...
        $cssparam = [
			...
			'table_cell_width' => 30,
			'return_path' => 'index_j.html',
			'break_html' => [true],
			'break_html_name' => [
				'dt_jm1'
			],
        ];
		__output_tournament_109_110_for_HTML( $series_info, $tournament_data, $entry_list, 'm', $cssparam );
		//$objTournament->output_dantai_tournament_for_HTML( $series_info, $tournament_data, $entry_list, 'm',$cssparam );
	}

	function output_tournament_110_for_HTML( $series_info, $tournament_data, $entry_list )
	{
        $objPage = new form_page();
        $objTournament = new form_page_dantai_tournament( $objPage );
        $cssparam = [
		...
		__output_tournament_109_110_for_HTML( $series_info, $tournament_data, $entry_list, 'w', $cssparam );
	...
	function ____output_tournament_match_for_HTML2_109_110( $series_info, $tournament_list, $entry_list, $mw )
	{
        $objPage = new form_page();
		if( $mw == 'm' ){
			$mwstr = '男子';
    ...
    function __output_tournament_match_for_HTML2_109_110( $series_info, $tournament_list, $entry_list, $mw )
    {
        $objPage = new form_page();
        $header = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">'."\n";
        $header .= '<html xmlns="http://www.w3.org/1999/xhtml">'."\n";
        ...
        $header .= '<div class="container">'."\n";
        $header .= '  <div class="content">'."\n";
        $header .= '     <h2 align="left" class="tx-h1"><a href="index_j.html">←戻る</a></h2>'."\n";

        $footer = '     <h2 align="left" class="tx-h1"><a href="index_j.html">←戻る</a></h2>'."\n";
        $footer .= '    <br />'."\n";
        $footer .= '    <br />'."\n";
        $footer .= '    </div>'."\n";
        $footer .= '    <!-- end .content --></div>'."\n";
        ...
        for( $lindex = 1; $lindex < 2; $lindex++ ){
            ...
            $mnum = 32;
            $level = 0;
            $html = $header . '    <h2>' . $series_info['dantai_'.$mw.'_name'] . '&nbsp;結果</h2>'."\n";
            for(;;){
                $mno = 1;
                for( $i1 = 0; $i1 < $mnum; $i1++ ){
                ...
                if( $level == 1 || $level == 5 ){
                	if( $level == 5 ){
				        $html .= '<h3>三位決定戦</h3>';
        				$html .= $objPage->output_one_match_for_HTML2( $series_info, $tournament_list[0]['match'][63], $entry_list, $mw );
					}
                    $html .= $footer;
                    $file = 'dtm_j' . $mw . $lindex . '_' . $tindex;
                    $path = $series_info['result_path'] . '/' . $file . '.html';
                    $fp = fopen( $path, 'w' );
                    fwrite( $fp, $html );
                    fclose( $fp );
	...
	function output_tournament_match_for_HTML2_109( $series_info, $tournament_list, $entry_list )
	{
		__output_tournament_match_for_HTML2_109_110( $series_info, $tournament_list, $entry_list, 'm' );
	}

	function output_tournament_match_for_HTML2_110( $series_info, $tournament_list, $entry_list )
	{
		__output_tournament_match_for_HTML2_109_110( $series_info, $tournament_list, $entry_list, 'w' );
	}
