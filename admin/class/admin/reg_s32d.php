<?php
	...
	function __get_league_parameter_66_67( $series_mw )
	{
		$param = array(
			'mw' => $series_mw,
			'team_num' => 3,
	...
	function get_league_parameter_66()
	{
        return __get_league_parameter_66_67( 'm' );
	}

	function get_league_parameter_67()
	{
        return __get_league_parameter_66_67( 'w' );
	}

	function __get_tournament_parameter_66_67()
	{
		$param = array(
			'mw' => $series_mw,
			'team_num' => 8,
	...
	function get_tournament_parameter_66()
	{
		__get_tournament_parameter_66_67( 'm' );
	}

	function get_tournament_parameter_67()
	{
		__get_tournament_parameter_66_67( 'w' );
	}

	function __get_entry_data_66_67_list_for_PDF( $series, $series_mw, $kaisai_rev )
	{
		$c = new common();
...
*/
	...
	function get_entry_data_66_list_for_PDF()
	{
		return __get_entry_data_66_67_list_for_PDF( 16, 'm', 1 );
	}

	function get_entry_data_67_list_for_PDF()
	{
		return __get_entry_data_66_67_list_for_PDF( 17, 'w', 1 );
	}


	function get_entry_data_list_66_sql( $mw )
	{
	...
	function output_league_66_for_HTML_2( $league_list, $entry_list, $mv )
	{
//print_r($league_list);
		if( $mv == 'm' ){
			$mvstr = '男子';
...
//							$html .= $league_list[$league_data]['match'][$match_no_index-1]['hon2'].'-'.$league_list[$league_data]['match'][$match_no_index-1]['win2'];
	...
	function __output_league_66_67_for_HTML( $navi_info, $league_param, $league_list, $entry_list, $mw )
	{
//echo $path;
//print_r($league_list);
		if( $mw == 'm' ){
...
//							$html .= $league_list[$league_data]['match'][$match_no_index-1]['hon2'].'-'.$league_list[$league_data]['match'][$match_no_index-1]['win2'];
	...
	function output_league_66_for_HTML( $navi_info, $league_param, $league_list, $entry_list )
	{
        $objPage = new form_page();
        $objLeague = new form_page_dantai_league( $objPage );
//print_r($league_list);
        $objLeague->output_league_for_HTML( $navi_info, $league_param, $league_list, $entry_list, 'm', false, false );
		//__output_league_66_67_for_HTML( $path, get_league_parameter_66(), $league_list, $entry_list, 'm' );
	}

	function output_league_67_for_HTML( $navi_info, $league_param, $league_list, $entry_list )
	{
        $objPage = new form_page();
        $objLeague = new form_page_dantai_league( $objPage );
        $objLeague->output_league_for_HTML( $navi_info, $league_param, $league_list, $entry_list, 'w', false, false );
		//__output_league_66_67_for_HTML( $path, get_league_parameter_67(), $league_list, $entry_list, 'w' );
	}
	...
	function __output_league_66_67_for_Excel_one( $objPage, $sheet, $col, $row, $league_param, $league_list, $entry_list, $mv )
	{
//print_r($league_list);
		if( $mv == 'm' ){
			$mvstr = '男子';
	...
	function __output_league_66_67_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, $mw )
	{
//print_r($league_list);
		if( $mw == 'm' ){
			$mwstr = '男子';
        ...
        $file_name = $series_info['name'] . '_団体リーグ戦リーグ表_' . $mwstr . '.xls';
        ...
        $sheet->setCellValueByColumnAndRow( 0, 2, $mwstr.'団体リーグ戦　リーグ表' );
		...
		foreach( $league_list as $lv ){
			if( $team_num < intval( $lv['team_num'] ) ){
				$team_num = intval( $lv['team_num'] );
			}
		}
        for( $i1 = 0; $i1 <= $team_num+3; $i1++ ){
		...
		*/
		...
		$row = 4;
		for( $league_data = 0; $league_data < count( $league_list ); $league_data++ ){
			$match_tbl = $league_list[$league_data]['chart_tbl']; //$league_param['chart_tbl'];
			$team_num = intval( $league_list[$league_data]['team_num'] );
			//print_r($match_tbl);
			$sheet->setCellValueByColumnAndRow(
...
//echo "1:",$mg_range1,':',$mg_range2,'<br />';
			...
			$sheet->setCellValueByColumnAndRow( $col+$team_num*5+5+7, $row, '総本数' );
			$sheet->setCellValueByColumnAndRow( $col+$team_num*5+5+7, $row+1, '総勝者数' );
			$sheet->setCellValueByColumnAndRow( $col+$team_num*5+5+10, $row, '順位' );

            $mg_range1 = PHPExcel_Cell::stringFromColumnIndex($col+$team_num*5+5) . $row;
            $mg_range2 = PHPExcel_Cell::stringFromColumnIndex($col+$team_num*5+5+4) . ($row+1);
            $sheet->mergeCells( $mg_range1 . ':' . $mg_range2 );
            $sheet->getStyle($mg_range1)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
            $sheet->getStyle($mg_range1)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
            $sheet->getStyle($mg_range1.':'.$mg_range2)->applyFromArray($border_style);

            $mg_range1 = PHPExcel_Cell::stringFromColumnIndex($col+$team_num*5+5+7);
            ...
            $mg_range1 = PHPExcel_Cell::stringFromColumnIndex($col+$team_num*5+5+5) . $row;
            $mg_range2 = PHPExcel_Cell::stringFromColumnIndex($col+$team_num*5+5+5+4) . ($row+1);
            $sheet->getStyle($mg_range1.':'.$mg_range2)->applyFromArray($border_style);

            $mg_range1 = PHPExcel_Cell::stringFromColumnIndex($col+$team_num*5+5+10) . $row;
            $mg_range2 = PHPExcel_Cell::stringFromColumnIndex($col+$team_num*5+5+10+4) . ($row+1);
            $sheet->mergeCells( $mg_range1 . ':' . $mg_range2 );
            $sheet->getStyle($mg_range1)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
            $sheet->getStyle($mg_range1)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
            $sheet->getStyle($mg_range1.':'.$mg_range2)->applyFromArray($border_style);
			for( $dantai_index_row=0; $dantai_index_row<$league_list[$league_data]['team_num']; $dantai_index_row++ ){
				...
				$sheet->setCellValueByColumnAndRow(
					$colscore+5+2, $row+$dantai_index_row*2+3,
					$league_list[$league_data]['team'][$dantai_index_row]['win']
				);
				$sheet->setCellValueByColumnAndRow(
					$colscore+5+2, $row+$dantai_index_row*2+2,
					$league_list[$league_data]['team'][$dantai_index_row]['hon']
				);
				$sheet->setCellValueByColumnAndRow(
					$colscore+10, $row+$dantai_index_row*2+2,
					$league_list[$league_data]['team'][$dantai_index_row]['standing']
				);
                $mg_range1 = PHPExcel_Cell::stringFromColumnIndex($colscore) . ($row+$dantai_index_row*2+2);
                $mg_range2 = PHPExcel_Cell::stringFromColumnIndex($colscore+4) . ($row+$dantai_index_row*2+3);
                $sheet->mergeCells( $mg_range1 . ':' . $mg_range2 );
                $sheet->getStyle($mg_range1)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                $sheet->getStyle($mg_range1)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
                $sheet->getStyle($mg_range1.':'.$mg_range2)->applyFromArray($border_style);

                $mg_range1 = PHPExcel_Cell::stringFromColumnIndex($colscore+7) . ($row+$dantai_index_row*2+2);
                $sheet->getStyle($mg_range1)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                $sheet->getStyle($mg_range1)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_BOTTOM);
                $sheet->getStyle($mg_range1)->getBorders()->getBottom()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
                $mg_range1 = PHPExcel_Cell::stringFromColumnIndex($colscore+7) . ($row+$dantai_index_row*2+3);
                $sheet->getStyle($mg_range1)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                $sheet->getStyle($mg_range1)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP);
                $mg_range1 = PHPExcel_Cell::stringFromColumnIndex($colscore+5) . ($row+$dantai_index_row*2+2);
                $mg_range2 = PHPExcel_Cell::stringFromColumnIndex($colscore+5+4) . ($row+$dantai_index_row*2+3);
                $sheet->getStyle($mg_range1.':'.$mg_range2)->applyFromArray($border_style);

                $mg_range1 = PHPExcel_Cell::stringFromColumnIndex($colscore+10) . ($row+$dantai_index_row*2+2);
                $mg_range2 = PHPExcel_Cell::stringFromColumnIndex($colscore+10+4) . ($row+$dantai_index_row*2+3);
                $sheet->mergeCells( $mg_range1 . ':' . $mg_range2 );
                $sheet->getStyle($mg_range1)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                $sheet->getStyle($mg_range1)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
                $sheet->getStyle($mg_range1.':'.$mg_range2)->applyFromArray($border_style);
            ...
            $mg_range1 = PHPExcel_Cell::stringFromColumnIndex($col) . $row;
            $mg_range2 = PHPExcel_Cell::stringFromColumnIndex($col+$team_num*5+5+5*3-1) . ($row+$team_num*2+1);
            $sheet->getStyle($mg_range1.':'.$mg_range2)->applyFromArray($border_style);
			$row += ( $team_num + 2 ) * 2;
		}
        for( $i1 = 1; $i1 <= $row; $i1++ ){
	...
	function output_league_66_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_66_67_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, 'm' );
	}

	function output_league_67_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_66_67_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, 'w' );
	}

	//---------------------------------------------------------------
	//
	//---------------------------------------------------------------
	function __output_league_match_for_HTML2_66_67( $objPage, $path, $league_list, $entry_list, $mv )
	{
		if( $mv == 'm' ){
			$mvstr = '男子';
		} else {
	...
	function output_league_match_for_HTML2_66( $navi_info, $league_param, $league_list, $entry_list )
	{
		//__output_league_match_for_HTML2_66_67( $objPage, $path, $league_list, $entry_list, 'm' );
        $objPage = new form_page();
        $objLeague = new form_page_dantai_league( $objPage );
        $objLeague->output_league_match_for_HTML2( $navi_info, $league_param, $league_list, $entry_list, 'm', true );
	}

	function output_league_match_for_HTML2_67( $navi_info, $league_param, $league_list, $entry_list )
	{
		//__output_league_match_for_HTML2_66_67( $objPage, $path, $league_list, $entry_list, 'w' );
        $objPage = new form_page();
        $objLeague = new form_page_dantai_league( $objPage );
	...
	function __output_league_match_for_excel_66_67( $objPage, $series_info, $league_param, $league_list, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//print_r($league_list);
//exit;
		$file_name = $series_info['name'] . '_団体リーグ戦結果_' . $mwstr . '.xls';
		//$file_name = $series_info['reg_year'] . 'allnaganoJuniorDantaiLeagueMatchResults_'.$mw.'.xls';
		$file_path = $series_info['output_path'] . '/' . $file_name;
		$reader = PHPExcel_IOFactory::createReader('Excel5');
		$excel = $reader->load(dirname(dirname(dirname(__FILE__))).'/templates/excel/dantaiLeagueMatchResultsBase.xls');
		$excel->setActiveSheetIndex( 0 );		//何番目のシートに有効にするか
		$sheet = $excel->getActiveSheet();	//有効になっているシートを取得
		$sheet->setCellValueByColumnAndRow( 0, 2, $mwstr.'団体リーグ戦結果' );
		...
		for( $league_data = 0; $league_data < count( $league_list ); $league_data++ ){
			...
			for( $match = 0; $match < $league_list[$league_data]['match_num']; $match++ ){
                ...
                $mg_range = 'B' . ($row+5) . ":E" . ($row+5);
                $sheet->mergeCells( $mg_range );
                $mg_range = 'F' . ($row+5) . ":I" . ($row+5);
                $sheet->mergeCells( $mg_range );
                $mg_range = 'J' . ($row+5) . ":M" . ($row+5);
                $sheet->mergeCells( $mg_range );
                $mg_range = 'N' . ($row+5) . ":Q" . ($row+5);
                $sheet->mergeCells( $mg_range );
                $mg_range = 'R' . ($row+5) . ":U" . ($row+5);
                $sheet->mergeCells( $mg_range );
                $mg_range = 'Z' . ($row+5) . ":AB" . ($row+5);
                $sheet->mergeCells( $mg_range );
		...
		for( $league_data = 0; $league_data < count( $league_list ); $league_data++ ){
			...
			for( $match = 0; $match < $league_list[$league_data]['match_num']; $match++ ){
				...
				$row += 2;
				$objPage->output_one_match_for_excel( $sheet, $col, $row, $series_info, $league_list[$league_data]['match'][$league_list[$league_data]['place_match_info'][$match]], $entry_list, $mv, 46, 46 );
				$row += 5;
			}
		}
		$writer = PHPExcel_IOFactory::createWriter( $excel, 'Excel2007' );
	...
	function output_league_match_for_excel_66( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_match_for_excel_66_67( $objPage, $series_info, $league_param, $league_list, $entry_list, 'm' );
	}

	function output_league_match_for_excel_67( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_match_for_excel_66_67( $objPage, $series_info, $league_param, $league_list, $entry_list, 'w' );
	}
	...
	function __output_tournament_match_66_67_for_excel( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
			$match_num = 16;
		...
		require_once dirname(dirname(dirname(__FILE__))).'/phpExcel/Classes/PHPExcel.php';
		require_once dirname(dirname(dirname(__FILE__))).'/phpExcel/Classes/PHPExcel/IOFactory.php';
		//$file_name = $series_info['reg_year'] . 'allnaganoJuniorDantaiTournamentMatchResults_' . $mw . '.xls';
		$file_name = $series_info['name'] . '_団体トーナメント結果_' . $mwstr . '.xls';
		$file_path = $series_info['output_path'] . '/' . $file_name;

		$reader = PHPExcel_IOFactory::createReader('Excel5');
		$excel = $reader->load(dirname(dirname(dirname(__FILE__))).'/templates/excel/dantaiTournamentMatchResultsBase.xls');
		$excel->setActiveSheetIndex( 0 );		//何番目のシートに有効にするか
		$sheet = $excel->getActiveSheet();	//有効になっているシートを取得
		...
		$row = 4;
		$match1 = 1;
		$team_num = $tournament_data['team_num'];
		$match_num = $tournament_data['match_num'];
		$match_num_extra = $tournament_data['match_num'] + $tournament_data['extra_match_num'];
		$match_num2 = intval( $tournament_data['team_num'] / 2 );
		for( $match = 1; $match <= $match_num_extra; $match++ ){
            if( $match <= $match_num2 ){
				if( $tournament_data['match'][$match+$match_num2-2]['place'] === 'no_match' ){
					continue;
				}
  				$sheet->setCellValueByColumnAndRow( 0, $row, '一回戦 第'.($match1).'試合' );
				$match1++;
			} else if( $match <= $team_num-4 ){
				$sheet->setCellValueByColumnAndRow( 0, $row, '準々決勝 第'.($match+7-$match_num).'試合' );
			} else if( $match <= $team_num-2 ){
                $sheet->setCellValueByColumnAndRow( 0, $row, '準決勝 第'.($match+3-$match_num).'試合' );
            } else if( $match == $team_num-1 ){
                $sheet->setCellValueByColumnAndRow( 0, $row, '決勝' );
            } else if( $match == $team_num ){
				if( $tournament_data['extra_match_num'] == 0 ){ break; }
                $sheet->setCellValueByColumnAndRow( 0, $row, '三位決定戦' );
            }
            $row++;
			if( $row > 5 ){
	            for( $r = 0; $r < 5; $r++ ) {
		...
		$row = 6;
		if( $match_num > 8 ){
			for( $i1 = 7; $i1 < $match_num-1; $i1++ ){
				if( $tournament_data['match'][$i1]['place'] === 'no_match' ){
					continue;
				}
				$objPage->output_one_match_for_excel( $sheet, $col, $row, $series_info, $tournament_data['match'][$i1], $entry_list, $mv, 46, 46 );
				$row += 7;
			}
		}
		for( $i1 = 3; $i1 <= 6; $i1++ ){
        ...
        $objPage->output_one_match_for_excel( $sheet, $col, $row, $series_info, $tournament_data['match'][0], $entry_list, $mv, 46, 46 );
        $row += 7;
        $objPage->output_one_match_for_excel( $sheet, $col, $row, $series_info, $tournament_data['match'][$team_num-1], $entry_list, $mv, 46, 46 );
	...
	function output_tournament_match_66_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_match_66_67_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'm' );
	}

	function output_tournament_match_67_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_match_66_67_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'w' );
	}
	...
	function __output_tournament_66_67_for_excel( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//exit;
        $file_name = $series_info['reg_year'] . 'allnaganoJuniorDantaiTounamentResults_' . $mw . '.xls';
        $file_path = $series_info['output_path'] . '/' . $file_name;
        $excel = new PHPExcel();
        $excel->setActiveSheetIndex( 0 );        //何番目のシートに有効にするか
        $sheet = $excel->getActiveSheet();    //有効になっているシートを取得
        ...
        if( $tournament_data['match'][7]['winner'] == 1 ){
            ...
            $sheet->setCellValue( 'H'.($row+3), $tournament_data['match'][7]['team1_name'] );
            ...
            $sheet->getStyle( 'F'.($row+4) )->applyFromArray( $styleArrayHR );
            $sheet->setCellValue( 'H'.($row+3), $tournament_data['match'][7]['team2_name'] );
            $sheet->mergeCells( 'H'.($row+3).':'.'J'.($row+4) );
            $sheet->getStyle('H'.($row+3))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
            $sheet->getStyle('H'.($row+3))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
        } else {
        ...
        if( $tournament_data['match'][7]['winner'] == 1 ){
            $sheet->setCellValueByColumnAndRow( 2, $row, $tournament_data['match'][7]['team1_name'] );
        } else if( $tournament_data['match'][7]['winner'] == 2 ){
            $sheet->setCellValueByColumnAndRow( 2, $row, $tournament_data['match'][7]['team2_name'] );
        }
        $sheet->getStyle('A'.($row).':F'.$row)->getFont()->setSize(12);
        $sheet->getRowDimension($row)->setRowHeight(16);
        $row++;
	...
	function __output_tournament_66_67_for_excel2( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw, $series_m, $series_w )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//print_r($tournament_data);
//exit;
        //$file_name = $series_info['reg_year'] . 'allnaganoJuniorDantaiTounamentResults.xlsx';
		$file_name = $series_info['name'] . '_団体トーナメント表.xlsx';
        $file_path = $series_info['output_path'] . '/' . $file_name;
        $excel = new PHPExcel();
        $reader = PHPExcel_IOFactory::createReader('Excel2007');
		$excel = $reader->load(dirname(dirname(dirname(__FILE__))).'/templates/excel/allnaganoJuniorDantaiTournament2.xlsx');
        $excel->setActiveSheetIndex( 0 );        //何番目のシートに有効にするか
        $sheet = $excel->getActiveSheet();    //有効になっているシートを取得
        $sheet->getDefaultStyle()->getFont()->setName('ＭＳ Ｐゴシック');
        $sheet->getDefaultStyle()->getFont()->setSize(9);
		...
		$tournament_list_m = $objPage->get_dantai_tournament_data( $series_m, 'm' );
        $tournament_data = $tournament_list_m[0];
        $match_level = intval( $tournament_data['match_level'] );
        $tournament_team_num = intval( $tournament_data['tournament_team_num'] );
        $tournament_team_num2 = intval( $tournament_data['tournament_team_num']/2 );
        $col = 1;
        ...
        $index = 1;
        for( $team = 0; $team < $tournament_team_num; $team++ ){
			if($tournament_data['team'][$team]['name'] != ''){
				$colstr1 = PHPExcel_Cell::stringFromColumnIndex( $col );
				$colstr2 = PHPExcel_Cell::stringFromColumnIndex( $col+1 );
	            $sheet->setCellValue( $colstr1.$row, $tournament_data['team'][$team]['name'] );
				$sheet->getStyle( $colstr1.$row.':'.$colstr2.($row+4) )->applyFromArray( $styleArrayBox );
    	        $col += 4;
			}
		}
            ...
            for( $m = 0; $m < $match_num; $m++ ){
                ...
                if( $tournament_data['match'][$match]['place'] == 'no_match' ){
                    ...
                    if(
                        ...
                        $sheet->getStyle( $colstr.($row-3).':'.$colstr.($row+2) )->applyFromArray( $styleArrayVR );
                        $red = 0;
                    } else {
                        $sheet->getStyle( $colstr.($row-3).':'.$colstr.($row+2) )->applyFromArray( $styleArrayV );
                    }
                    if( $moffset == 0 ){
                        $posTbl1[$mup] = $col;
                        $posTbl3[$mup] = $red;
                    ...
                    $c3 = intval( ( $c1 + $c2 ) / 2 );
                    $colstr1 = PHPExcel_Cell::stringFromColumnIndex( $c1 );
                    $colstr11 = PHPExcel_Cell::stringFromColumnIndex( $c1+1 );
                    $colstr2 = PHPExcel_Cell::stringFromColumnIndex( $c2 );
                    $colstr3 = PHPExcel_Cell::stringFromColumnIndex( $c3 );
                    $colstr31 = PHPExcel_Cell::stringFromColumnIndex( $c3+1 );
                    if( $moffset == 0 ){
                        $posTbl1[$mup] = $c3;
                        $posTbl3[$mup] = 0;
                    } else {
                    ...
                    $winstr = $objPage->get_tounament_chart_winstring_for_excel( $tournament_data['match'][$match] );
                    $sheet->setCellValue( $colstr11.($row-1), $winstr[1][1].' '.$winstr[1][2] );
                    if( $tournament_data['match'][$match]['winner'] == 1 ){
                        $sheet->getStyle( $colstr1.$row.':'.$colstr1.($row+2) )->applyFromArray( $styleArrayVR );
                        $sheet->getStyle( $colstr11.$row.':'.$colstr3.$row )->applyFromArray( $styleArrayHR );
                    } else {
                        if( $level > 0 && $n1 == 0 ){
                            $sheet->getStyle( $colstr1.$row.':'.$colstr1.($row+2) )->applyFromArray( $styleArrayVR );
                        } else {
                            $sheet->getStyle( $colstr1.$row.':'.$colstr1.($row+2) )->applyFromArray( $styleArrayV );
						}
						$sheet->getStyle( $colstr11.$row.':'.$colstr3.$row )->applyFromArray( $styleArrayH );
                    }
                    if( $tournament_data['match'][$match]['winner'] == 2 ){
                        $sheet->getStyle( $colstr2.$row.':'.$colstr2.($row+2) )->applyFromArray( $styleArrayVR );
                        $sheet->getStyle( $colstr31.$row.':'.$colstr2.$row )->applyFromArray( $styleArrayHR );
                    } else {
                        if( $level > 0 && $n2 == 0 ){
                            $sheet->getStyle( $colstr2.$row.':'.$colstr2.($row+2) )->applyFromArray( $styleArrayVR );
						} else {
                            $sheet->getStyle( $colstr2.$row.':'.$colstr2.($row+2) )->applyFromArray( $styleArrayV );
                        }
						$sheet->getStyle( $colstr31.$row.':'.$colstr2.$row )->applyFromArray( $styleArrayH );
                    }
                    $sheet->setCellValue( $colstr2.($row-1), $winstr[2][1].' '.$winstr[2][2] );
                    $col += 8;
                }
            }
            $row -= 3;
        ...
        // 決勝
		$colstr = PHPExcel_Cell::stringFromColumnIndex( $posTbl1[0] );
        $sheet->getStyle( $colstr.'7:'.$colstr.'9' )->applyFromArray( $styleArrayVR );
		$colstr1 = PHPExcel_Cell::stringFromColumnIndex( $posTbl1[0]-2 );
		$colstr2 = PHPExcel_Cell::stringFromColumnIndex( $posTbl1[0]+3 );
        $sheet->mergeCells( $colstr1.'5:'.$colstr2.'5' );
		if( $tournament_data['match'][0]['winner'] == 1 ){
            $sheet->setCellValue( $colstr1.'5', $tournament_data['match'][0]['team1_name'] );
        } else if( $tournament_data['match'][0]['winner'] == 2 ){
            $sheet->setCellValue( $colstr1.'5', $tournament_data['match'][0]['team2_name'] );
        }

		// 3決
        if( $tournament_data['match'][1]['winner'] == 1 ){
            $sheet->setCellValue( 'BM13', $tournament_data['match'][1]['team2_name'] );
        } else if( $tournament_data['match'][1]['winner'] == 2 ){
            $sheet->setCellValue( 'BM13', $tournament_data['match'][1]['team1_name'] );
        }
        if( $tournament_data['match'][2]['winner'] == 1 ){
            $sheet->setCellValue( 'BQ13', $tournament_data['match'][2]['team2_name'] );
        } else if( $tournament_data['match'][2]['winner'] == 2 ){
            $sheet->setCellValue( 'BQ13', $tournament_data['match'][2]['team1_name'] );
        }
        if( $tournament_data['match'][$tournament_team_num-1]['winner'] == 1 ){
            $sheet->getStyle( 'BM9:BM11' )->applyFromArray( $styleArrayVR );
            $sheet->getStyle( 'BN9:BO9' )->applyFromArray( $styleArrayHR );
        } else if( $tournament_data['match'][$tournament_team_num-1]['winner'] == 2 ){
            $sheet->getStyle( 'BQ9:BQ11' )->applyFromArray( $styleArrayVR );
            $sheet->getStyle( 'BP9:BQ9' )->applyFromArray( $styleArrayHR );
        }
		$sheet->getStyle( 'BO7:BO8' )->applyFromArray( $styleArrayVR );
        $winstr = $objPage->get_tounament_chart_winstring_for_excel( $tournament_data['match'][$tournament_team_num-1] );
        $sheet->setCellValue( 'BM8', $winstr[1][1].' '.$winstr[1][2] );
        $sheet->setCellValue( 'BP8', $winstr[2][2].' '.$winstr[2][1] );

        if( $tournament_data['match'][0]['winner'] == 1 ){
            $sheet->setCellValue( 'A33', $tournament_data['match'][0]['team1_name'] );
        } else if( $tournament_data['match'][0]['winner'] == 2 ){
            $sheet->setCellValue( 'A33', $tournament_data['match'][0]['team2_name'] );
        }
        if( $tournament_data['match'][0]['winner'] == 1 ){
            $sheet->setCellValue( 'K33', $tournament_data['match'][0]['team2_name'] );
        } else if( $tournament_data['match'][0]['winner'] == 2 ){
            $sheet->setCellValue( 'K33', $tournament_data['match'][0]['team1_name'] );
        }
        if( $tournament_data['match'][$tournament_team_num-1]['winner'] == 1 ){
            $sheet->setCellValue( 'U33', $tournament_data['match'][$tournament_team_num-1]['team1_name'] );
        } else if( $tournament_data['match'][$tournament_team_num-1]['winner'] == 2 ){
            $sheet->setCellValue( 'U33', $tournament_data['match'][$tournament_team_num-1]['team2_name'] );
        }
        if( $tournament_data['match'][$tournament_team_num-1]['winner'] == 1 ){
            $sheet->setCellValue( 'AE33', $tournament_data['match'][$tournament_team_num-1]['team2_name'] );
        } else if( $tournament_data['match'][$tournament_team_num-1]['winner'] == 2 ){
            $sheet->setCellValue( 'AE33', $tournament_data['match'][$tournament_team_num-1]['team1_name'] );
        }

		$tournament_list_w = $objPage->get_dantai_tournament_data( $series_w, 'w' );
        $tournament_data = $tournament_list_w[0];
        $match_level = intval( $tournament_data['match_level'] );
        $tournament_team_num = intval( $tournament_data['tournament_team_num'] );
        $tournament_team_num2 = intval( $tournament_data['tournament_team_num']/2 );
        $col = 1;
        ...
        $index = 1;
        for( $team = 0; $team < $tournament_team_num; $team++ ){
            $sheet->setCellValue( PHPExcel_Cell::stringFromColumnIndex( $col ).$row, $tournament_data['team'][$team]['name'] );
            $col += 4;
        }
        ...
        // 決勝
        $sheet->getStyle( 'P40:P43' )->applyFromArray( $styleArrayVR );
        if( $tournament_data['match'][0]['winner'] == 1 ){
            $sheet->setCellValue( 'N39', $tournament_data['match'][0]['team1_name'] );
        } else if( $tournament_data['match'][0]['winner'] == 2 ){
            $sheet->setCellValue( 'N39', $tournament_data['match'][0]['team2_name'] );
        }

        // 3決
        if( $tournament_data['match'][1]['winner'] == 1 ){
            $sheet->setCellValue( 'AH48', $tournament_data['match'][1]['team2_name'] );
        } else if( $tournament_data['match'][1]['winner'] == 2 ){
            $sheet->setCellValue( 'AH48', $tournament_data['match'][1]['team1_name'] );
        }
        if( $tournament_data['match'][2]['winner'] == 1 ){
            $sheet->setCellValue( 'AL48', $tournament_data['match'][2]['team2_name'] );
        } else if( $tournament_data['match'][2]['winner'] == 2 ){
            $sheet->setCellValue( 'AL48', $tournament_data['match'][2]['team1_name'] );
        }
        if( $tournament_data['match'][7]['winner'] == 1 ){
            $sheet->getStyle( 'AH44:AH46' )->applyFromArray( $styleArrayVR );
            $sheet->getStyle( 'AI44:AJ44' )->applyFromArray( $styleArrayHR );
            $sheet->getStyle( 'AJ42:AJ43' )->applyFromArray( $styleArrayVR );
        } else if( $tournament_data['match'][7]['winner'] == 2 ){
            $sheet->getStyle( 'AL44:AL46' )->applyFromArray( $styleArrayVR );
            $sheet->getStyle( 'AK44:AL44' )->applyFromArray( $styleArrayHR );
            $sheet->getStyle( 'AJ42:AJ43' )->applyFromArray( $styleArrayVR );
        }
        $winstr = $objPage->get_tounament_chart_winstring_for_excel( $tournament_data['match'][$tournament_team_num-1] );
        $sheet->setCellValue( 'AH43', $winstr[1][1].' '.$winstr[1][2] );
        $sheet->setCellValue( 'AM43', $winstr[2][2].' '.$winstr[2][1] );

        if( $tournament_data['match'][0]['winner'] == 1 ){
            $sheet->setCellValue( 'A64', $tournament_data['match'][0]['team1_name'] );
        } else if( $tournament_data['match'][0]['winner'] == 2 ){
            $sheet->setCellValue( 'A64', $tournament_data['match'][0]['team2_name'] );
        }
        if( $tournament_data['match'][0]['winner'] == 1 ){
            $sheet->setCellValue( 'K64', $tournament_data['match'][0]['team2_name'] );
        } else if( $tournament_data['match'][0]['winner'] == 2 ){
            $sheet->setCellValue( 'K64', $tournament_data['match'][0]['team1_name'] );
        }
        if( $tournament_data['match'][7]['winner'] == 1 ){
            $sheet->setCellValue( 'U64', $tournament_data['match'][7]['team1_name'] );
        } else if( $tournament_data['match'][7]['winner'] == 2 ){
            $sheet->setCellValue( 'U64', $tournament_data['match'][7]['team2_name'] );
        }
        if( $tournament_data['match'][7]['winner'] == 1 ){
            $sheet->setCellValue( 'AE64', $tournament_data['match'][7]['team2_name'] );
        } else if( $tournament_data['match'][7]['winner'] == 2 ){
            $sheet->setCellValue( 'AE64', $tournament_data['match'][7]['team1_name'] );
        }
	...
	function output_tournament_chart_66_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, $mw )
	{
		return __output_tournament_66_67_for_excel2( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'm', 66, 67 );
	}

	function output_tournament_chart_67_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, $mw )
	{
		return __output_tournament_66_67_for_excel2( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'w', 66, 67 );
	}
	...
	function __output_tournament_chart_66_67_for_excel( $objPage, $series_info, $tournament_list, $league_data, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
		...
		require_once dirname(dirname(dirname(__FILE__))).'/phpExcel/Classes/PHPExcel.php';
		require_once dirname(dirname(dirname(__FILE__))).'/phpExcel/Classes/PHPExcel/IOFactory.php';
		$file_name = $series_info['reg_year'] . 'nanshinJuniorDantaiTournamentMatchResults_'.$mw.'.xls';
		$file_path = $series_info['output_path'] . '/' . $file_name;
		$reader = PHPExcel_IOFactory::createReader('Excel2007');
		$excel = $reader->load(dirname(dirname(dirname(__FILE__))).'/templates/excel/dantaiTournamentChartBase.xls');
		$excel->setActiveSheetIndex( 0 );		//何番目のシートに有効にするか
		$sheet = $excel->getActiveSheet();	//有効になっているシートを取得
		$sheet->setCellValue( 'X4', $mwstr.'団体戦結果' );
	...
	function output_tournament_chart_66_for_excel_( $objPage, $series_info, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_chart_66_67_for_excel( $objPage, $series_info, $tournament_data, $league_data, $entry_list, 'm' );
	}

	function output_tournament_chart_67_for_excel_( $objPage, $series_info, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_chart_66_67_for_excel( $objPage, $series_info, $tournament_data, $league_data, $entry_list, 'w' );
	}
	...
	function __output_tournament_match_for_HTML2_66_67( $series_info, $tournament_list, $entry_list, $mw )
	{
        $objPage = new form_page();
		if( $mw == 'm' ){
			$mwstr = '男子';
		...
		$header .= '<body>'."\n";
		$header .= '<!--'."\n";
		$header .= print_r($tournament_list[0],true) . "\n";
		$header .= print_r($entry_list,true) . "\n";
		$header .= '-->'."\n";
		$header .= '<div class="container">'."\n";
		$header .= '  <div class="content">'."\n";
        ...
        if( $mw == 'm' ){
            $mno = 1;
    		for( $i1 = 1; $i1 <= 8; $i1++ ){
                if( $tournament_list[0]['match'][$i1+6]['place'] != 'no_match' ){
    	    		$html .= '<h3>団体決勝トーナメント&nbsp;一回戦&nbsp;第'.$mno.'試合</h3>';
		...
		$html .= '<h3>三位決定戦</h3>';
		$html .= $objPage->output_one_match_for_HTML2( $series_info, $tournament_list[0]['match'][$tournament_list[0]['tournament_team_num']-1], $entry_list, $mw );
		$html .= $footer;
	...
	function output_tournament_match_for_HTML2_66( $series_info, $tournament_list, $entry_list )
	{
		__output_tournament_match_for_HTML2_66_67( $series_info, $tournament_list, $entry_list, 'm' );
	}

	function output_tournament_match_for_HTML2_67( $series_info, $tournament_list, $entry_list )
	{
		__output_tournament_match_for_HTML2_66_67( $series_info, $tournament_list, $entry_list, 'w' );
	}
	...
	function output_tournament_67_for_PDF( $tournament_data, $entry_list, $mv )
	{
	}

	function output_tournament_66_for_PDF( $tournament_data, $entry_list, $mv )
	{
//print_r($tournament_data);
		if( $mv == 'm' ){
			$mvstr = '男子';
...
exit;
	...
	function __output_tournament_66_67_for_HTML( $series_info, $tournament_list, $entry_list, $mv )
	{
//print_r($tournament_data);
//print_r($entry_list);
        $objPage = new form_page();
		...
		for( $tournament_team = 0; $tournament_team < $team_num; $tournament_team++ ){
			...
			if( $id > 0 ){
				foreach( $entry_list as $ev ){
					if( $id == intval( $ev['id'] ) ){
						$name = '';
						if( isset($ev['school_name_ryaku']) && $ev['school_name_ryaku'] != '' ){
							$name = $ev['school_name_ryaku'];
						} else {
							$name = $ev['school_name'];
						}
						$pref = isset($ev['school_address_pref_name']) ? $ev['school_address_pref_name'] : '';
						break;
					}
				}
			}
...
//exit;
	...
	function output_tournament_66_for_HTML( $series_info, $tournament_data, $entry_list )
	{
		__output_tournament_66_67_for_HTML( $series_info, $tournament_data, $entry_list, 'm' );
	}

	function output_tournament_67_for_HTML( $series_info, $tournament_data, $entry_list )
	{
		__output_tournament_66_67_for_HTML( $series_info, $tournament_data, $entry_list, 'w' );
	}
	...
	function __output_entry_data_list_all_1_excel66_67( $sheet, $series, $mv, $start_pos, $series_name, $kaisai_rev )
	{
		$c = new common();
	...
	function output_entry_data_list_all_1_excel66( $sheet )
	{
		__output_entry_data_list_all_1_excel66_67( $sheet, 16, 'm', 4, '団体戦男子', 1 );
		__output_entry_data_list_all_1_excel16_67( $sheet, 17, 'w', 4+48, '団体戦女子', 1 );
		return false;
	}

	function output_entry_data_list_all_1_excel67( $sheet )
	{
		__output_entry_data_list_all_1_excel66_67( $sheet, 16, 'm', 4, '団体戦男子', 1 );
		__output_entry_data_list_all_1_excel66_67( $sheet, 17, 'w', 4+48, '団体戦女子', 1 );
		return false;
	}

	//--------------------------------------------------------------

	function __get_entry_data_list2_66_67( $series )
	{
		$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
		$sql = 'select * from `entry_info`'
			.' where `series`='.$series.' and `year`='.$_SESSION['auth']['year']
	...
	function get_entry_data_list2_66()
	{
		return __get_entry_data_list2_66_67( 66 );
	}

	function get_entry_data_list2_67()
	{
		return __get_entry_data_list2_66_67( 67 );
	}

	//--------------------------------------------------------------

	function __get_entry_data_for_draw_csv_66_67( $list, $series )
	{
		$c = new common();
	...
	function get_entry_data_for_draw_csv_66( $list )
	{
		return array(
			'csv' => __get_entry_data_for_draw_csv_66_67( $list, 16 ),
			'file' => 'dantai_m.csv'
		);
	}

	function get_entry_data_for_draw_csv_67( $list )
	{
		return array(
			'csv' => __get_entry_data_for_draw_csv_66_67( $list, 17 ),
			'file' => 'dantai_w.csv'
		);
	}
	...
	function __output_realtime_html_for_one_board_66_67( $place, $place_match_no )
	{
		global $navi_info;
...
*/
	...
	function output_realtime_html_for_one_board_66( $place, $place_match_no )
	{
		//return __output_realtime_html_for_one_board_66_67( $place, $place_match_no );
		return '';
	}

	function output_realtime_html_for_one_board_67( $place, $place_match_no )
	{
		//return __output_realtime_html_for_one_board_66_67( $place, $place_match_no );
		return '';
	}
