...
//echo $sql;
		...
		foreach( $list as &$lv ){
            ...
            if( count( $m ) > 0 ){
                ...
                $lv['match_info'] = $league_param['match_info'];
            }
			$m = explode( ',', $lv['place_match_info_array'] );
			$lv['place_match_info'] = array();
            if( count( $m ) > 0 ){
                ...
                $lv['place_match_info'] = $league_param['place_match_info'];
            }
			$lv['chart_team_tbl'] = array();
			$m = explode( ',', $lv['chart_team_tbl_array'] );
            if( count( $m ) > 0 ){
                ...
                $lv['chart_team_tbl'] = $league_param['chart_team_tbl'];
            }
			$lv['chart_tbl'] = array();
			$m = explode( ',', $lv['chart_tbl_array'] );
            if( count( $m ) > 0 ){
                ...
                $lv['chart_tbl'] = $league_param['chart_tbl'];
            }
			$lv['end_match'] = 0;
			$lv['team'] = array();
			$team_num = intval( $lv['team_num'] );
...
//echo $sql,"<br />\n";
			...
			$sql = 'update `dantai_league` set `extra_match_exists`='.intval($post['extra_match_exists_'.$league_index]).' where `id`='.$id;
//echo $sql,"<br />\n";
			db_query( $dbs, $sql );
			$league_index++;
		}
	...
	function output_league_match_for_HTML2( $navi_info, $league_param, $league_list, $entry_list, $mw )
	{
		...
		for( $league_data = 0; $league_data < count( $league_list ); $league_data++ ){
			for( $match_index = 0; $match_index < count( $league_list[$league_data]['match'] ); $match_index++ ){
				...
				$html .= $this->output_one_match_for_HTML2( $navi_info, $league_list[$league_data]['match'][$league_param['place_match_info'][$match_index]], $entry_list, $mv );
			//$html .= '<h3>' . $league_list[$league_data]['name'] . '&nbsp;第2試合</h3>';
