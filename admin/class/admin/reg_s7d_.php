...
*/
	...
	{
		return __get_entry_data_32_33_list_for_PDF( 32, 'm', 1 );
	}

	function get_entry_data_33_list_for_PDF()
	{
		return __get_entry_data_32_33_list_for_PDF( 33, 'w', 1 );
	}
...
//print_r($league_list);
		...
		$file_name = 'H33nanshinJuniorDantaiLeagueResults_' . $mw . '.xls';
        ...
        for( $i1 = 0; $i1 <= $team_num; $i1++ ){
            ...
            $sheet->getColumnDimension(PHPExcel_Cell::stringFromColumnIndex($col++))->setWidth(2.33+0.71);
...
//print_r($league_list);
//exit;
		$file_name = 'H29nanshinJuniorDantaiLeagueMatchResults_'.$mw.'.xls';
		$file_path = $series_info['output_path'] . '/' . $file_name;
	...
	function __output_tournament_match_32_33_for_excel( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw )
	{
		...
		require_once dirname(dirname(dirname(__FILE__))).'/phpExcel/Classes/PHPExcel.php';
		require_once dirname(dirname(dirname(__FILE__))).'/phpExcel/Classes/PHPExcel/IOFactory.php';
		$file_name = 'H29nanshinJuniorDantaiTournamentMatchResults_' . $mw . '.xls';
		$file_path = $series_info['output_path'] . '/' . $file_name;
...
//exit;
        $file_name = 'H29nanshinJuniorDantaiTounamentResults_' . $mw . '.xls';
        $file_path = $series_info['output_path'] . '/' . $file_name;
        $excel = new PHPExcel();
        $excel->setActiveSheetIndex( 0 );        //何番目のシートに有効にするか
        $sheet = $excel->getActiveSheet();    //有効になっているシートを取得
	...
	{
		return __output_tournament_32_33_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'm' );
	}

	function output_tournament_33_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_32_33_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'w' );
	}
...
//exit;
	...
	{
		__output_entry_data_list_all_1_excel32_33( $sheet, 32, 'm', 4, '団体戦男子', 1 );
		__output_entry_data_list_all_1_excel32_33( $sheet, 33, 'w', 4+48, '団体戦女子', 1 );
		return false;
	}
	...
	{
		return array(
			'csv' => __get_entry_data_for_draw_csv_32_33( $list, 32 ),
			'file' => 'dantai_m.csv'
		);
	}
	...
	{
		return array(
			'csv' => __get_entry_data_for_draw_csv_32_33( $list, 33 ),
			'file' => 'dantai_w.csv'
		);
	}
