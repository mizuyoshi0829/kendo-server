<?php
	...
	function get_tournament_parameter_11()
	{
		$param = array(
			'mw' => 'w',
			'team_num' => 64,
	...
	function get_tournament_parameter_65()
	{
		$param = array(
			'mw' => 'w',
			'team_num' => 64,
	...
	function __output_tournament_chart_11_54_for_HTML( $tournament_data, $entry_list )
	{
//echo '<!-- ';
...
//exit;
	...
	function output_tournament_chart_11_for_HTML( $tournament_data, $entry_list )
	{
		return __output_tournament_chart_11_54_for_HTML( $tournament_data, $entry_list );
	}

	function output_tournament_chart_54_for_HTML( $tournament_data, $entry_list )
	{
		return __output_tournament_chart_11_54_for_HTML( $tournament_data, $entry_list );
	}
	...
	function __output_tournament_11_65_for_excel( $objPage, $series_info, $tournament_data, $entry_list, $series_mw )
    {
//print_r($tournament_data);
//exit;
		...
		$mv = $tournament_data['data'][0]['series_mw'];

		if( $mv == 'm' ){
			$mwstr = '男子';
			$file_name = '2021AllJapanResult.xlsx';
		} else {
			$mwstr = '女子';
			$file_name = '2021AllJapanWomenResult.xlsx';
		}
        //$file_name = 'H29nanshinJuniorKojinTounamentResults_' . $mw . '.xlsx';
        $file_path = $series_info['output_path'] . '/' . $file_name;
        $excel = new PHPExcel();
        $reader = PHPExcel_IOFactory::createReader('Excel2007');
        $excel = $reader->load(dirname(__FILE__).'/allJapanWomen.xlsx');
        ...
        $sheet->setCellValueByColumnAndRow( 0, 1, $tournament_data['data'][0]['tournament_name'] . '試合結果' );
        ...
        for( $level = 0; $level < $match_level-1; $level++ ){
            ...
            for( $m = 0; $m < $match_num; $m++ ){
                ...
                if( $tournament_data['data'][0]['match'][$match]['place'] == 'no_match' ){
                    ...
                    if( $tournament_data['data'][0]['match'][$match]['faul1_2'] == 1 ){
                        $sheet->setCellValue( $colstr2.($r1-1), '▲' );
                    }
                    ...
                    if( $tournament_data['data'][0]['match'][$match]['faul2_2'] == 1 ){
                        $sheet->setCellValue( $colstr2.($r2), '▲' );
                    }
        ...
        if( $tournament_data['data'][0]['match'][0]['faul1_2'] == 1 ){
            $sheet->setCellValue( $col1.($row+2), '▲' );
        }
        ...
        if( $tournament_data['data'][0]['match'][0]['winner'] == 1 ){
            ...
            $sheet->setCellValue( 'L38', $tournament_data['data'][0]['match'][0]['player1_name'].'('.$tournament_data['data'][0]['match'][0]['player1_belonging_to_name'].')' );
            ...
            $sheet->setCellValue( 'L38', $tournament_data['data'][0]['match'][0]['player2_name'].'('.$tournament_data['data'][0]['match'][0]['player2_belonging_to_name'].')' );
        } else {
            $sheet->getStyle( $col1.$row.':'.$col1b.$row )->applyFromArray( $styleArrayH );
            $sheet->getStyle( $col2.$row.':'.$col2b.$row )->applyFromArray( $styleArrayH );
        ...
        if( $tournament_data['data'][0]['match'][0]['faul2_2'] == 1 ){
            $sheet->setCellValue( $col2b.($row+2), '▲' );
        }
        ...
        if( $tournament_data['data'][0]['match'][0]['extra'] == 1 ){
            $sheet->setCellValue( $col1b.($row+2), '延' );
            $sheet->mergeCells( $col1b.($row+2).':'.$col2.($row+3) );
            $sheet->getStyle($col1b.($row+2))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
            $sheet->getStyle($col1b.($row+2))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
        }
	...
	function output_tournament_11_for_excel( $objPage, $series_info, $tournament_data, $entry_list, $series_mw )
    {
		return __output_tournament_11_65_for_excel( $objPage, $series_info, $tournament_data, $entry_list, $series_mw );
	}

	function output_tournament_65_for_excel( $objPage, $series_info, $tournament_data, $entry_list, $series_mw )
    {
		return __output_tournament_11_65_for_excel( $objPage, $series_info, $tournament_data, $entry_list, $series_mw );
	}
	...
	function __output_tournament_11_65_for_HTML( $path, $tournament_data, $entry_list )
	{
//echo '<!-- ';
//print_r($tournament_data);
//echo ' -->';
	...
	//	$pdf = '' . "\n"
			...
			. '<title>' . $tournament_data['data'][0]['tournament_name'] . '試合結果速報</title>' . "\n"
		...
		//    . '    position: absolute;' . "\n"
			...
			. '}' . "\n"
			. '</style>' . "\n"
			. '<H1 style="border-bottom: solid 1px #000000;" lang="ja">' . $tournament_data['data'][0]['tournament_name'] . '試合結果速報</H1>' . "\n"
			...
			for( $line = 0; $line < 2; $line++ ){
				...
				for( $i2 = 0; $i2 < count( $match_tbl[$line][$i1-1] ); $i2++ ){
					...
					//	$one_match['match_no'] = $tournament_data['match'][$match_no-1]['dantai_tournament_match_id'];
						...
						$one_match['place'] = $tournament_data['data'][0]['match'][$match_no-1]['place'];
						$one_match['place_match_no'] = $tournament_data['data'][0]['match'][$match_no-1]['place_match_no'];
						$one_match['team1'] = array( 'pos' => $pos );
						$match_no++;
					} else {
						$one_match['team2'] = array( 'pos' => $pos );
						if(
							$match_tbl[$line][$i1-1][$i2-1]['place'] === 'no_match'
							&& $match_tbl[$line][$i1-1][$i2]['place'] === 'no_match'
						){
...
//print_r($match_tbl);
		...
		for(;;){
			...
			if( $line == $line2-1 ){
				...
				$pdf .= '<td height="'.$table_height.'" class="div_border_br'.$win.' div_result_one_tournament text_center">';
				...
				$pdf .= '<td height="'.$table_height.'" class="div_border_b'.$win.' div_result_one_tournament text_center">';
				...
				$pdf .= '<td height="'.$table_height.'" class="div_border_none div_result_one_tournament text_center">１２３</td>';
				$pdf .= '<td height="'.$table_height.'" class="div_border_none div_result_one_tournament text_center">１２３</td>';
			} else if( $line == $line2 + 2 ){
				$pdf .= '<td height="'.$table_height.'" class="div_border_none div_result_one_tournament">';
				if( $tournament_data['data'][0]['match'][0]['extra'] == 1 ){
                    $pdf .= '<span class="text_extra3">延</span>';
				}
				$pdf .= '</td>';
				$pdf .= '<td height="'.$table_height.'" class="div_border_none div_result_one_tournament"></td>';
			} else {
					...
					if( $line == $one_match_tbl['team2']['pos'] ){
						...
						if( $trspan2[$level] > 0 ){
							...
							if( $level == $tournament_data['data'][0]['match_level']-2 ){
								...
								if( $line == $line2 && $one_match_tbl['extra'] == 1 ){
									$extra = '<span class="text_extra2">延</span>';
								}
							} else {
								if( ( $line <= $trmatch2[$level] && $one_match_tbl['winner'] == 1 )
									|| ( $line > $trmatch2[$level] && $one_match_tbl['winner'] == 2 )
...
//exit;
	...
	function output_tournament_11_for_HTML( $path, $tournament_data, $entry_list )
	{
		__output_tournament_11_65_for_HTML( $path, $tournament_data, $entry_list );
	}

	function output_tournament_65_for_HTML( $path, $tournament_data, $entry_list )
	{
		__output_tournament_11_65_for_HTML( $path, $tournament_data, $entry_list );
	}
	...
	function output_entry_data_list_all_1_excel11( $sheet, $series, $series_mw, $start_pos, $series_name )
	{
		$c = new common();
	...
	function output_entry_data_list_all_2_excel11( $sheet, $series, $series_mw, $start_pos, $series_name )
	{
		$c = new common();
	...
	function __get_entry_data_list2_11_65( $series, $mw )
	{
		$c = new common();
		...
		foreach( $list as $lv ){
			...
			$lv['join'] = 1;
			$lv['school_name'] = get_field_string( $fields, 'kojin_'.$mw.'1_sei' )
			    . ' ' . get_field_string( $fields, 'kojin_'.$mw.'1_mei' )
				. '(' . $c->get_pref_name( $pref_array, get_field_string_number( $fields, 'kojin_address_pref', 0 ) ) . ')';
			$ret[] = $lv;
		}
		db_close( $dbs );
	...
	function get_entry_data_list2_11( $series=11 )
	{
		return __get_entry_data_list2_11_65( 11, 'w' );
	}

	function get_entry_data_list2_65( $series=65 )
	{
		return __get_entry_data_list2_11_65( 65, 'm' );
	}

	//--------------------------------------------------------------

	function get_entry_data_11_list_for_PDF( $series, $series_mw )
	{
		$c = new common();
...
//print_r($list);
	...
	function get_entry_data_for_draw_csv_11( $list, $series, $series_mw )
	{
		$c = new common();
		$preftbl = $c->get_pref_array();
		$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
	...
	function output_realtime_html_for_one_board_11( $place, $place_match_no )
	{
		global $navi_info;
