<?php
	...
	function __get_league_parameter_70_71( $series_mw )
	{
		$param = array(
			'mw' => $series_mw,
			'team_num' => 3,
	...
	function get_league_parameter_70()
	{
        return __get_league_parameter_70_71( 'm' );
	}

	function get_league_parameter_71()
	{
        return __get_league_parameter_70_71( 'w' );
	}

	function __get_tournament_parameter_70_71()
	{
		$param = array(
			'mw' => $series_mw,
			'team_num' => 8,
	...
	function get_tournament_parameter_70()
	{
		__get_tournament_parameter_70_71( 'm' );
	}

	function get_tournament_parameter_71()
	{
		__get_tournament_parameter_70_71( 'w' );
	}

	function __get_entry_data_70_71_list_for_PDF( $series, $series_mw, $kaisai_rev )
	{
		$c = new common();
...
*/
	...
	function get_entry_data_70_list_for_PDF()
	{
		return __get_entry_data_70_71_list_for_PDF( 16, 'm', 1 );
	}

	function get_entry_data_71_list_for_PDF()
	{
		return __get_entry_data_70_71_list_for_PDF( 17, 'w', 1 );
	}


	function get_entry_data_list_70_sql( $mw )
	{
	...
	function output_league_70_for_HTML_2( $league_list, $entry_list, $mv )
	{
//print_r($league_list);
		if( $mv == 'm' ){
			$mvstr = '男子';
...
//							$html .= $league_list[$league_data]['match'][$match_no_index-1]['hon2'].'-'.$league_list[$league_data]['match'][$match_no_index-1]['win2'];
	...
	function __output_league_70_71_for_HTML( $navi_info, $league_param, $league_list, $entry_list, $mw )
	{
//echo $path;
//print_r($league_list);
		if( $mw == 'm' ){
...
//							$html .= $league_list[$league_data]['match'][$match_no_index-1]['hon2'].'-'.$league_list[$league_data]['match'][$match_no_index-1]['win2'];
	...
	function output_league_70_for_HTML( $navi_info, $league_param, $league_list, $entry_list )
	{
...
//print_r($league_list);
		...
		//__output_league_70_71_for_HTML( $path, get_league_parameter_70(), $league_list, $entry_list, 'm' );
	}

	function output_league_71_for_HTML( $navi_info, $league_param, $league_list, $entry_list )
	{
        $objPage = new form_page();
        $objLeague = new form_page_dantai_league( $objPage );
        $objLeague->output_league_for_HTML( $navi_info, $league_param, $league_list, $entry_list, 'w', false );
		//__output_league_70_71_for_HTML( $path, get_league_parameter_71(), $league_list, $entry_list, 'w' );
	}
	...
	function __output_league_70_71_for_Excel_one( $objPage, $sheet, $col, $row, $league_param, $league_list, $entry_list, $mv )
	{
//print_r($league_list);
		if( $mv == 'm' ){
			$mvstr = '男子';
	...
	function __output_league_70_71_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, $mw )
	{
//print_r($league_list);
		if( $mw == 'm' ){
			$mwstr = '男子';
...
//echo "1:",$mg_range1,':',$mg_range2,'<br />';
	...
	function output_league_70_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_70_71_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, 'm' );
	}

	function output_league_71_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_70_71_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, 'w' );
	}

	//---------------------------------------------------------------
	//
	//---------------------------------------------------------------
	function __output_league_match_for_HTML2_70_71( $objPage, $path, $league_list, $entry_list, $mv )
	{
		if( $mv == 'm' ){
			$mvstr = '男子';
		} else {
	...
	function output_league_match_for_HTML2_70( $navi_info, $league_param, $league_list, $entry_list )
	{
		//__output_league_match_for_HTML2_70_71( $objPage, $path, $league_list, $entry_list, 'm' );
        $objPage = new form_page();
        $objLeague = new form_page_dantai_league( $objPage );
        $objLeague->output_league_match_for_HTML2( $navi_info, $league_param, $league_list, $entry_list, 'm', true );
	}

	function output_league_match_for_HTML2_71( $navi_info, $league_param, $league_list, $entry_list )
	{
		//__output_league_match_for_HTML2_70_71( $objPage, $path, $league_list, $entry_list, 'w' );
        $objPage = new form_page();
        $objLeague = new form_page_dantai_league( $objPage );
	...
	function __output_league_match_for_excel_70_71( $objPage, $series_info, $league_param, $league_list, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//exit;
	...
	function output_league_match_for_excel_70( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_match_for_excel_70_71( $objPage, $series_info, $league_param, $league_list, $entry_list, 'm' );
	}

	function output_league_match_for_excel_71( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_match_for_excel_70_71( $objPage, $series_info, $league_param, $league_list, $entry_list, 'w' );
	}
	...
	function __output_tournament_match_70_71_for_excel( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
	...
	function output_tournament_match_70_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_match_70_71_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'm' );
	}

	function output_tournament_match_71_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_match_70_71_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'w' );
	}
	...
	function __output_tournament_70_71_for_excel( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//exit;
	...
	function __output_tournament_70_71_for_excel2( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw, $series_m, $series_w )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//exit;
        ...
        $excel = $reader->load(dirname(__FILE__).'/allnaganoJuniorDantaiTournament.xlsx');
        $excel->setActiveSheetIndex( 0 );        //何番目のシートに有効にするか
        $sheet = $excel->getActiveSheet();    //有効になっているシートを取得
        $sheet->getDefaultStyle()->getFont()->setName('ＭＳ Ｐゴシック');
        $sheet->getDefaultStyle()->getFont()->setSize(9);
        ...
        if( $tournament_data['match'][0]['winner'] == 1 ){
            $sheet->setCellValue( 'A63', $tournament_data['match'][0]['team1_name'] );
        } else if( $tournament_data['match'][0]['winner'] == 2 ){
            $sheet->setCellValue( 'A63', $tournament_data['match'][0]['team2_name'] );
        }
        if( $tournament_data['match'][0]['winner'] == 1 ){
	...
	function output_tournament_chart_70_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, $mw )
	{
		return __output_tournament_70_71_for_excel2( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'm', 70, 71 );
	}

	function output_tournament_chart_71_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, $mw )
	{
		return __output_tournament_70_71_for_excel2( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'w', 70, 71 );
	}
	...
	function __output_tournament_chart_70_71_for_excel( $objPage, $series_info, $tournament_list, $league_data, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
	...
	function output_tournament_chart_70_for_excel_( $objPage, $series_info, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_chart_70_71_for_excel( $objPage, $series_info, $tournament_data, $league_data, $entry_list, 'm' );
	}

	function output_tournament_chart_71_for_excel_( $objPage, $series_info, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_chart_70_71_for_excel( $objPage, $series_info, $tournament_data, $league_data, $entry_list, 'w' );
	}
	...
	function __output_tournament_match_for_HTML2_70_71( $objPage, $series_info, $tournament_list, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
        ...
        $tindex = 1;
		$objPage = new form_page();
		$html = $header . '    <h2>' . $mwstr . '団体トーナメント結果</h2>'."\n";

        $mno = 1;
   		for( $i1 = 1; $i1 <= 32; $i1++ ){
            if( $tournament_list[0]['match'][$i1+30]['place'] != 'no_match' ){
  	    		$html .= '<h3>団体トーナメント&nbsp;一回戦&nbsp;第'.$mno.'試合</h3>';
    	    	$html .= $objPage->output_one_match_for_HTML2( $series_info, $tournament_list[0]['match'][$i1+30], $entry_list, $mw );
                $mno++;
		    }
        }
		for( $i1 = 1; $i1 <= 16; $i1++ ){
			$html .= '<h3>団体トーナメント&nbsp;二回戦&nbsp;第'.$i1.'試合</h3>';
			$html .= $objPage->output_one_match_for_HTML2( $series_info, $tournament_list[0]['match'][$i1+14], $entry_list, $mw );
		}
		for( $i1 = 1; $i1 <= 8; $i1++ ){
			$html .= '<h3>団体トーナメント&nbsp;三回戦&nbsp;第'.$i1.'試合</h3>';
			$html .= $objPage->output_one_match_for_HTML2( $series_info, $tournament_list[0]['match'][$i1+6], $entry_list, $mw );
		}
		for( $i1 = 1; $i1 <= 4; $i1++ ){
			$html .= '<h3>団体トーナメント&nbsp;準々決勝&nbsp;第'.$i1.'試合</h3>';
			$html .= $objPage->output_one_match_for_HTML2( $series_info, $tournament_list[0]['match'][$i1+2], $entry_list, $mw );
		}
		for( $i1 = 1; $i1 <= 2; $i1++ ){
			$html .= '<h3>団体トーナメント&nbsp;準決勝&nbsp;第'.$i1.'試合</h3>';
			$html .= $objPage->output_one_match_for_HTML2( $series_info, $tournament_list[0]['match'][$i1], $entry_list, $mw );
		}
		$html .= '<h3>団体トーナメント&nbsp;決勝</h3>';
	...
	function output_tournament_match_for_HTML2_70( $series_info, $tournament_list, $entry_list )
	{
        $objPage = new form_page();
		__output_tournament_match_for_HTML2_70_71( $objPage, $series_info, $tournament_list, $entry_list, 'm' );
	}

	function output_tournament_match_for_HTML2_71( $series_info, $tournament_list, $entry_list )
	{
        $objPage = new form_page();
		__output_tournament_match_for_HTML2_70_71( $objPage, $series_info, $tournament_list, $entry_list, 'w' );
	}
	...
	function output_tournament_71_for_PDF( $tournament_data, $entry_list, $mv )
	{
	}

	function output_tournament_70_for_PDF( $tournament_data, $entry_list, $mv )
	{
//print_r($tournament_data);
		if( $mv == 'm' ){
			$mvstr = '男子';
...
exit;
	...
	function __output_tournament_70_71_for_HTML( $objPage, $series_info, $tournament_list, $entry_list, $mw )
	{
//print_r($tournament_data);
//print_r($entry_list);
		if( $mw == 'm' ){
...
//exit;
	...
	function output_tournament_70_for_HTML( $series_info, $tournament_data, $entry_list )
	{
        $objPage = new form_page();
		__output_tournament_70_71_for_HTML( $objPage, $series_info, $tournament_data, $entry_list, 'm' );
	}

	function output_tournament_71_for_HTML( $series_info, $tournament_data, $entry_list )
	{
        $objPage = new form_page();
		__output_tournament_70_71_for_HTML( $objPage, $series_info, $tournament_data, $entry_list, 'w' );
	}
	...
	function __output_entry_data_list_all_1_excel70_71( $sheet, $series, $mv, $start_pos, $series_name, $kaisai_rev )
	{
		$c = new common();
	...
	function output_entry_data_list_all_1_excel70( $sheet )
	{
		__output_entry_data_list_all_1_excel70_71( $sheet, 16, 'm', 4, '団体戦男子', 1 );
		__output_entry_data_list_all_1_excel16_71( $sheet, 17, 'w', 4+48, '団体戦女子', 1 );
		return false;
	}

	function output_entry_data_list_all_1_excel71( $sheet )
	{
		__output_entry_data_list_all_1_excel70_71( $sheet, 16, 'm', 4, '団体戦男子', 1 );
		__output_entry_data_list_all_1_excel70_71( $sheet, 17, 'w', 4+48, '団体戦女子', 1 );
		return false;
	}

	//--------------------------------------------------------------

	function __get_entry_data_list2_70_71( $series )
	{
		//$objPage = new form_page();
		$c = new common();
		$preftbl = $c->get_pref_array();
		...
		$sql = 'select * from `entry_info`'
			...
			.' order by `disp_order` asc';
		$list = db_query_list( $dbs, $sql );
		$sql = 'select * from `entry_field` where `field` in (\'school_pref\',\'join\') and `year`='.$_SESSION['auth']['year'];
		$field_list = db_query_list( $dbs, $sql );
		...
		foreach( $list as $lv ){
			...
			foreach( $field_list as $fv ){
				$info = intval( $fv['info'] );
				if( $id == $info ){
					if( $fv['field'] == 'school_pref' ){
						$lv['pref_name'] = $c->get_pref_name( $preftbl, $fv['data'] );
						$lv['school_name'] = $lv['pref_name'];
						$lv['school_name_ryaku'] = $lv['pref_name'];
						//$objPage->update_entry_field_data( $id, 'school_name', $lv['pref_name'], $dbs );
	...
	function get_entry_data_list2_70()
	{
		return __get_entry_data_list2_70_71( 70 );
	}

	function get_entry_data_list2_71()
	{
		return __get_entry_data_list2_70_71( 71 );
	}

	//--------------------------------------------------------------

	function __get_entry_data_for_draw_csv_70_71( $list, $series )
	{
		$c = new common();
	...
	function get_entry_data_for_draw_csv_70( $list )
	{
		return array(
			'csv' => __get_entry_data_for_draw_csv_70_71( $list, 16 ),
			'file' => 'dantai_m.csv'
		);
	}

	function get_entry_data_for_draw_csv_71( $list )
	{
		return array(
			'csv' => __get_entry_data_for_draw_csv_70_71( $list, 17 ),
			'file' => 'dantai_w.csv'
		);
	}
	...
	function __output_realtime_html_for_one_board_70_71( $place, $place_match_no )
	{
		global $navi_info;
...
*/
	...
	function output_realtime_html_for_one_board_70( $place, $place_match_no )
	{
		//return __output_realtime_html_for_one_board_70_71( $place, $place_match_no );
		return '';
	}

	function output_realtime_html_for_one_board_71( $place, $place_match_no )
	{
		//return __output_realtime_html_for_one_board_70_71( $place, $place_match_no );
		return '';
	}
