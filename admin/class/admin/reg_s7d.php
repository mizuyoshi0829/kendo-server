<?php
	...
	function __get_league_parameter_32_33( $series_mw )
	{
		$param = array(
			'mw' => $series_mw,
			'team_num' => 3,
	...
	function get_league_parameter_32()
	{
        return __get_league_parameter_32_33( 'm' );
	}

	function get_league_parameter_33()
	{
        return __get_league_parameter_32_33( 'w' );
	}

	function __get_tournament_parameter_32_33()
	{
		$param = array(
			'mw' => $series_mw,
			'team_num' => 8,
	...
	function get_tournament_parameter_32()
	{
		__get_tournament_parameter_32_33( 'm' );
	}

	function get_tournament_parameter_33()
	{
		__get_tournament_parameter_32_33( 'w' );
	}

	function __get_entry_data_32_33_list_for_PDF( $series, $series_mw, $kaisai_rev )
	{
		$c = new common();
...
*/
	...
	function get_entry_data_32_list_for_PDF()
	{
		return __get_entry_data_32_33_list_for_PDF( 16, 'm', 1 );
	}

	function get_entry_data_33_list_for_PDF()
	{
		return __get_entry_data_32_33_list_for_PDF( 17, 'w', 1 );
	}


	function get_entry_data_list_32_sql( $mw )
	{
	...
	function output_league_32_for_HTML_2( $league_list, $entry_list, $mv )
	{
//print_r($league_list);
		if( $mv == 'm' ){
			$mvstr = '男子';
...
//							$html .= $league_list[$league_data]['match'][$match_no_index-1]['hon2'].'-'.$league_list[$league_data]['match'][$match_no_index-1]['win2'];
	...
	function __output_league_32_33_for_HTML( $navi_info, $league_param, $league_list, $entry_list, $mw )
	{
//echo $path;
//print_r($league_list);
		if( $mw == 'm' ){
...
//							$html .= $league_list[$league_data]['match'][$match_no_index-1]['hon2'].'-'.$league_list[$league_data]['match'][$match_no_index-1]['win2'];
	...
	function output_league_32_for_HTML( $navi_info, $league_param, $league_list, $entry_list )
	{
...
//print_r($league_list);
		...
		//__output_league_32_33_for_HTML( $path, get_league_parameter_32(), $league_list, $entry_list, 'm' );
	}

	function output_league_33_for_HTML( $navi_info, $league_param, $league_list, $entry_list )
	{
        $objPage = new form_page();
        $objLeague = new form_page_dantai_league( $objPage );
		...
		//__output_league_32_33_for_HTML( $path, get_league_parameter_33(), $league_list, $entry_list, 'w' );
	}
	...
	function __output_league_32_33_for_Excel_one( $objPage, $sheet, $col, $row, $league_param, $league_list, $entry_list, $mv )
	{
//print_r($league_list);
		if( $mv == 'm' ){
			$mvstr = '男子';
	...
	function __output_league_32_33_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, $mw )
	{
//print_r($league_list);
		if( $mw == 'm' ){
			$mwstr = '男子';
		...
		$file_name = $series_info['reg_year'] . '北信越大会団体リーグ成績表_' . $mwstr . '.xls';
...
//echo "1:",$mg_range1,':',$mg_range2,'<br />';
	...
	function output_league_32_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_32_33_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, 'm' );
	}

	function output_league_33_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_32_33_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, 'w' );
	}

	//---------------------------------------------------------------
	//
	//---------------------------------------------------------------
	function __output_league_match_for_HTML2_32_33( $objPage, $path, $league_list, $entry_list, $mv )
	{
		if( $mv == 'm' ){
			$mvstr = '男子';
		} else {
	...
	function output_league_match_for_HTML2_32( $navi_info, $league_param, $league_list, $entry_list )
	{
		//__output_league_match_for_HTML2_32_33( $objPage, $path, $league_list, $entry_list, 'm' );
        $objPage = new form_page();
	...
	function output_league_match_for_HTML2_33( $navi_info, $league_param, $league_list, $entry_list )
	{
		//__output_league_match_for_HTML2_32_33( $objPage, $path, $league_list, $entry_list, 'w' );
        $objPage = new form_page();
	...
	function __output_league_match_for_excel_32_33( $objPage, $series_info, $league_param, $league_list, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//print_r($league_list);
//exit;
		$file_name = $series_info['reg_year'] . '北信越大会団体リーグ対戦詳細結果_' . $mwstr . '.xls';
		$file_path = $series_info['output_path'] . '/' . $file_name;
		...
		for( $league_data = 0; $league_data < count( $league_list ); $league_data++ ){
			...
			for( $match = 0; $match < $league_list[$league_data]['match_num']; $match++ ){
				...
				$row += 2;
				$objPage->output_one_match_for_excel(
					$sheet, $col, $row, $series_info,
					$league_list[$league_data]['match'][$league_list[$league_data]['place_match_info'][$match]],
					$entry_list, $mw, 46, 46
				);
				$row += 4;
			}
	...
	function output_league_match_for_excel_32( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_match_for_excel_32_33( $objPage, $series_info, $league_param, $league_list, $entry_list, 'm' );
	}

	function output_league_match_for_excel_33( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_match_for_excel_32_33( $objPage, $series_info, $league_param, $league_list, $entry_list, 'w' );
	}
	...
	function __output_tournament_match_32_33_for_excel( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
		...
		require_once dirname(dirname(dirname(__FILE__))).'/phpExcel/Classes/PHPExcel.php';
		require_once dirname(dirname(dirname(__FILE__))).'/phpExcel/Classes/PHPExcel/IOFactory.php';
		$file_name = $series_info['reg_year'] . '北信越大会団体トーナメント詳細結果_' . $mwstr . '.xls';
		$file_path = $series_info['output_path'] . '/' . $file_name;
...
//        $objPage->output_one_match_for_excel( $sheet, $col, $row, $series_info, $tournament_data['match'][7], $entry_list, $mv, 46, 46 );
	...
	function output_tournament_match_32_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_match_32_33_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'm' );
	}

	function output_tournament_match_33_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_match_32_33_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'w' );
	}
	...
	function __output_tournament_32_33_for_excel( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//exit;
        $file_name = $series_info['reg_year'] . '北信越大会団体トーナメント表_' . $mwstr . '.xls';
        $file_path = $series_info['output_path'] . '/' . $file_name;
        $excel = new PHPExcel();
        $excel->setActiveSheetIndex( 0 );        //何番目のシートに有効にするか
        $sheet = $excel->getActiveSheet();    //有効になっているシートを取得
	...
	function __output_tournament_32_33_for_excel2( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw, $series_m, $series_w )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//print_r($tournament_data);
//exit;
        $file_name = $series_info['reg_year'] . '北信越大会団体トーナメント表.xlsx';
        $file_path = $series_info['output_path'] . '/' . $file_name;
        $excel = new PHPExcel();
...
*/
        ...
        if( $tournament_data['match'][1]['winner'] == 1 ){
            $sheet->setCellValue( 'Q29', $tournament_data['match'][1]['team2_name'] );
        } else if( $tournament_data['match'][1]['winner'] == 2 ){
            $sheet->setCellValue( 'Q29', $tournament_data['match'][1]['team1_name'] );
        }
        if( $tournament_data['match'][2]['winner'] == 1 ){
            $sheet->setCellValue( 'Y29', $tournament_data['match'][2]['team2_name'] );
        } else if( $tournament_data['match'][2]['winner'] == 2 ){
            $sheet->setCellValue( 'Y29', $tournament_data['match'][2]['team1_name'] );
        }
...
*/
        ...
        if( $tournament_data['match'][1]['winner'] == 1 ){
            $sheet->setCellValue( 'Q60', $tournament_data['match'][1]['team2_name'] );
        } else if( $tournament_data['match'][1]['winner'] == 2 ){
            $sheet->setCellValue( 'Q60', $tournament_data['match'][1]['team1_name'] );
        }
        if( $tournament_data['match'][2]['winner'] == 1 ){
            $sheet->setCellValue( 'Y60', $tournament_data['match'][2]['team2_name'] );
        } else if( $tournament_data['match'][2]['winner'] == 2 ){
            $sheet->setCellValue( 'Y60', $tournament_data['match'][2]['team1_name'] );
        }
	...
	function output_tournament_32_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_32_33_for_excel2( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'm', 32, 33 );
	}

	function output_tournament_33_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_32_33_for_excel2( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'w', 32, 33 );
	}
	...
	function __output_tournament_chart_32_33_for_excel( $objPage, $series_info, $tournament_list, $league_data, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
	...
	function output_tournament_chart_32_for_excel( $objPage, $series_info, $tournament_data, $league_data, $entry_list )
	{
		//return __output_tournament_chart_32_33_for_excel( $objPage, $series_info, $tournament_data, $league_data, $entry_list, 'm' );
		return __output_tournament_32_33_for_excel2( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'm', 32, 33 );
	}

	function output_tournament_chart_33_for_excel( $objPage, $series_info, $tournament_data, $league_data, $entry_list )
	{
		//return __output_tournament_chart_32_33_for_excel( $objPage, $series_info, $tournament_data, $league_data, $entry_list, 'w' );
		return __output_tournament_32_33_for_excel2( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'w', 32, 33 );
	}
	...
	function __output_tournament_match_for_HTML2_32_33( $series_info, $tournament_list, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
	...
	function output_tournament_match_for_HTML2_32( $series_info, $tournament_list, $entry_list )
	{
		__output_tournament_match_for_HTML2_32_33( $series_info, $tournament_list, $entry_list, 'm' );
	}

	function output_tournament_match_for_HTML2_33( $series_info, $tournament_list, $entry_list )
	{
		__output_tournament_match_for_HTML2_32_33( $series_info, $tournament_list, $entry_list, 'w' );
	}
	...
	function output_tournament_33_for_PDF( $tournament_data, $entry_list, $mv )
	{
	}

	function output_tournament_32_for_PDF( $tournament_data, $entry_list, $mv )
	{
//print_r($tournament_data);
		if( $mv == 'm' ){
			$mvstr = '男子';
...
exit;
	...
	function __output_tournament_32_33_for_HTML( $series_info, $tournament_list, $entry_list, $mv )
	{
//print_r($tournament_data);
//print_r($entry_list);
		if( $mv == 'm' ){
			...
			$table_name_name_width = 200;
			$table_name_pref_width = 10;
			$table_height = 11; //6;
...
//exit;
	...
	function output_tournament_32_for_HTML( $series_info, $tournament_data, $entry_list )
	{
		__output_tournament_32_33_for_HTML( $series_info, $tournament_data, $entry_list, 'm' );
	}

	function output_tournament_33_for_HTML( $series_info, $tournament_data, $entry_list )
	{
		__output_tournament_32_33_for_HTML( $series_info, $tournament_data, $entry_list, 'w' );
	}
	...
	function __output_entry_data_list_all_1_excel32_33( $sheet, $series, $mv, $start_pos, $series_name, $kaisai_rev )
	{
		$c = new common();
	...
	function output_entry_data_list_all_1_excel32( $sheet )
	{
		__output_entry_data_list_all_1_excel32_33( $sheet, 16, 'm', 4, '団体戦男子', 1 );
		__output_entry_data_list_all_1_excel16_33( $sheet, 17, 'w', 4+48, '団体戦女子', 1 );
		return false;
	}

	function output_entry_data_list_all_1_excel33( $sheet )
	{
		__output_entry_data_list_all_1_excel32_33( $sheet, 16, 'm', 4, '団体戦男子', 1 );
		__output_entry_data_list_all_1_excel32_33( $sheet, 17, 'w', 4+48, '団体戦女子', 1 );
		return false;
	}

	//--------------------------------------------------------------

	function __get_entry_data_list2_32_33( $series )
	{
		$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
		$sql = 'select * from `entry_info`'
			.' where `series`='.$series.' and `year`='.$_SESSION['auth']['year']
	...
	function get_entry_data_list2_32()
	{
		return __get_entry_data_list2_32_33( 32 );
	}

	function get_entry_data_list2_33()
	{
		return __get_entry_data_list2_32_33( 33 );
	}

	//--------------------------------------------------------------

	function __get_entry_data_for_draw_csv_32_33( $list, $series )
	{
		$c = new common();
	...
	function get_entry_data_for_draw_csv_32( $list )
	{
		return array(
			'csv' => __get_entry_data_for_draw_csv_32_33( $list, 16 ),
			'file' => 'dantai_m.csv'
		);
	}

	function get_entry_data_for_draw_csv_33( $list )
	{
		return array(
			'csv' => __get_entry_data_for_draw_csv_32_33( $list, 17 ),
			'file' => 'dantai_w.csv'
		);
	}
	...
	function __output_realtime_html_for_one_board_32_33( $place, $place_match_no )
	{
		global $navi_info;
...
*/
	...
	function output_realtime_html_for_one_board_32( $place, $place_match_no )
	{
		//return __output_realtime_html_for_one_board_32_33( $place, $place_match_no );
		return '';
	}

	function output_realtime_html_for_one_board_33( $place, $place_match_no )
	{
		//return __output_realtime_html_for_one_board_32_33( $place, $place_match_no );
		return '';
	}
