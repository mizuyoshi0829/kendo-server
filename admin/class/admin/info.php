...
*/
            if( $_POST['pass'] == __ADMIN_PASSWORD_11__ ){
                ...
                $_SESSION['auth']['result_path_prefix'] = $sl[0]['result_path_prefix'];
...
//print_r($serieslist);
        ...
        $this->smarty_assign['mw'] = $category_mw;
        $this->header = 'admin' . DIRECTORY_SEPARATOR . 'header.html';
        $this->footer = 'admin' . DIRECTORY_SEPARATOR . 'footer.html';
        if( $mode == 'new' ){
            ...
            if( $id > 0 ){
                ...
                /*
                if( $series == 58 || $series == 59 ){
                    $this->get_entry_post_photo_crop_data(
                        get_field_string( $_SESSION['p'],'photo'),
                        get_field_string( $_SESSION['p'],'photo_effect'),
                        $force
                    );
                }
                if( $series == 60 || $series == 61 ){
                    $this->get_entry_post_photo_crop_data(
                        get_field_string( $_SESSION['p'],'player1_photo'),
                        get_field_string( $_SESSION['p'],'player1_photo_effect'),
                        $force
                    );
                    $this->get_entry_post_photo_crop_data(
                        get_field_string( $_SESSION['p'],'player2_photo'),
                        get_field_string( $_SESSION['p'],'player2_photo_effect'),
                        $force
                    );
                }
                */
...
//return;
            ...
            $file_name = 'list' . '_' .date('YmdHis') . sprintf("%04d",microtime()*1000) . '.xlsx';
        ...
        if( $mode == 'outputall' ){
            ...
            $file_name = 'listall' . '_' . $category_name . '.' . $ftime . '.xlsx';
            $file_path = $serieslist['output_path'] . '/' . 'listall' . '.' . $ftime . '.xlsx';
            $temp_path = dirname(dirname(dirname(__FILE__))).'/templates/excel/listallbase' . $series . '.xls';
...
*/
        ...
        if( $mode == 'output_catalog_pdf' ){
            $func = 'get_entry_data_' . $series . '_for_catalog_PDF';
            $fname = $func($this);
            $file_path = dirname(dirname(dirname(__FILE__))) . '/output/' . $fname;
            header( 'Content-Type: application/octet-stream' );
        ...
        if( $mode == 'output_catalog_pdf2' ){
            $func = 'get_entry_data_' . $series . '_for_catalog_PDF2';
            $fname = $func($this);
            $file_path = dirname(dirname(dirname(__FILE__))) . '/output/' . $fname;
            header( 'Content-Type: application/octet-stream' );
            header( 'Content-Disposition: attachment; filename="'.$fname.'"');
        ...
        if( $mode == 'output_ID_pdf' ){
            $func = 'get_entry_data_' . $series . '_for_ID_PDF';
            $fname = $func($this);
            $file_path = dirname(dirname(dirname(__FILE__))) . '/output/' . $fname;
            header( 'Content-Type: application/octet-stream' );
            header( 'Content-Disposition: attachment; filename="'.$fname.'"');
...
//exit;
            ...
            $fp = fopen( dirname(dirname(dirname(__FILE__))).'/output/output.'.$fdate.'.html', 'w' );
