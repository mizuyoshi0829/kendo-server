<?php
	...
	function __get_league_parameter_74_75( $series_mw )
	{
		$param = array(
			'mw' => $series_mw,
			'team_num' => 3,
	...
	function get_league_parameter_74()
	{
        return __get_league_parameter_74_75( 'm' );
	}

	function get_league_parameter_75()
	{
        return __get_league_parameter_74_75( 'w' );
	}

	function __get_tournament_parameter_74_75( $series_mw )
	{
		$param = array(
			'mw' => $series_mw,
			'team_num' => 8,
	...
	function get_tournament_parameter_74()
	{
		__get_tournament_parameter_74_75( 'm' );
	}

	function get_tournament_parameter_75()
	{
		__get_tournament_parameter_74_75( 'w' );
	}

	function __get_entry_data_74_75_list_for_PDF( $series, $series_mw, $kaisai_rev )
	{
		$c = new common();
...
*/
	...
	function get_entry_data_74_list_for_PDF()
	{
		return __get_entry_data_74_75_list_for_PDF( 16, 'm', 1 );
	}

	function get_entry_data_75_list_for_PDF()
	{
		return __get_entry_data_74_75_list_for_PDF( 17, 'w', 1 );
	}


	function get_entry_data_list_74_sql( $mw )
	{
	...
	function output_league_74_for_HTML_2( $league_list, $entry_list, $mv )
	{
//print_r($league_list);
		if( $mv == 'm' ){
			$mvstr = '男子';
...
//							$html .= $league_list[$league_data]['match'][$match_no_index-1]['hon2'].'-'.$league_list[$league_data]['match'][$match_no_index-1]['win2'];
	...
	function __output_league_74_75_for_HTML( $navi_info, $league_param, $league_list, $entry_list, $mw )
	{
//echo $path;
//print_r($league_list);
		if( $mw == 'm' ){
...
//							$html .= $league_list[$league_data]['match'][$match_no_index-1]['hon2'].'-'.$league_list[$league_data]['match'][$match_no_index-1]['win2'];
	...
	function output_league_74_for_HTML( $navi_info, $league_param, $league_list, $entry_list )
	{
...
//print_r($league_list);
		...
		//__output_league_74_75_for_HTML( $path, get_league_parameter_74(), $league_list, $entry_list, 'm' );
	}

	function output_league_75_for_HTML( $navi_info, $league_param, $league_list, $entry_list )
	{
        $objPage = new form_page();
        $objLeague = new form_page_dantai_league( $objPage );
        $objLeague->output_league_for_HTML( $navi_info, $league_param, $league_list, $entry_list, 'w', false, false );
		//__output_league_74_75_for_HTML( $path, get_league_parameter_75(), $league_list, $entry_list, 'w' );
	}
	...
	function __output_league_74_75_for_Excel_one( $objPage, $sheet, $col, $row, $league_param, $league_list, $entry_list, $mv )
	{
//print_r($league_list);
		if( $mv == 'm' ){
			$mvstr = '男子';
	...
	function __output_league_74_75_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, $mw )
	{
//print_r($league_list);
		if( $mw == 'm' ){
			$mwstr = '男子';
...
//echo "1:",$mg_range1,':',$mg_range2,'<br />';
	...
	function output_league_74_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_74_75_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, 'm' );
	}

	function output_league_75_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_74_75_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, 'w' );
	}

	//---------------------------------------------------------------
	//
	//---------------------------------------------------------------
	function __output_league_match_for_HTML2_74_75( $objPage, $path, $league_list, $entry_list, $mv )
	{
		if( $mv == 'm' ){
			$mvstr = '男子';
		} else {
	...
	function output_league_match_for_HTML2_74( $navi_info, $league_param, $league_list, $entry_list )
	{
		//__output_league_match_for_HTML2_74_75( $objPage, $path, $league_list, $entry_list, 'm' );
        $objPage = new form_page();
        $objLeague = new form_page_dantai_league( $objPage );
        $objLeague->output_league_match_for_HTML2( $navi_info, $league_param, $league_list, $entry_list, 'm', true );
	}

	function output_league_match_for_HTML2_75( $navi_info, $league_param, $league_list, $entry_list )
	{
		//__output_league_match_for_HTML2_74_75( $objPage, $path, $league_list, $entry_list, 'w' );
        $objPage = new form_page();
        $objLeague = new form_page_dantai_league( $objPage );
	...
	function __output_league_match_for_excel_74_75( $objPage, $series_info, $league_param, $league_list, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//exit;
	...
	function output_league_match_for_excel_74( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_match_for_excel_74_75( $objPage, $series_info, $league_param, $league_list, $entry_list, 'm' );
	}

	function output_league_match_for_excel_75( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_match_for_excel_74_75( $objPage, $series_info, $league_param, $league_list, $entry_list, 'w' );
	}
	...
	function __output_tournament_match_74_75_for_excel( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
			$match_num = 8;
		...
		$file_name = $series_info['name'] . '_団体トーナメント結果_' . $mwstr . '.xlsx';
		$file_path = $series_info['output_path'] . '/' . $file_name;

		$reader = PHPExcel_IOFactory::createReader('Excel5');
		$excel = $reader->load(dirname(dirname(dirname(__FILE__))).'/templates/excel/dantaiTournamentMatchResultsBase_74_75.xls');
		$excel->setActiveSheetIndex( 0 );		//何番目のシートに有効にするか
		$sheet = $excel->getActiveSheet();	//有効になっているシートを取得
		...
		$match1 = 1;
		for( $match = 1; $match < $match_num; $match++ ){
            if( $match_num > 4 && $match <= $match_num-4 ){
				if( $tournament_data['match'][$match+2]['place'] === 'no_match' ){
					continue;
				}
  				$sheet->setCellValueByColumnAndRow( 0, $row, '一回戦 第'.($match1).'試合' );
				$match1++;
			} else if( $match <= $match_num-2 ){
                $sheet->setCellValueByColumnAndRow( 0, $row, '準決勝 第'.($match+4-$match_num).'試合' );
            } else if( $match == $match_num-1 ){
	...
	function output_tournament_match_74_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_match_74_75_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'm' );
	}

	function output_tournament_match_75_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_match_74_75_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'w' );
	}
	...
	function __output_tournament_74_75_for_excel( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//exit;
	...
	function __output_tournament_74_75_for_excel2( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw, $series_m, $series_w )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//exit;
        ...
        $excel = $reader->load(dirname(dirname(dirname(__FILE__))).'/templates/excel/dantaiTournamentChartBase_74_75.xlsx');
        $excel->setActiveSheetIndex( 0 );        //何番目のシートに有効にするか
        $sheet = $excel->getActiveSheet();    //有効になっているシートを取得
        $sheet->getDefaultStyle()->getFont()->setName('ＭＳ Ｐゴシック');
        $sheet->getDefaultStyle()->getFont()->setSize(9);
        ...
        $sheet->getStyle( $colstr.'6:'.$colstr.'8' )->applyFromArray( $styleArrayVR );
		$colstr1 = PHPExcel_Cell::stringFromColumnIndex( $posTbl1[0]-2 );
		$colstr2 = PHPExcel_Cell::stringFromColumnIndex( $posTbl1[0]+3 );
        $sheet->mergeCells( $colstr1.'4:'.$colstr2.'4' );
		if( $tournament_data['match'][0]['winner'] == 1 ){
            $sheet->setCellValue( $colstr1.'4', $tournament_data['match'][0]['team1_name'] );
        } else if( $tournament_data['match'][0]['winner'] == 2 ){
            $sheet->setCellValue( $colstr1.'4', $tournament_data['match'][0]['team2_name'] );
        }
...
*/
        if( $tournament_data['match'][0]['winner'] == 1 ){
            $sheet->setCellValue( 'A26', $tournament_data['match'][0]['team1_name'] );
        } else if( $tournament_data['match'][0]['winner'] == 2 ){
            $sheet->setCellValue( 'A26', $tournament_data['match'][0]['team2_name'] );
        }
        if( $tournament_data['match'][0]['winner'] == 1 ){
            $sheet->setCellValue( 'K26', $tournament_data['match'][0]['team2_name'] );
        } else if( $tournament_data['match'][0]['winner'] == 2 ){
            $sheet->setCellValue( 'K26', $tournament_data['match'][0]['team1_name'] );
        }
        if( $tournament_data['match'][1]['winner'] == 1 ){
            $sheet->setCellValue( 'U26', $tournament_data['match'][1]['team2_name'] );
        } else if( $tournament_data['match'][1]['winner'] == 2 ){
            $sheet->setCellValue( 'U26', $tournament_data['match'][1]['team1_name'] );
        }
        if( $tournament_data['match'][2]['winner'] == 1 ){
            $sheet->setCellValue( 'AE26', $tournament_data['match'][2]['team2_name'] );
        } else if( $tournament_data['match'][2]['winner'] == 2 ){
            $sheet->setCellValue( 'AE26', $tournament_data['match'][2]['team1_name'] );
        }
        ...
        // 決勝
        $sheet->getStyle( 'P34:P36' )->applyFromArray( $styleArrayVR );
        if( $tournament_data['match'][0]['winner'] == 1 ){
            $sheet->setCellValue( 'N32', $tournament_data['match'][0]['team1_name'] );
        } else if( $tournament_data['match'][0]['winner'] == 2 ){
            $sheet->setCellValue( 'N32', $tournament_data['match'][0]['team2_name'] );
        }
...
*/
        if( $tournament_data['match'][0]['winner'] == 1 ){
            $sheet->setCellValue( 'A54', $tournament_data['match'][0]['team1_name'] );
        } else if( $tournament_data['match'][0]['winner'] == 2 ){
            $sheet->setCellValue( 'A54', $tournament_data['match'][0]['team2_name'] );
        }
        if( $tournament_data['match'][0]['winner'] == 1 ){
            $sheet->setCellValue( 'K54', $tournament_data['match'][0]['team2_name'] );
        } else if( $tournament_data['match'][0]['winner'] == 2 ){
            $sheet->setCellValue( 'K54', $tournament_data['match'][0]['team1_name'] );
        }
        if( $tournament_data['match'][1]['winner'] == 1 ){
            $sheet->setCellValue( 'U54', $tournament_data['match'][1]['team2_name'] );
        } else if( $tournament_data['match'][1]['winner'] == 2 ){
            $sheet->setCellValue( 'U54', $tournament_data['match'][1]['team1_name'] );
        }
        if( $tournament_data['match'][2]['winner'] == 1 ){
            $sheet->setCellValue( 'AE54', $tournament_data['match'][2]['team2_name'] );
        } else if( $tournament_data['match'][2]['winner'] == 2 ){
            $sheet->setCellValue( 'AE54', $tournament_data['match'][2]['team1_name'] );
        }
	...
	function output_tournament_chart_74_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, $mw )
	{
		return __output_tournament_74_75_for_excel2( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'm', 74, 75 );
	}

	function output_tournament_chart_75_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, $mw )
	{
		return __output_tournament_74_75_for_excel2( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'w', 74, 75 );
	}
	...
	function __output_tournament_match_for_HTML2_74_75( $series_info, $tournament_list, $entry_list, $mw )
	{
        $objPage = new form_page();
		if( $mw == 'm' ){
			$mwstr = '男子';
        ...
        $mno = 1;
 		for( $i1 = 3; $i1 <= 6; $i1++ ){
            if( $tournament_list[0]['match'][$i1]['place'] != 'no_match' ){
...
*/
	...
	function output_tournament_match_for_HTML2_74( $series_info, $tournament_list, $entry_list )
	{
		__output_tournament_match_for_HTML2_74_75( $series_info, $tournament_list, $entry_list, 'm' );
	}

	function output_tournament_match_for_HTML2_75( $series_info, $tournament_list, $entry_list )
	{
		__output_tournament_match_for_HTML2_74_75( $series_info, $tournament_list, $entry_list, 'w' );
	}
	...
	function output_tournament_75_for_PDF( $tournament_data, $entry_list, $mv )
	{
	}

	function output_tournament_74_for_PDF( $tournament_data, $entry_list, $mv )
	{
//print_r($tournament_data);
		if( $mv == 'm' ){
			$mvstr = '男子';
...
exit;
	...
	function __output_tournament_74_75_for_HTML( $series_info, $tournament_list, $entry_list, $mv )
	{
//print_r($tournament_data);
//print_r($entry_list);
        $objPage = new form_page();
...
//exit;
	...
	function output_tournament_74_for_HTML( $series_info, $tournament_data, $entry_list )
	{
		__output_tournament_74_75_for_HTML( $series_info, $tournament_data, $entry_list, 'm' );
	}

	function output_tournament_75_for_HTML( $series_info, $tournament_data, $entry_list )
	{
		__output_tournament_74_75_for_HTML( $series_info, $tournament_data, $entry_list, 'w' );
	}
	...
	function __output_entry_data_list_all_1_excel74_75( $sheet, $series, $mv, $start_pos, $series_name, $kaisai_rev )
	{
		$c = new common();
	...
	function output_entry_data_list_all_1_excel74( $sheet )
	{
		__output_entry_data_list_all_1_excel74_75( $sheet, 16, 'm', 4, '団体戦男子', 1 );
		__output_entry_data_list_all_1_excel16_75( $sheet, 17, 'w', 4+48, '団体戦女子', 1 );
		return false;
	}

	function output_entry_data_list_all_1_excel75( $sheet )
	{
		__output_entry_data_list_all_1_excel74_75( $sheet, 16, 'm', 4, '団体戦男子', 1 );
		__output_entry_data_list_all_1_excel74_75( $sheet, 17, 'w', 4+48, '団体戦女子', 1 );
		return false;
	}

	//--------------------------------------------------------------

	function __get_entry_data_list2_74_75( $series )
	{
		$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
		$sql = 'select * from `entry_info`'
			.' where `series`='.$series.' and `year`='.$_SESSION['auth']['year']
	...
	function get_entry_data_list2_74()
	{
		return __get_entry_data_list2_74_75( 74 );
	}

	function get_entry_data_list2_75()
	{
		return __get_entry_data_list2_74_75( 75 );
	}

	//--------------------------------------------------------------

	function __get_entry_data_for_draw_csv_74_75( $list, $series )
	{
		$c = new common();
	...
	function get_entry_data_for_draw_csv_74( $list )
	{
		return array(
			'csv' => __get_entry_data_for_draw_csv_74_75( $list, 16 ),
			'file' => 'dantai_m.csv'
		);
	}

	function get_entry_data_for_draw_csv_75( $list )
	{
		return array(
			'csv' => __get_entry_data_for_draw_csv_74_75( $list, 17 ),
			'file' => 'dantai_w.csv'
		);
	}
	...
	function __output_realtime_html_for_one_board_74_75( $place, $place_match_no )
	{
		global $navi_info;
...
*/
	...
	function output_realtime_html_for_one_board_74( $place, $place_match_no )
	{
		//return __output_realtime_html_for_one_board_74_75( $place, $place_match_no );
		return '';
	}

	function output_realtime_html_for_one_board_75( $place, $place_match_no )
	{
		//return __output_realtime_html_for_one_board_74_75( $place, $place_match_no );
		return '';
	}
