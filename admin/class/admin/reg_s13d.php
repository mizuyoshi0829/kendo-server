<?php
	...
	function __get_league_parameter_41_17( $series_mw )
	{
		$param = array(
			'mw' => $series_mw,
			'team_num' => 3,
	...
	function get_league_parameter_41()
	{
        return array(
			'mw' => 'm',
			'team_num' => 3,
	...
	function __get_entry_data_41_41_list_for_PDF( $series, $series_mw, $kaisai_rev )
	{
		$c = new common();
...
*/
	...
	function get_entry_data_41_list_for_PDF()
	{
		return __get_entry_data_41_41_list_for_PDF( 41, 'm', 1 );
	}

	function get_entry_data_list_41_sql( $mw )
	{
	...
	function output_league_41_for_HTML_2( $league_list, $entry_list, $mv )
	{
//print_r($league_list);
		if( $mv == 'm' ){
			$mvstr = '男子';
...
//							$html .= $league_list[$league_data]['match'][$match_no_index-1]['hon2'].'-'.$league_list[$league_data]['match'][$match_no_index-1]['win2'];
	...
	function __output_league_41_41_for_HTML( $navi_info, $league_param, $league_list, $entry_list, $mw )
	{
//echo $path;
//print_r($league_list);
		if( $mw == 'm' ){
...
//							$html .= $league_list[$league_data]['match'][$match_no_index-1]['hon2'].'-'.$league_list[$league_data]['match'][$match_no_index-1]['win2'];
	...
	function output_league_41_for_HTML( $navi_info, $league_param, $league_list, $entry_list )
	{
		//__output_league_41_41_for_HTML( $navi_info, $league_param, $league_list, $entry_list, 'm' );
        $objPage = new form_page();
        $objLeague = new form_page_dantai_league( $objPage );
        $objLeague->output_league_for_HTML( $navi_info, $league_param, $league_list, $entry_list, 'm', false, false, 'index', false );
	...
	function __output_league_41_41_for_Excel_one( $objPage, $sheet, $col, $row, $league_param, $league_list, $entry_list, $mv )
	{
//print_r($league_list);
		if( $mv == 'm' ){
			$mvstr = '男子';
	...
	function __output_league_41_41_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, $mw )
	{
//print_r($league_list);
		if( $mw == 'm' ){
			$mwstr = '男子';
		...
		$file_name = 'nenrin2024LeagueResults.' . $ftime . '.xlsx';
		$file_path = $series_info['output_path'] . '/' . $file_name;

        $excel = new PHPExcel();
        $reader = PHPExcel_IOFactory::createReader('Excel2007');
		$excel = $reader->load(dirname(dirname(dirname(__FILE__))).'/templates/excel/nenrin_league_2024.xlsx');
		...
		for( $league_data = 0; $league_data < count( $league_list ); $league_data++ ){
			...
			for( $dantai_index_row = 0; $dantai_index_row < $team_num; $dantai_index_row++ ){
				for( $dantai_index_col = 0; $dantai_index_col < $team_num; $dantai_index_col++ ){
					$match_no_index = $match_tbl[$dantai_index_row][$dantai_index_col];
					$match_team_index = $league_list[$league_data]['chart_team_tbl'][$dantai_index_row][$dantai_index_col];
					//$match_team_index = $league_param['chart_team_tbl'][$dantai_index_row][$dantai_index_col];
					if( $match_team_index == 1 ){
						if( $match_no_index > 0 && $league_list[$league_data]['match'][$match_no_index-1]['end_match'] == 5 ){
							$cord = $objPage->get_excel_coordinates( $col+$dantai_index_col*4+1, $row+$dantai_index_row*2+1 );
							if( $league_list[$league_data]['match'][$match_no_index-1]['winner'] == 1 ){
								$objDrawing = new PHPExcel_Worksheet_Drawing();
								$objDrawing->setName( $cord );
								$objDrawing->setDescription( $cord );
								$objDrawing->setPath(dirname(dirname(__FILE__)).'/cir2.png');
							...
							$sheet->setCellValueByColumnAndRow(
								$col+$dantai_index_col*4+2, $row+$dantai_index_row*2+1,
								$league_list[$league_data]['match'][$match_no_index-1]['hon1']
							);
							$sheet->setCellValueByColumnAndRow(
				...
				foreach( $entry_list as $ev ){
					if( $league_list[$league_data]['team'][$dantai_index_row]['team'] == $ev['id'] ){
						$sheet->setCellValueByColumnAndRow( $col, $row+$dantai_index_row*2+1, $ev['school_name'] );
					}
				}
				$colscore = $col + $league_list[$league_data]['team_num'] * 4 + 1;
				$sheet->setCellValueByColumnAndRow(
					$colscore, $row+$dantai_index_row*2+1,
					$league_list[$league_data]['team'][$dantai_index_row]['win_match']
				);
				$sheet->setCellValueByColumnAndRow(
					$colscore+4, $row+$dantai_index_row*2+1,
					$league_list[$league_data]['team'][$dantai_index_row]['lost_match']
				);
				$sheet->setCellValueByColumnAndRow(
					$colscore+8, $row+$dantai_index_row*2+1,
					$league_list[$league_data]['team'][$dantai_index_row]['win']
				);
				$sheet->setCellValueByColumnAndRow(
					$colscore+12, $row+$dantai_index_row*2+1,
					$league_list[$league_data]['team'][$dantai_index_row]['hon']
				);
				$sheet->setCellValueByColumnAndRow(
					$colscore+16, $row+$dantai_index_row*2+1,
					$league_list[$league_data]['team'][$dantai_index_row]['standing']
				);
                if( $league_list[$league_data]['extra_match_exists'] == 1 && $dantai_index_row == 1 ){
    				$sheet->setCellValueByColumnAndRow(
	    				$colscore+9, $row+$dantai_index_row*2+1, '代'
			    	);
                }
			}
			$row += $team_num * 2 + 2;
	...
	function output_league_41_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_41_41_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, 'm' );
	}

	//---------------------------------------------------------------
	//
	//---------------------------------------------------------------
	function __output_league_match_for_HTML2_41_41( $objPage, $navi_info, $league_list, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
		...
		$html .= $footer;
		//$file = $navi_info['result_path'] . '/dlm'.$mv.'.html';
	...
	function output_league_match_for_HTML2_41( $series_info, $league_param, $league_list, $entry_list )
	{
        $objPage = new form_page();
        //$objPage->output_league_match_for_HTML2( $series_info, $league_param, $league_list, $entry_list, 'm' );
        $objLeague = new form_page_dantai_league( $objPage );
		//__output_league_match_for_HTML2_20_21( $objPage, $series_info, $league_list, $entry_list, 'm' );
        $objLeague->output_league_match_for_HTML2( $series_info, $league_param, $league_list, $entry_list, 'm', true, true, 'index' );
	}
	...
	function __output_league_match_for_excel_41_41( $objPage, $series_info, $league_param, $league_list, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//print_r($league_list);
//exit;
		$file_name = 'H29nanshinnLeagueMatchResults_'.$mw.'.xls';
		$file_path = $series_info['output_path'] . '/' . $file_name;
	...
	function output_league_match_for_excel_41( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		//return __output_league_match_for_excel_41_41( $objPage, $series_info, $league_param, $league_list, $entry_list, 'm' );
        $objPage = new form_page();
		$data = [
		...
		$objPage->update_pdf_queue( $data, true );
		return '';
	}
	...
	function output_league_41_for_PDF( $series_info, $league_param, $league_list, $entry_list )
	{
        $objPage = new form_page();
		$data = [
			'navi_id' => $series_info['navi_id'],
		...
		//$objMatch->output_dantai_one_match_pdf2($series_info['navi_id'],1,4);
	}
	...
	function __output_tournament_match_41_41_for_excel( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
		...
		require_once dirname(dirname(dirname(__FILE__))).'/phpExcel/Classes/PHPExcel/IOFactory.php';
		$ftime = date('YmdHis') . sprintf("%04d",microtime()*1000);
		$file_name = 'nenrin2023preTournamentMatchResults.' . $ftime . '.xls';
		$file_path = $series_info['output_path'] . '/' . $file_name;
		...
		$excel->setActiveSheetIndex( 0 );		//何番目のシートに有効にするか
		$sheet = $excel->getActiveSheet();	//有効になっているシートを取得
		$sheet->setCellValueByColumnAndRow( 0, 2, '団体トーナメント結果' );
		...
		$extra_match_num = $tournament_data['extra_match_num'];
		for( $match = $match_num; $match >= 0; $match-- ){
            if( $match_num >= 16 && $match >= 16 ){
				if( $tournament_data['match'][$match-1]['place'] === 'no_match' ){
					continue;
				}
  				$sheet->setCellValueByColumnAndRow( 0, $row, '一回戦 第'.($match1).'試合' );
				$match1++;
			} else if( $match_num >= 8 && $match >= 8 ){
				if( $tournament_data['match'][$match-1]['place'] === 'no_match' ){
					continue;
				}
				if( $match_num >= 16 ){
					$sheet->setCellValueByColumnAndRow( 0, $row, '二回戦 第'.($match2).'試合' );
				} else {
					$sheet->setCellValueByColumnAndRow( 0, $row, '一回戦 第'.($match2).'試合' );
				}
				$match2++;
			} else if( $match >= 4 ){
				$sheet->setCellValueByColumnAndRow( 0, $row, '準々決勝 第'.(8-$match).'試合' );
			} else if( $match >= 2 ){
                $sheet->setCellValueByColumnAndRow( 0, $row, '準決勝 第'.(4-$match).'試合' );
            } else if( $match == 1 ){
                $sheet->setCellValueByColumnAndRow( 0, $row, '決勝' );
            } else if( $match == 0 ){
			    if( $extra_match_num == 0 ){ continue; }
        ...
        $row += 7;
		if( $extra_match_num > 0 ){
			$objPage->output_one_match_for_excel( $sheet, $col, $row, $series_info, $tournament_data['match'][$match_num], $entry_list, $mv, 46, 46 );
		}
	...
	function output_tournament_match_41_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list )
	{
		//return __output_tournament_match_41_41_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'm' );
        $objPage = new form_page();
		$data = [
			'navi_id' => $series_info['navi_id'],
			'place' => 0,
	...
	function __output_tournament_41_41_for_excel( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//exit;
        $file_name = 'H29nanshinnDantaiTounamentResults_' . $mw . '.xls';
        $file_path = $series_info['output_path'] . '/' . $file_name;
        $excel = new PHPExcel();
        $excel->setActiveSheetIndex( 0 );        //何番目のシートに有効にするか
        $sheet = $excel->getActiveSheet();    //有効になっているシートを取得
...
//exit;
        ...
        if( $tournament_data['match'][1]['winner'] == 1 ){
            $sheet->getStyle( $col1.$pos1_1.':'.$col1.($row-1) )->applyFromArray( $styleArrayVR2 );
        } else {
            $sheet->getStyle( $col1.$row.':'.$col1.($pos1_2-1) )->applyFromArray( $styleArrayVR2 );
        }
        if( $tournament_data['match'][2]['winner'] == 1 ){
            $sheet->getStyle( $col2.$pos2_1.':'.$col2.($row-1) )->applyFromArray( $styleArrayVR );
        } else {
            $sheet->getStyle( $col2.$row.':'.$col2.($pos2_2-1) )->applyFromArray( $styleArrayVR );
        }
        ...
        if( $tournament_data['match'][0]['winner'] == 1 ){
            ...
            //$sheet->getStyle( $col2.$row )->applyFromArray( $styleArrayV );
            //$sheet->getStyle( $col2.($row+1) )->applyFromArray( $styleArrayV );
            ...
            //$sheet->getStyle( $col1.$row )->applyFromArray( $styleArrayV2 );
            //$sheet->getStyle( $col1.($row+1) )->applyFromArray( $styleArrayV2 );
        ...
        if( $tournament_data['match'][$tournament_team_num-1]['winner'] == 1 ){
            ...
            $sheet->setCellValue( 'H'.($row+3), $tournament_data['match'][$tournament_team_num-1]['team1_name'] );
            ...
            $sheet->getStyle( 'F'.($row+4) )->applyFromArray( $styleArrayHR );
            $sheet->setCellValue( 'H'.($row+3), $tournament_data['match'][$tournament_team_num-1]['team2_name'] );
            $sheet->mergeCells( 'H'.($row+3).':'.'J'.($row+4) );
            $sheet->getStyle('H'.($row+3))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
            $sheet->getStyle('H'.($row+3))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
        } else {
        ...
        if( $tournament_data['match'][$tournament_team_num-1]['winner'] == 1 ){
            $sheet->setCellValueByColumnAndRow( 2, $row, $tournament_data['match'][$tournament_team_num-1]['team1_name'] );
        } else if( $tournament_data['match'][$tournament_team_num-1]['winner'] == 2 ){
            $sheet->setCellValueByColumnAndRow( 2, $row, $tournament_data['match'][$tournament_team_num-1]['team2_name'] );
        }
        $sheet->getStyle('A'.($row).':F'.$row)->getFont()->setSize(12);
        $sheet->getRowDimension($row)->setRowHeight(16);
        $row++;
	...
	function output_tournament_41_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list )
	{
		return __output_tournament_41_41_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'm' );
	}
	...
	function __output_tournament_chart_41_41_for_excel( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw )
    {
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//exit;
		$ftime = date('YmdHis') . sprintf("%04d",microtime()*1000);
        $file_name = 'nenrin_tournament_2024.' . $ftime . '.xlsx';
        $file_path = $series_info['output_path'] . '/' . $file_name;

		$excel = new PHPExcel();
        $reader = PHPExcel_IOFactory::createReader('Excel2007');
		$excel = $reader->load(dirname(dirname(dirname(__FILE__))).'/templates/excel/nenrin_tournament_2024.xlsx');
        $excel->setActiveSheetIndex( 0 );        //何番目のシートに有効にするか
        $sheet = $excel->getActiveSheet();    //有効になっているシートを取得
		$sheet->setTitle( 'トーナメント表' );
        ...
        for( $level = 0; $level < $match_level; $level++ ){
            ...
            for( $m = 0; $m < $match_num; $m++ ){
				...
				$sheet->setCellValueByColumnAndRow( $str_c1, $str_r, $winstr[1][0] );
				$sheet->setCellValueByColumnAndRow( $str_c1, $str_r+1, $winstr[1][1] );
				$sheet->setCellValueByColumnAndRow( $str_c1, $str_r+2, $winstr[1][2] );
				$sheet->setCellValueByColumnAndRow( $str_c2, $str_r, $winstr[2][0] );
				$sheet->setCellValueByColumnAndRow( $str_c2, $str_r+1, $winstr[2][1] );
				$sheet->setCellValueByColumnAndRow( $str_c2+1, $str_r+2, $winstr[2][2] );
                if( $match == 1 ){
                    $pos1_1 = $c1;
                    $pos1_2 = $c2;
                } else {
                ...
                if( $tournament_data['match'][$match]['winner'] == 1 ){
					...
					$colstr2 = PHPExcel_Cell::stringFromColumnIndex( $c3-1 );
					$colname = PHPExcel_Cell::stringFromColumnIndex( $c1-1 );
					$sheet->getStyle( $colstr1.$row )->applyFromArray( $styleArrayVR_left );
					if( $level > 0 ){
						$sheet->getStyle( $colstr1.($row+1) )->applyFromArray( $styleArrayVR_left );
					}
					$sheet->getStyle( $colstr1.$row.':'.$colstr2.$row )->applyFromArray( $styleArrayHR_top );
					$sheet->setCellValueByColumnAndRow( $c3-1, $nextrow+2, '=$'.$colname.'$'.($row+2) );
					$cord_win = PHPExcel_Cell::stringFromColumnIndex( $str_c1 ) . $str_r;
					$cord_lose = PHPExcel_Cell::stringFromColumnIndex( $str_c2 ) . $str_r;
                }
                if( $tournament_data['match'][$match]['winner'] == 2 ){
					$colstr1 = PHPExcel_Cell::stringFromColumnIndex( $c3 );
					$colstr2 = PHPExcel_Cell::stringFromColumnIndex( $c2-1 );
					$colname = PHPExcel_Cell::stringFromColumnIndex( $c2-1 );
					$sheet->getStyle( $colstr2.$row )->applyFromArray( $styleArrayVR_right );
					if( $level > 0 ){
						$sheet->getStyle( $colstr2.($row+1) )->applyFromArray( $styleArrayVR_right );
					}
					$sheet->getStyle( $colstr1.$row.':'.$colstr2.$row )->applyFromArray( $styleArrayHR_top );
					$sheet->setCellValueByColumnAndRow( $c3-1, $nextrow+2, '=$'.$colname.'$'.($row+2) );
					$cord_win = PHPExcel_Cell::stringFromColumnIndex( $str_c2 ) . $str_r;
					$cord_lose = PHPExcel_Cell::stringFromColumnIndex( $str_c1 ) . $str_r;
                }
                if( $tournament_data['match'][$match]['winner'] != 0 ){
					$objDrawing = new PHPExcel_Worksheet_Drawing();
					$objDrawing->setName( $cord_win );
					$objDrawing->setDescription( $cord_win );
					$objDrawing->setPath(dirname(dirname(__FILE__)).'/cir2.png');
					$objDrawing->setWidth(36);
					$objDrawing->setHeight(36);
					$objDrawing->setWorksheet($sheet);
					$objDrawing->setCoordinates( $cord_win );
					$objDrawing->setOffsetX(-2);
					$objDrawing->setOffsetY(-2);
					$objDrawing2 = new PHPExcel_Worksheet_Drawing();
					$objDrawing2->setName( $cord_lose );
					$objDrawing2->setDescription( $cord_lose );
					$objDrawing2->setPath(dirname(dirname(__FILE__)).'/tri.png');
					$objDrawing2->setWidth(36);
					$objDrawing2->setHeight(36);
					$objDrawing2->setWorksheet($sheet);
					$objDrawing2->setCoordinates( $cord_lose );
...
//exit;
	...
	function output_tournament_chart_41_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, $mw )
	{
		return __output_tournament_chart_41_41_for_excel( $objPage, $series_info, $tournament_param, $tournament_data, $league_data, $entry_list, 'm' );
	...
	function __output_tournament_match_for_HTML2_41_41( $series_info, $tournament_list, $entry_list, $mw )
	{
		$objPage = new form_page();
		if( $mw == 'm' ){
			$mwstr = '男子';
		...
		$html = $header . '    <h2>トーナメント結果</h2>'."\n";
		/*
        $tindex = 1;
		for( $i1 = 15; $i1 <= 30; $i1++ ){
			if( $tournament_list[0]['match'][$i1]['place'] != 0 && $tournament_list[0]['match'][$i1]['place'] != 'no_match' ){
				$html .= '<h3>トーナメント&nbsp;一回戦&nbsp;第'.$tindex.'試合</h3>';
		...
		*/
		...
		for( $i1 = 3; $i1 <= 6; $i1++ ){
			if( $tournament_list[0]['match'][$i1]['place'] != 0 && $tournament_list[0]['match'][$i1]['place'] != 'no_match' ){
				$html .= '<h3>トーナメント&nbsp;準々決勝&nbsp;第'.$tindex.'試合</h3>';
				$html .= $objPage->output_one_match_for_HTML2( $series_info, $tournament_list[0]['match'][$i1], $entry_list, $mw );
				$tindex++;
			}
		}
		...
		for( $i1 = 1; $i1 <= 2; $i1++ ){
			if( $tournament_list[0]['match'][$i1]['place'] != 0 && $tournament_list[0]['match'][$i1]['place'] != 'no_match' ){
				$html .= '<h3>トーナメント&nbsp;準決勝&nbsp;第'.$tindex.'試合</h3>';
				$html .= $objPage->output_one_match_for_HTML2( $series_info, $tournament_list[0]['match'][$i1], $entry_list, $mw );
				$tindex++;
			}
		}
		$html .= '<h3>トーナメント&nbsp;決勝</h3>';
...
*/
		$html .= $footer;
        $file = 'dtm_' . $series_info['result_prefix'] . $mw . '1';
	...
	function output_tournament_match_for_HTML2_41( $series_info, $tournament_list, $entry_list )
	{
		__output_tournament_match_for_HTML2_41_41( $series_info, $tournament_list, $entry_list, 'm' );
	}
	...
	function output_tournament_41_for_PDF( $series_info, $tournament_data, $entry_list, $mv )
	{
        $objPage = new form_page();
		$data = [
			'navi_id' => $series_info['navi_id'],
	...
	function __output_tournament_41_41_for_HTML( $series_info, $tournament_list, $entry_list, $mw )
	{
//print_r($tournament_data);
//print_r($entry_list);
		$objPage = new form_page();
	...
	//	$pdf = '' . "\n"
		...
		//    . '    position: absolute;' . "\n"
			...
			. '}' . "\n"
			. '</style>' . "\n"
			. '<H1 style="border-bottom: solid 1px #000000;" lang="ja">トーナメント表</H1>' . "\n"
...
//exit;
	...
	function output_tournament_41_for_HTML( $series_info, $tournament_data, $entry_list )
	{
		__output_tournament_41_41_for_HTML( $series_info, $tournament_data, $entry_list, 'm' );
	}
	...
	function __output_entry_data_list_all_1_excel41_41( $sheet, $series, $mv, $start_pos, $series_name, $kaisai_rev )
	{
		$c = new common();
	...
	function output_entry_data_list_all_1_excel41( $sheet )
	{
		__output_entry_data_list_all_1_excel41_41( $sheet, 41, 'm', 4, '団体戦男子', 1 );
		__output_entry_data_list_all_1_excel41_41( $sheet, 41, 'w', 4+48, '団体戦女子', 1 );
		return false;
	}

	//--------------------------------------------------------------

	function __get_entry_data_list2_41_41( $series )
	{
		$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
		$sql = 'select * from `entry_info`'
			.' where `series`='.$series.' and `year`='.$_SESSION['auth']['year']
	...
	function get_entry_data_list2_41()
	{
		return __get_entry_data_list2_41_41( 41 );
	}

	//--------------------------------------------------------------

	function __get_entry_data_for_draw_csv_41_41( $list, $series )
	{
		$c = new common();
	...
	function get_entry_data_for_draw_csv_41( $list )
	{
		return array(
			'csv' => __get_entry_data_for_draw_csv_41_41( $list, 41 ),
			'file' => 'dantai_m.csv'
		);
	}
	...
	function __output_realtime_html_for_one_board_41_41( $place, $place_match_no )
	{
		global $navi_info;
...
*/
	...
	function output_realtime_html_for_one_board_41( $place, $place_match_no )
	{
		//return __output_realtime_html_for_one_board_41_41( $place, $place_match_no );
		return '';
	}
