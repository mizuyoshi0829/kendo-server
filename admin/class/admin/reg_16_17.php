<?php
	...
	{
        return __get_league_parameter_16_17( 'm' );
	}
...
//							$html .= $league_list[$league_data]['match'][$match_no_index-1]['hon2'].'-'.$league_list[$league_data]['match'][$match_no_index-1]['win2'];
	...
	function __output_league_16_17_for_HTML( $path, $league_param, $league_list, $entry_list, $mv )
	{
//echo $path;
//print_r($league_list);
		if( $mv == 'm' ){
...
//							$html .= $league_list[$league_data]['match'][$match_no_index-1]['hon2'].'-'.$league_list[$league_data]['match'][$match_no_index-1]['win2'];
	...
	function output_league_16_for_HTML( $path, $league_list, $entry_list )
	{
		__output_league_16_17_for_HTML( $path, get_league_parameter_16(), $league_list, $entry_list, 'm' );
	}

	function output_league_17_for_HTML( $path, $league_list, $entry_list )
	{
		__output_league_16_17_for_HTML( $path, get_league_parameter_17(), $league_list, $entry_list, 'w' );
	}
...
//print_r($league_list);
	...
	function __output_league_16_17_for_Excel( $objPage, $path, $league_param, $league_list, $entry_list, $mw )
	{
//print_r($league_list);
		if( $mw == 'm' ){
			$mwstr = '男子';
		...
		$file_name = 'H28nanshinLeagueResults_' . $mw . '.xls';
...
//echo "1:",$mg_range1,':',$mg_range2,'<br />';
	...
	function output_league_16_for_Excel( $objPage, $path, $league_list, $entry_list )
	{
		return __output_league_16_17_for_Excel( $objPage, $path, get_league_parameter_16(), $league_list, $entry_list, 'm' );
	}

	function output_league_17_for_Excel( $objPage, $path, $league_list, $entry_list )
	{
		return __output_league_16_17_for_Excel( $objPage, $path, get_league_parameter_17(), $league_list, $entry_list, 'w' );
	}

	//---------------------------------------------------------------
	//
	//---------------------------------------------------------------
	function __output_league_match_for_HTML2_16_17( $objPage, $path, $league_list, $entry_list, $mv )
	{
		if( $mv == 'm' ){
			$mvstr = '男子';
		} else {
	...
	function output_league_match_for_HTML2_16( $objPage, $path, $league_list, $entry_list )
	{
		__output_league_match_for_HTML2_16_17( $objPage, $path, $league_list, $entry_list, 'm' );
	}

	function output_league_match_for_HTML2_17( $objPage, $path, $league_list, $entry_list )
	{
		__output_league_match_for_HTML2_16_17( $objPage, $path, $league_list, $entry_list, 'w' );
	}
	...
	function __output_league_match_for_excel_16_17( $objPage, $path, $league_list, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//print_r($league_list);
//exit;
		$file_path = $path . '/H28nanshinLeagueMatchResults_'.$mw.'.xls';
		$file_name = 'H28nanshinLeagueMatchResults_'.$mw.'.xls';
		...
		for( $league_data = 0; $league_data < count( $league_list ); $league_data++ ){
			...
			for( $match = 0; $match < $league_list[$league_data]['match_num']; $match++ ){
				...
				$row += 2;
				$objPage->output_one_match_for_excel( $sheet, $col, $row, $league_list[$league_data]['match'][$match], $entry_list, $mv, 46, 46 );
				$row += 4;
			}
		}
		$writer = PHPExcel_IOFactory::createWriter( $excel, 'Excel5' );
	...
	function output_league_match_for_excel_16( $objPage, $path, $league_list, $entry_list )
	{
		return __output_league_match_for_excel_16_17( $objPage, $path, $league_list, $entry_list, 'm' );
	}

	function output_league_match_for_excel_17( $objPage, $path, $league_list, $entry_list )
	{
		return __output_league_match_for_excel_16_17( $objPage, $path, $league_list, $entry_list, 'w' );
	}
	...
	function __output_tournament_match_16_17_for_excel( $objPage, $path, $tournament_data, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
		...
		require_once dirname(dirname(dirname(__FILE__))).'/phpExcel/Classes/PHPExcel.php';
		require_once dirname(dirname(dirname(__FILE__))).'/phpExcel/Classes/PHPExcel/IOFactory.php';
		$file_name = 'H28nanshinTournamentMatchResults_' . $mw . '.xls';
		$file_path = $path . '/' . $file_name;
		...
		$row = 6;
		for( $i1 = 1; $i1 <= 4; $i1++ ){
            $objPage->output_one_match_for_excel( $sheet, $col, $row, $tournament_data['match'][$i1+2], $entry_list, $mv, 46, 46 );
            $row += 6;
        }
		for( $i1 = 1; $i1 <= 2; $i1++ ){
            $objPage->output_one_match_for_excel( $sheet, $col, $row, $tournament_data['match'][$i1], $entry_list, $mv, 46, 46 );
            $row += 6;
        }
        $objPage->output_one_match_for_excel( $sheet, $col, $row, $tournament_data['match'][0], $entry_list, $mv, 46, 46 );
        $row += 6;
        $objPage->output_one_match_for_excel( $sheet, $col, $row, $tournament_data['match'][7], $entry_list, $mv, 46, 46 );
	...
	function output_tournament_match_16_for_excel( $objPage, $path, $tournament_data, $entry_list, $mv )
	{
		return __output_tournament_match_16_17_for_excel( $objPage, $path, $tournament_data, $entry_list, 'm' );
	}

	function output_tournament_match_17_for_excel( $objPage, $path, $tournament_data, $entry_list, $mv )
	{
		return __output_tournament_match_16_17_for_excel( $objPage, $path, $tournament_data, $entry_list, 'w' );
	}
	...
	function __output_tournament_16_17_for_excel( $objPage, $path, $tournament_data, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//exit;
        $file_name = 'H28nanshinDantaiTounamentResults_' . $mw . '.xls';
        $file_path = $path . '/' . $file_name;
        $excel = new PHPExcel();
        $excel->setActiveSheetIndex( 0 );        //何番目のシートに有効にするか
        $sheet = $excel->getActiveSheet();    //有効になっているシートを取得
        ...
        for( $team = 0; $team < $tournament_team_num; $team++ ){
            ...
            if( $player < $tournament_team_num2 ){
                $sheet->setCellValueByColumnAndRow( $col, $row, $index );
                $sheet->setCellValueByColumnAndRow( $col+1, $row, $tournament_data['team'][$team]['name'] );            } else {
        ...
        if( $tournament_data['match'][$tournament_player_num-1]['winner'] == 1 ){
            $sheet->setCellValueByColumnAndRow( 2, $row, $tournament_data['match'][$tournament_team_num-1]['team1_name'] );
        } else if( $tournament_data['match'][$tournament_player_num-1]['winner'] == 2 ){
	...
	function output_tournament_16_for_excel( $objPage, $path, $tournament_data, $entry_list, $mv )
	{
		return __output_tournament_16_17_for_excel( $objPage, $path, $tournament_data, $entry_list, 'm' );
	}

	function output_tournament_17_for_excel( $objPage, $path, $tournament_data, $entry_list, $mv )
	{
		return __output_tournament_16_17_for_excel( $objPage, $path, $tournament_data, $entry_list, 'w' );
	}
	...
	function __output_tournament_chart_16_17_for_excel( $objPage, $path, $tournament_data, $league_data, $entry_list, $mv )
	{
		if( $mv == 'm' ){
			$mvstr = '男子';
		} else {
	...
	function output_tournament_chart_16_for_excel( $objPage, $path, $tournament_data, $league_data, $entry_list, $mv )
	{
		return __output_tournament_chart_16_17_for_excel( $objPage, $path, $tournament_data, $league_data, $entry_list, 'm' );
	}

	function output_tournament_chart_17_for_excel( $objPage, $path, $tournament_data, $league_data, $entry_list, $mv )
	{
		return __output_tournament_chart_16_17_for_excel( $objPage, $path, $tournament_data, $league_data, $entry_list, 'w' );
	}
	...
	function __output_tournament_match_for_HTML2_16_17( $objPage, $path, $tournament_list, $entry_list, $mv )
	{
		if( $mv == 'm' ){
			$mvstr = '男子';
		} else {
	...
	function output_tournament_match_for_HTML2_16( $objPage, $path, $tournament_list, $entry_list )
	{
		__output_tournament_match_for_HTML2_16_17( $objPage, $path, $tournament_list, $entry_list, 'm' );
	}

	function output_tournament_match_for_HTML2_17( $objPage, $path, $tournament_list, $entry_list )
	{
		__output_tournament_match_for_HTML2_16_17( $objPage, $path, $tournament_list, $entry_list, 'w' );
	}
...
exit;
	...
	function __output_tournament_16_17_for_HTML( $path, $tournament_data, $entry_list, $mv )
	{
//print_r($tournament_data);
//print_r($entry_list);
		if( $mv == 'm' ){
...
//exit;
	...
	function output_tournament_16_for_HTML( $path, $tournament_data, $entry_list )
	{
		__output_tournament_16_17_for_HTML( $path, $tournament_data, $entry_list, 'm' );
	}

	function output_tournament_17_for_HTML( $path, $tournament_data, $entry_list )
	{
		__output_tournament_16_17_for_HTML( $path, $tournament_data, $entry_list, 'w' );
	}
