<?php
    ...
    function __output_realtime_html_for_one_board_name( $name, $name2 )
    {
        $ret = [
            'html' => '',
            'font_class' => '',
    ...
    function __output_realtime_html_for_one_board_get_dantai_player_name( $data, $seriesinfo, $match_info, $player, $match )
    {
        $player_field = 'player' . $player;
        $player_no = $data['matches'][$match][$player_field];
        if( $player_no == 0 ){
            return __output_realtime_html_for_one_board_name( '', '' );
        }
        if( $player_no == __PLAYER_NAME__ ){
            return __output_realtime_html_for_one_board_name( $data['matches'][$match][$player_field.'_change_name'], '' );
        }
        ...
        if( $data['entry'.$player][$f.'_disp'] != '' ){
            return __output_realtime_html_for_one_board_name( $data['entry'.$player][$f.'_disp'], '' );
        }
        ...
        return __output_realtime_html_for_one_board_name( $name, $name_add );
    }

    function __output_realtime_html_for_one_board_school_name( $data, $is_dantai, $team )
    {
        if( $is_dantai ){
            ...
            if( $fontlen >= 15 ){
                $font_class = ' tb_frame_content2_smallfont15';
            } else if( $fontlen > 5 ){
                $font_class = ' tb_frame_content2_smallfont' . $fontlen;
            }
            $hinfo = __output_realtime_html_for_one_board_name( get_field_string($data['entry'.$team],'school_name_ryaku'), '' );
        } else {
            $hinfo = __output_realtime_html_for_one_board_name( '', '' );
        }
        $html = '        <div class="tb_frame_content2' . $hinfo['font_class'] . '" id="school_name1">'."\n"
            . '          ' . $hinfo['html'] . "\n"
            . '        </div>'."\n";
        return $html;
    }

    function __output_realtime_html_for_one_board_waza( $data, $match, $team )
    {
        $html = '';
        for( $i2 = 1; $i2 <= 3; $i2++ ){
            $head = 'waza' . $team . '_';
            ...
            if( $data['matches'][$match][$head.$i2] == 2 ){ $html .= 'ド'; }
            if( $data['matches'][$match][$head.$i2] == 3 ){ $html .= 'コ'; }
            if( $data['matches'][$match][$head.$i2] == 4 ){ $html .= '反'; }
            if( $data['matches'][$match][$head.$i2] == 5 ){ $html .= '○'; }
            if( $data['matches'][$match][$head.$i2] == 6 ){ $html .= 'ツ'; }
            if( $data['matches'][$match][$head.$i2] == 7 ){ $html .= '判'; }
            $html .= '</div>'."\n";
        }
        return $html;
    }
...
/**/
            ...
            if( $place_match_no > 1 ){
                ...
                $exist_matches[1] = false;
            }
            $exist_matches[2] = false;
...
//print_r($data_now);
        ...
        $html .= '</div>'."\n";
        $html .= __output_realtime_html_for_one_board_school_name( $data, $is_dantai, 1 );
        $html .= '      </div>'."\n";
        for( $i1 = 1; $i1 <= 5; $i1++ ){
            $html .= '      <div class="tb_frame">'."\n";
            $html .= '        <div class="tb_frame_title tb_frame_bbottom';
            if( $is_dantai && $place_match_player_no > 0 && $place_match_player_no == $i1 ){
            ...
            $html .=  '</div>'."\n";
            if( $is_dantai ){
                $hinfo = __output_realtime_html_for_one_board_get_dantai_player_name( $data, $series_info, $match_info, 1, $i1 );
            } else {
                $n = '';
                if( $i1 == 1 ){
                    if( $exist_matches[1] ){
                        $n = get_field_string( $data_prev['players'][1], 'name_str2' );
                    }
                } else if( $i1 == 2 ){
                } else if( $i1 == 3 ){
                    if( $exist_matches[3] ){
                        $n = get_field_string( $data_now['players'][1], 'name_str2' );
                    }
                } else if( $i1 == 4 ){
                } else if( $i1 == 5 ){
                    if( $exist_matches[5] ){
                        $n = get_field_string( $data_next['players'][1], 'name_str2' );
                    }
                }
                $hinfo = __output_realtime_html_for_one_board_name( $n, '' );
            }
            $html .=  '        <div class="tb_frame_content';
            if( $is_dantai && $data['matches'][$i1]['player1'] != 0 ){
                if(
            ...
            $html .= $hinfo['font_class'] . '" id="player1_' . $i1 . '">';
            if( $exist_matches[$i1] ){
                $html .= $hinfo['html'];
            ...
            if( $exist_matches[$i1] ){
                if( $data['matches'][$i1]['faul1_1'] == 2 ){ $html .=   '<div class="tb_frame_faul_shidou">指</div>'; }
                if( $data['matches'][$i1]['faul1_2'] == 1 ){ $html .=   '<div class="tb_frame_faul_faul"><span class="text-red">▲</span></div>'; }
            ...
            if( $exist_matches[$i1] ){
                $html .= __output_realtime_html_for_one_board_waza( $data, $i1, 1 );
            }
            $html .=   '      </div>'."\n";
            $html .=   '      </div>'."\n";
        }
        ...
        $html .= '">';
        if( $is_dantai ){ $html .= '代表戦'; }
        $html .= '</div>'."\n";
        $html .= '        <div class="tb_frame_content';
        if( $is_dantai ){
            $hinfo = __output_realtime_html_for_one_board_get_dantai_player_name( $data, $series_info, $match_info, 1, 6 );
        } else {
            $hinfo = __output_realtime_html_for_one_board_name( '', '' );
        }
        $html .= $hinfo['font_class'] . '" id="player1_6">';
        $html .= $hinfo['html'];
        if( $is_dantai && $data['matches'][6]['supporter1'] == 1 ){
            $html .= '<div class="tb_frame_supporter1">●</div>';
        }
        $html .= '</div>'."\n";
        $html .= '        <div class="tb_frame_faul tb_frame_btop">'."\n";
        if( $is_dantai ){
            if($data['matches'][6]['faul1_1']==2){ $html .= '<div class="tb_frame_faul_shidou">指</div>'; }
            if($data['matches'][6]['faul1_2']==1){ $html .= '<div class="tb_frame_faul_faul"><span class="text-red">▲</span></div>'; }
        ...
        $html .= '        <div class="tb_frame_waza">'."\n";
        if( $is_dantai ){
            $html .= __output_realtime_html_for_one_board_waza( $data, 6, 1 );
        }
        $html .=   '      </div>'."\n";
        $html .= '      </div>'."\n";
        if( $series_info['enable_referee'] != 0 ){
            $html .= '      <div class="tb_frame hide_referee">'."\n";
            $html .= '          <div class="tb_frame_hide_refereetitle tb_frame_bbottom">主審</div>'."\n";
            $html .= '          <div class="tb_frame_referee">'.string_insert_br( $data['referee1_name'] ).'</div>'."\n";
            $html .= '      </div>'."\n";
        }
        $html .= '      <div class="clearfloat"></div>'."\n";
        $html .= '            <div class="tb_frame">'."\n";
        $html .= __output_realtime_html_for_one_board_school_name( $data, $is_dantai, 2 );
        $html .= '            </div>'."\n";
        for( $i1 = 1; $i1 <= 5; $i1++ ){
            $html .= '      <div class="tb_frame">'."\n";
            ...
            // 
            if( $is_dantai ){
                $hinfo = __output_realtime_html_for_one_board_get_dantai_player_name( $data, $series_info, $match_info, 2, $i1 );
            } else {
                $n = '';
                if( $i1 == 1 ){
                    if( $exist_matches[1] ){
                        $n = get_field_string( $data_prev['players'][2], 'name_str2' );
                    }
                } else if( $i1 == 2 ){
                } else if( $i1 == 3 ){
                    if( $exist_matches[3] ){
                        $n = get_field_string( $data_now['players'][2], 'name_str2' );
                    }
                } else if( $i1 == 4 ){
                } else if( $i1 == 5 ){
                    if( $exist_matches[5] ){
                        $n = get_field_string( $data_next['players'][2], 'name_str2' );
                    }
                }
                $hinfo = __output_realtime_html_for_one_board_name( $n, '' );
            }
            $html .= '        <div class="tb_frame_content';
            if( $is_dantai && $data['matches'][$i1]['player2'] != 0 ){
                if(
        ...
        $html .= '        <div class="tb_frame_content';
        if( $is_dantai ){
            $hinfo = __output_realtime_html_for_one_board_get_dantai_player_name( $data, $series_info, $match_info, 2, 6 );
        } else {
            $hinfo = __output_realtime_html_for_one_board_name( '', '' );
        }
        $html .= $hinfo['font_class'] . '" id="player2_6">';
        $html .= $hinfo['html'];
        if( $is_dantai ){
...
//print_r($data_now);
        ...
        if( $data_now['matches']['faul1_1'] == 2 ){ $html .=   '<div class="tb_frame_faul_shidou">指</div>'; }
        if( $data_now['matches']['faul1_2'] == 1 ){ $html .=   '<div class="tb_frame_faul_faul"><span class="text-red">▲</span></div>'; }
        ...
        $html .= '      </div>'."\n";
        $html .= '      <div class="tb_frame_faul">'."\n";
        if( $data_now['matches']['faul2_1'] == 2 ){ $html .=   '<div class="tb_frame_faul_shidou_b">指</div>'; }
        if( $data_now['matches']['faul2_2'] == 1 ){ $html .=   '<div class="tb_frame_faul_faul_b"><span class="text-red">▲</span></div>'; }
        $html .= '      </div>'."\n";
        $html .= '      <div class="tb_frame_waza">'."\n";
        for( $i2 = 1; $i2 <= 3; $i2++ ){
    ...
    function __output_realtime_html_for_one_board_sub3( $navi_id, $place, $place_match_no, $match_no, $objPage, $match_info, $match, $series_info )
    {
        ...
        $html .= '      </div>'."\n";
        $html .= '      <div class="tb_frame_faul">'."\n";
        if( $data['matches'][$match_no]['faul1_1'] == 2 ){ $html .=   '<div class="tb_frame_faul_shidou">指</div>'; }
        if( $data['matches'][$match_no]['faul1_2'] == 1 ){ $html .=   '<div class="tb_frame_faul_faul"><span class="text-red">▲</span></div>'; }
        ...
        $html .= '      </div>'."\n";
        $html .= '      <div class="tb_frame_faul">'."\n";
        if( $data['matches'][$match_no]['faul2_1'] == 2 ){ $html .=   '<div class="tb_frame_faul_shidou_b">指</div>'; }
        if( $data['matches'][$match_no]['faul2_2'] == 1 ){ $html .=   '<div class="tb_frame_faul_faul_b"><span class="text-red">▲</span></div>'; }
        $html .= '      </div>'."\n";
        $html .= '      <div class="tb_frame_waza">'."\n";
        for( $i2 = 1; $i2 <= 3; $i2++ ){
...
//print_r($data);
        ...
        for( $i1 = 1; $i1 <= 5; $i1++ ){
            ...
            if( $i1 == 3 ){
                $info = __output_realtime_html_for_one_board_get_dantai_player_name( $data, $series_info, $match_info, 1, 1 );
                $html .= $info['html'];
            }
            $html .=   '</div>'."\n";
            $html .=   '        <div class="tb_frame_faul tb_frame_btop">'."\n";
        ...
        for( $i1 = 1; $i1 <= 5; $i1++ ){
            ...
            $html .= '        </div>'."\n";
            $html .= '        <div class="tb_frame_faul_2 tb_frame_btop">'."\n";
            if( $i1 == 3 ){
                if($data['matches'][1]['faul2_1']==2){ $html .= '<div class="tb_frame_faul_shidou_b">指</div>'; }
                if($data['matches'][1]['faul2_2']==1){ $html .= '<div class="tb_frame_faul_faul_b"><span class="text-red">▲</span></div>'; }
            }
            $html .= '        </div>'."\n";
            $html .= '        <div class="tb_frame_content';
            $html .= '" id="player2_'.$i1.'">';
            if( $i1 == 3 ){
                $info = __output_realtime_html_for_one_board_get_dantai_player_name( $data, $series_info, $match_info, 2, 1 );
                $html .= $info['html'];
            }
            $html .= '</div>'."\n";
            $html .= '      </div>'."\n";
...
//print_r($data_now);
        ...
        $html .= '      </div>'."\n";
        $html .= '      <div class="tb_frame_faul">'."\n";
        if( $data_now['matches'][$faul_field_left.'1'] == 2 ){ $html .=   '<div class="tb_frame_faul_shidou">指</div>'; }
        if( $data_now['matches'][$faul_field_left.'2'] == 1 ){ $html .=   '<div class="tb_frame_faul_faul"><span class="text-red">▲</span></div>'; }
        if( $data_now['matches']['extra'] == 1 ){
            $html .=   '          <div class="tb_frame_faul_extra" id="extra_match">延長</div>'."\n";
        }
        if( $data_now['matches']['end_match'] == 1 ){
        ...
        $html .= '      </div>'."\n";
        $html .= '      <div class="tb_frame_faul">'."\n";
        if( $data_now['matches'][$faul_field_right.'1'] == 2 ){ $html .=   '<div class="tb_frame_faul_shidou">指</div>'; }
        if( $data_now['matches'][$faul_field_right.'2'] == 1 ){ $html .=   '<div class="tb_frame_faul_faul"><span class="text-red">▲</span></div>'; }
        $html .= '      </div>'."\n";
        $html .= '      <div class="tb_frame_waza">'."\n";
        for( $i2 = 1; $i2 <= 3; $i2++ ){
