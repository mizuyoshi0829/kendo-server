...
//print_r($serieslist);
	...
	function get_navi_current_input_match_player_no( $navi_id, $place )
    {
        if( $navi_id == 0 ){ return 0; }
        $dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
        $sql = 'select * from `navi_input_info` where `navi_id`=' . $navi_id . ' and `place`=' . $place;
...
//print_r($serieslist);
	...
	function get_navi_current_input_match_player_no_of_match( $series_info, $match_id )
    {
        if( $series_info['navi_id'] == 0 ){ return 0; }
        $dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
        $sql = 'select `navi_input_info`.`player` as `player`'
            . ' from `navi_input_info`'
            . ' inner join `navi_info` on `navi_info`.`place_no`=`navi_input_info`.`place`'
            . ' and `navi_info`.`place_match_no`=`navi_input_info`.`match`'
            . ' where `navi_input_info`.`navi_id`=' . $series_info['navi_id']
            . ' and `navi_info`.`match_id`=' . $match_id;
        $list = db_query_list( $dbs, $sql );
//echo $sql,"\n";
//print_r($list);
        if( count( $list ) == 0 ){ return 0; }
...
//print_r($serieslist);
        if( count( $list ) == 0 ){
            $sql = 'insert into `navi_input_info` set `created`=NOW(),`modified`=NOW(),`navi_id`='.$navi_id.',`place`='.$place.',`match`='.$match.',`player`='.$player;
        } else {
            $sql = 'update `navi_input_info` set `match`='.$match.',`player`='.$player.' where `id`='.$list[0]['id'];
        }
        db_query( $dbs, $sql );
   }
	...
	{
		$tbl = array(
			array( 'value' => 1, 'title' => '北海道', 'title2' => '北海', 'mode' => 2 ),
			array( 'value' => 2, 'title' => '青森', 'mode' => 4 ),
			array( 'value' => 3, 'title' => '岩手', 'mode' => 4 ),
			array( 'value' => 4, 'title' => '宮城', 'mode' => 4 ),
			array( 'value' => 5, 'title' => '秋田', 'mode' => 4 ),
			array( 'value' => 6, 'title' => '山形', 'mode' => 4 ),
			array( 'value' => 7, 'title' => '福島', 'mode' => 4 ),
			array( 'value' => 8, 'title' => '茨城', 'mode' => 4 ),
			array( 'value' => 9, 'title' => '栃木', 'mode' => 4 ),
			array( 'value' => 10, 'title' => '群馬', 'mode' => 4 ),
			array( 'value' => 11, 'title' => '埼玉', 'mode' => 4 ),
			array( 'value' => 12, 'title' => '千葉', 'mode' => 4 ),
			array( 'value' => 13, 'title' => '東京', 'mode' => 1 ),
			array( 'value' => 14, 'title' => '神奈川', 'mode' => 4 ),
			array( 'value' => 15, 'title' => '新潟', 'mode' => 4 ),
			array( 'value' => 16, 'title' => '富山', 'mode' => 4 ),
			array( 'value' => 17, 'title' => '石川', 'mode' => 4 ),
			array( 'value' => 18, 'title' => '福井', 'mode' => 4 ),
			array( 'value' => 19, 'title' => '山梨', 'mode' => 4 ),
			array( 'value' => 20, 'title' => '長野', 'mode' => 4 ),
			array( 'value' => 21, 'title' => '岐阜', 'mode' => 4 ),
			array( 'value' => 22, 'title' => '静岡', 'mode' => 4 ),
			array( 'value' => 23, 'title' => '愛知', 'mode' => 4 ),
			array( 'value' => 24, 'title' => '三重', 'mode' => 4 ),
			array( 'value' => 25, 'title' => '滋賀', 'mode' => 4 ),
			array( 'value' => 26, 'title' => '京都', 'mode' => 3 ),
			array( 'value' => 27, 'title' => '大阪', 'mode' => 3 ),
			array( 'value' => 28, 'title' => '兵庫', 'mode' => 4 ),
			array( 'value' => 29, 'title' => '奈良', 'mode' => 4 ),
			array( 'value' => 30, 'title' => '和歌山', 'mode' => 4 ),
			array( 'value' => 31, 'title' => '鳥取', 'mode' => 4 ),
			array( 'value' => 32, 'title' => '島根', 'mode' => 4 ),
			array( 'value' => 33, 'title' => '岡山', 'mode' => 4 ),
			array( 'value' => 34, 'title' => '広島', 'mode' => 4 ),
			array( 'value' => 35, 'title' => '山口', 'mode' => 4 ),
			array( 'value' => 36, 'title' => '徳島', 'mode' => 4 ),
			array( 'value' => 37, 'title' => '香川', 'mode' => 4 ),
			array( 'value' => 38, 'title' => '愛媛', 'mode' => 4 ),
			array( 'value' => 39, 'title' => '高知', 'mode' => 4 ),
			array( 'value' => 40, 'title' => '福岡', 'mode' => 4 ),
			array( 'value' => 41, 'title' => '佐賀', 'mode' => 4 ),
			array( 'value' => 42, 'title' => '長崎', 'mode' => 4 ),
			array( 'value' => 43, 'title' => '熊本', 'mode' => 4 ),
			array( 'value' => 44, 'title' => '大分', 'mode' => 4 ),
			array( 'value' => 45, 'title' => '宮崎', 'mode' => 4 ),
			array( 'value' => 46, 'title' => '鹿児島', 'mode' => 4 ),
			array( 'value' => 47, 'title' => '沖縄', 'mode' => 4 )
		);
		return $tbl;
	}
	...
	{
		$tbl = array(
			array( 'value' => 1950, 'title' => '1988年/昭和25年' ),
			array( 'value' => 1951, 'title' => '1988年/昭和26年' ),
			array( 'value' => 1952, 'title' => '1988年/昭和27年' ),
			array( 'value' => 1953, 'title' => '1988年/昭和28年' ),
			array( 'value' => 1954, 'title' => '1988年/昭和29年' ),
			array( 'value' => 1955, 'title' => '1988年/昭和30年' ),
			array( 'value' => 1956, 'title' => '1988年/昭和31年' ),
			array( 'value' => 1957, 'title' => '1988年/昭和32年' ),
			array( 'value' => 1958, 'title' => '1988年/昭和33年' ),
			array( 'value' => 1959, 'title' => '1988年/昭和34年' ),
			array( 'value' => 1960, 'title' => '1988年/昭和35年' ),
			array( 'value' => 1961, 'title' => '1988年/昭和36年' ),
			array( 'value' => 1962, 'title' => '1988年/昭和37年' ),
			array( 'value' => 1963, 'title' => '1988年/昭和38年' ),
			array( 'value' => 1964, 'title' => '1988年/昭和39年' ),
			array( 'value' => 1965, 'title' => '1988年/昭和40年' ),
			array( 'value' => 1966, 'title' => '1988年/昭和41年' ),
			array( 'value' => 1967, 'title' => '1988年/昭和42年' ),
			array( 'value' => 1968, 'title' => '1988年/昭和43年' ),
			array( 'value' => 1969, 'title' => '1988年/昭和44年' ),
			array( 'value' => 1970, 'title' => '1988年/昭和45年' ),
			array( 'value' => 1971, 'title' => '1988年/昭和46年' ),
			array( 'value' => 1972, 'title' => '1988年/昭和47年' ),
			array( 'value' => 1973, 'title' => '1988年/昭和48年' ),
			array( 'value' => 1974, 'title' => '1988年/昭和49年' ),
			array( 'value' => 1975, 'title' => '1988年/昭和50年' ),
			array( 'value' => 1976, 'title' => '1988年/昭和51年' ),
			array( 'value' => 1977, 'title' => '1988年/昭和52年' ),
			array( 'value' => 1978, 'title' => '1988年/昭和53年' ),
			array( 'value' => 1979, 'title' => '1988年/昭和54年' ),
			array( 'value' => 1980, 'title' => '1988年/昭和55年' ),
			array( 'value' => 1981, 'title' => '1988年/昭和56年' ),
			array( 'value' => 1982, 'title' => '1988年/昭和57年' ),
			array( 'value' => 1983, 'title' => '1988年/昭和58年' ),
			array( 'value' => 1984, 'title' => '1988年/昭和59年' ),
			array( 'value' => 1985, 'title' => '1988年/昭和60年' ),
			array( 'value' => 1986, 'title' => '1988年/昭和61年' ),
			array( 'value' => 1987, 'title' => '1988年/昭和62年' ),
			array( 'value' => 1988, 'title' => '1988年/昭和63年' ),
			array( 'value' => 1989, 'title' => '1989年/平成元年/昭和64年' ),
			array( 'value' => 1990, 'title' => '1990年/平成2年' ),
			array( 'value' => 1991, 'title' => '1991年/平成3年' ),
			array( 'value' => 1992, 'title' => '1992年/平成4年' ),
			array( 'value' => 1993, 'title' => '1993年/平成5年' ),
			array( 'value' => 1994, 'title' => '1994年/平成6年' ),
			array( 'value' => 1995, 'title' => '1995年/平成7年' ),
			array( 'value' => 1996, 'title' => '1996年/平成8年' ),
			array( 'value' => 1997, 'title' => '1997年/平成9年' ),
			array( 'value' => 1998, 'title' => '1998年/平成10年' ),
			array( 'value' => 1999, 'title' => '1999年/平成11年' ),
			array( 'value' => 2000, 'title' => '2000年/平成12年' ),
			array( 'value' => 2001, 'title' => '2001年/平成13年' ),
			array( 'value' => 2002, 'title' => '2002年/平成14年' ),
			array( 'value' => 2003, 'title' => '2003年/平成15年' ),
			array( 'value' => 2004, 'title' => '2004年/平成16年' ),
			array( 'value' => 2005, 'title' => '2005年/平成17年' ),
			array( 'value' => 2006, 'title' => '2006年/平成18年' ),
			array( 'value' => 2007, 'title' => '2007年/平成19年' ),
			array( 'value' => 2008, 'title' => '2008年/平成20年' ),
			array( 'value' => 2009, 'title' => '2009年/平成21年' ),
			array( 'value' => 2010, 'title' => '2010年/平成22年' ),
			array( 'value' => 2011, 'title' => '2011年/平成23年' ),
			array( 'value' => 2012, 'title' => '2012年/平成24年' )
		);
		return $tbl;
	}

	function get_gengou_array()
	{
        $tbl = array(
            array( 'name' => '明治', 'year' => 1868, 'month' => 1, 'day' => 25 ),
            array( 'name' => '大正', 'year' => 1912, 'month' => 7, 'day' => 30 ),
            array( 'name' => '昭和', 'year' => 1926, 'month' => 12, 'day' => 25 ),
            array( 'name' => '平成', 'year' => 1989, 'month' => 1, 'day' => 8 ),
        );
		return $tbl;
    }

	function get_birth_year_for_smarty( $gengou )
	{
		if( $gengou == null ){ $gengou = $this->get_gengou_array(); }
        $now = time();
        $lt = localtime( $now, true );
        $tbl = array();
        $start = 1925; //$lt['tm_year'] + 1900 - 70;
        $end = $lt['tm_year'] + 1900;
        $gengou_count = count( $gengou );
        if( $gengou_count == 0 ){ return $tbl; }
        $gengou_index = $gengou_count - 1;
        for(;;){
            if( $gengou[$gengou_index]['year'] <= $start ){ break; }
            $gengou_index--;
        }
        for(;;){
			$t = array( 'value' => $start );
            $t['title'] = $start . '年/' . $gengou[$gengou_index]['name'] . ($start-$gengou[$gengou_index]['year']+1) . '年';
            if( $gengou_index < $gengou_count - 1 ){
                if( $gengou[$gengou_index+1]['year'] == $start ){
                    $gengou_index++;
                    $t['title'] .= ( '/' . $gengou[$gengou_index]['name'] . '元年' );
                }
            }
            $tbl[] = $t;
            if( $start == $end ){ break; }
    ...
    // 未来用
	function get_birth_year_for_smarty2( $gengou )
	{
		if( $gengou == null ){ $gengou = $this->get_gengou_array(); }
        $now = time();
        $lt = localtime( $now, true );
        $tbl = array();
        $start = $lt['tm_year'] + 1900;
        $end = $lt['tm_year'] + 1900 + 1;
        $gengou_count = count( $gengou );
        if( $gengou_count == 0 ){ return $tbl; }
        $gengou_index = $gengou_count - 1;
        for(;;){
	...
	function get_birth_year_name( $tbl, $year, $month, $day )
	{
		if( $gengou == null ){ $gengou = $this->get_gengou_array(); }
        $gengou_count = count( $gengou );
        if( $gengou_count == 0 ){ return ''; }
        $gengou_index = $gengou_count - 1;
        for(;;){
            if( $gengou[$gengou_index]['year'] < $year ){ break; }
            if( $gengou_index == 0 ){ return ''; }
            $gengou_index--;
        }
        if( $gengou_index < $gengou_count - 1
            && $gengou[$gengou_index+1]['year'] == $year
            && $gengou[$gengou_index+1]['month'] <= $month
            && ( $day == 0 || $gengou[$gengou_index+1]['day'] <= $day )
        ){
            $gengou_index++;
            $s = $year . '年(' . $gengou[$gengou_index]['name'] . '元年)';
        } else {
            $s = $year . '年(' . $gengou[$gengou_index]['name'] . ($year-$gengou[$gengou_index]['year']+1) . '年)';
        }
        return $s;
	}
	...
	function get_yosen_rank_array()
	{
		$tbl = array(
			...
			array( 'value' => 5, 'title' => '5位' ),
			array( 'value' => 6, 'title' => '6位' )
		);
		return $tbl;
	}
	...
	function get_dan_array()
	{
		$tbl = array(
			array( 'value' => '二段', 'title' => '二段' ),
			array( 'value' => '初段', 'title' => '初段' ),
			array( 'value' => '1級', 'title' => '1級' ),
			array( 'value' => '2級', 'title' => '2級' ),
			array( 'value' => '3級', 'title' => '3級' ),
			array( 'value' => '4級', 'title' => '4級' ),
		);
		return $tbl;
	}

	function get_dan_array_for_smarty( $tbl )
	{
		if( $tbl == null ){ $tbl = $this->get_dan_array(); }
		return $this->get_array_for_smarty( $tbl, '-' );
	}

	function get_dan_name( $tbl, $no )
	{
		if( $tbl == null ){ $tbl = $this->get_dan_array(); }
		return $this->get_name_from_array( $tbl, $no );
	}
	...
	function entry_user_login( $id, $pass, $series, $reg_year )
	{
		$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
	//	$sql = 'select * from `entry_users` where `user_name`=\''.$id.'\' and `user_pass`=SHA1(\'' . $pass . '\') and `series`='.$series;
		$sql = 'select entry_users.* from `entry_users`'
            . ' inner join `entry_info` on `entry_users`.`info`=`entry_info`.`id`'
		    . ' where `entry_users`.`user_name`=\''.$id.'\''
            . ' and `entry_users`.`user_pass`=SHA1(\'' . $pass . '\')'
            . ' and `entry_users`.`series`='.$series
            . ' and `entry_info`.`year`='.$reg_year;
		$list = db_query_list( $dbs, $sql );
//echo $sql;
//print_r($list);
//exit;
...
//print_r($list);
	...
	function get_entry_fields_info( $series, $input )
	{
		$sql = 'select * from `entry_field_def`'
			. ' where `series`='.intval($series).' and `del`=0 order by `field_order` asc';
//			. ' where (`series`=0 or `series`='.intval($series).') and `del`=0 order by `field_order` asc';
//order by `id` asc';
//echo '[',$sql,']';
		$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
		$list = db_query_list( $dbs, $sql );
...
*/
	...
	function get_entry_field_def( $series, $excel=false )
	{
		$sql = 'select * from `entry_field_def`'
			. ' where (`series`=0 or `series`='.intval($series).') and `del`=0';
        if( $excel ){
            $sql .= ' and `excel_out_order`>0 order by `excel_out_order` asc';
        } else {
	...
	function init_entry_post_data_from_def( $data, $def, $add1, $add2, $series )
	{
		foreach( $def as $dv ){
			...
			if( $dv['kind'] == 'address' ){
				...
				$data[$field] = '';
			} else if( $dv['kind'] == 'gakunen_dan_j' || $dv['kind'] == 'gakunen_dan_e' ){
				$data[$field.'_gakunen'] = 0;
				$data[$field.'_dan'] = '';
			} else if( $dv['kind'] == 'photo' ){
				$data[$field] = '';
				$data[$field.'_effect'] = '';
			} else if( $dv['kind'] == 'date' ){
				$data[$field.'_year'] = '';
				$data[$field.'_month'] = '';
				$data[$field.'_day'] = '';
			} else if( $dv['kind'] == 'year_month' || $dv['kind'] == 'year_month2' ){
				$data[$field.'_year'] = '';
				$data[$field.'_month'] = '';
			} else if( $dv['kind'] == 'include' ){
				$func = 'init_entry_post_data_from_def_'.$series.'_'.$dv['field'];
				$func( $data, $dv, $add1, $add2 );
			} else if( $dv['kind'] == 'form_pdf' ){
				$data[$field] = '';
			} else {
				if( $dv['data'] == 'text' ){
					$data[$field] = '';
	...
	function get_entry_post_data_from_def( $def, $add1, $add2 )
	{
		...
		foreach( $def as $dv ){
			...
			if( $dv['kind'] == 'check' ){
                ...
                if( $dv['required'] == 1 && ( $_SESSION['p'][$field] === '' || $_SESSION['p'][$field] === '0' ) ){
                    $_SESSION['e'][$field] = $dv['name'] . 'を入力してください。';
                    $err = 1;
                }
			} else if( $dv['kind'] == 'select' ){
				$_SESSION['p'][$field] = get_field_array( $_POST, $field );
                if( $dv['required'] == 1 && ( $_SESSION['p'][$field] === '' || $_SESSION['p'][$field] === '0' ) ){
                    $_SESSION['e'][$field] = $dv['name'] . 'を入力してください。';
                    $err = 1;
                ...
                if(
                    $dv['required'] == 1
                    && ( $_SESSION['p'][$field.'_sei'] === '' || $_SESSION['p'][$field.'_mei'] === '' )
                ){
                    $_SESSION['e'][$field] = $dv['name'] . 'を入力してください。';
                    $err = 1;
                }
                ...
                if(
                    $dv['required'] == 1
                    && ( $_SESSION['p'][$field.'_tel'] === '' && $_SESSION['p'][$field.'_fax'] === '' )
                ){
                    $_SESSION['e'][$field] = $dv['name'] . 'を入力してください。';
                    $err = 1;
                }
                ...
                if(
                    $dv['required'] == 1
                    && ( $_SESSION['p'][$field.'_mobile'] === '' && $_SESSION['p'][$field.'_tel'] === '' )
                ){
                    $_SESSION['e'][$field] = $dv['name'] . 'を入力してください。';
                    $err = 1;
                }
                ...
                if(
                    $dv['required'] == 1
                    && ( $_SESSION['p'][$field.'_school_name'] === '' && $_SESSION['p'][$field] === '' )
                ){
                    $_SESSION['e'][$field] = $dv['name'] . 'を入力してください。';
                    $err = 1;
                }
                ...
                if(
                    $dv['required'] == 1
                    && ( $_SESSION['p'][$field.'_org'] === '' && $_SESSION['p'][$field] === '' )
                ){
                    $_SESSION['e'][$field] = $dv['name'] . 'を入力してください。';
                    $err = 1;
                }
                ...
                if(
                    $dv['required'] == 1
                    && ( $_SESSION['p'][$field.'_gakunen'] === '' || $_SESSION['p'][$field.'_dan'] === '' )
                ){
                    $_SESSION['e'][$field] = $dv['name'] . 'を入力してください。';
                    $err = 1;
                }
			} else if( $dv['kind'] == 'date' ){
				$_SESSION['p'][$field.'_year'] = get_field_string( $_POST, $field.'_year' );
				$_SESSION['p'][$field.'_month'] = get_field_string( $_POST, $field.'_month' );
				$_SESSION['p'][$field.'_day'] = get_field_string( $_POST, $field.'_day' );
                if(
                    $dv['required'] == 1
                    && ( $_SESSION['p'][$field.'_year'] === ''
                    || $_SESSION['p'][$field.'_month'] === ''
                    || $_SESSION['p'][$field.'_day'] === ''
                    || $_SESSION['p'][$field.'_year'] === '0'
                    || $_SESSION['p'][$field.'_month'] === '0'
                    || $_SESSION['p'][$field.'_day'] === '0' )
                ){
                    $_SESSION['e'][$field] = $dv['name'] . 'を入力してください。';
                    $err = 1;
                }
            ...
            if( $dv['kind'] == 'email' && $dv['required'] == 1 ){
				$email = get_field_string( $_POST, $field );
                if( $email != '' ){
                    foreach( $def as $dv2 ){
                        $field2 = $add1 . $dv2['field'] . $add2;
                        if( $dv2['kind'] == 'email2' ){
                            $email2 = get_field_string( $_POST, $field2 );
                            if( $email != $email2 ){
                                $_SESSION['e'][$dv2['field']] = $dv['name'] . 'の入力内容を確認してください。';
                                $err = 1;
                            }
                            break;
                        }
	...
	function get_entry_db_data_from_def( $data, $list, $def, $add1, $add2, $series )
	{
		foreach( $def as $dv ){
			...
			if( $dv['kind'] == 'check' ){
				...
				$data[$field] = get_field_string( $list, $field );
				$data[$field.'_effect'] = get_field_string( $list, $field.'_effect' );
				$data[$field.'_rotate'] = 0;
				if( $data[$field.'_effect'] != '' ){
					$j = json_decode( $data[$field.'_effect'], true );
					if( isset( $j['crop'] ) && isset( $j['crop']['rotate'] ) ){
						$data[$field.'_rotate'] = $j['crop']['rotate'];
					}
				}
                $force = [
                    'photo' => 1,
                ...
                //$this->get_entry_post_photo_crop_data($data[$field],$data[$field.'_effect'],$force);
			} else if( $dv['kind'] == 'date' ){
				$data[$field.'_year'] = get_field_string( $list, $field.'_year' );
				$data[$field.'_month'] = get_field_string( $list, $field.'_month' );
				$data[$field.'_day'] = get_field_string( $list, $field.'_day' );
			} else if( $dv['kind'] == 'year_month' ){
				$data[$field.'_year'] = get_field_string( $list, $field.'_year' );
				$data[$field.'_month'] = get_field_string( $list, $field.'_month' );
			} else if( $dv['kind'] == 'year_month2' ){
				$data[$field.'_year'] = get_field_string( $list, $field.'_year' );
				$data[$field.'_month'] = get_field_string( $list, $field.'_month' );
			} else if( $dv['kind'] == 'form_pdf' ){
				$data[$field] = get_field_string( $list, $field );
...
*/
	...
	function add_entry_post_data_select_name( $series )
	{
        ...
        $gengou_array = $this->get_gengou_array();
...
//print_r($sel);
			...
			} else if( $lv['kind'] == 'grade_e' ){
				$_SESSION['p'][$lv['field'].'_name'] = $this->get_grade_elementary_name( $grade_elementary_array, $_SESSION['p'][$lv['field']] );
			...
			} else if( $lv['kind'] == 'gakunen_dan_e' ){
				$_SESSION['p'][$lv['field'].'_gakunen_name'] = $this->get_grade_elementary_name( $grade_elementary_array, $_SESSION['p'][$lv['field'].'_gakunen'] );
			} else if( $lv['kind'] == 'yosen_rank' ){
				$_SESSION['p'][$lv['field'].'_name'] = $this->get_yosen_rank_name( $yosen_rank_array, $_SESSION['p'][$lv['field']] );
			} else if( $lv['kind'] == 'date' ){
				$_SESSION['p'][$lv['field'].'_year_name'] = $this->get_birth_year_name( $gengou_array, $_SESSION['p'][$lv['field'].'_year'], $_SESSION['p'][$lv['field'].'_month'], $_SESSION['p'][$lv['field'].'_day'] );
			} else if( $lv['kind'] == 'year_month' ){
				$_SESSION['p'][$lv['field'].'_year_name'] = $this->get_birth_year_name( $gengou_array, $_SESSION['p'][$lv['field'].'_year'], $_SESSION['p'][$lv['field'].'_month'], 0 );
			} else if( $lv['kind'] == 'year_month2' ){
				$_SESSION['p'][$lv['field'].'_year_name'] = $this->get_birth_year_name( $gengou_array, $_SESSION['p'][$lv['field'].'_year'], $_SESSION['p'][$lv['field'].'_month'], 0 );
			}
		}
...
*/
	...
	function get_entry_post_photo_file_data( $field, $id )
	{
	//	ini_set( 'memory_limit', '80M' );
		...
		$name = __UPLOAD_ORG_PATH__ . $fname . '.jpg';
		imagejpeg( $im, $name, 95 );
		chmod( $name, 0666 );
    ...
    function get_entry_post_photo_name( $fname )
    {
        $n = [
            'photo' => __UPLOAD_PATH__ . $fname.'_01.jpg',
            'photo2' => __UPLOAD_PATH__ . $fname.'_02.jpg',
            'trim' => __UPLOAD_PATH__ . 'trim/' . $fname.'_01.jpg',
            'trim_gray' => __UPLOAD_PATH__ . 'trim/' . $fname . '_01_gray.jpg',
            'trim2' => __UPLOAD_PATH__ . 'trim/' . $fname.'_02.jpg',
            'trim2_gray' => __UPLOAD_PATH__ . 'trim/' . $fname . '_02_gray.jpg',
            'brightness' => __UPLOAD_PATH__ . 'pdf/' . $fname.'_01.jpg',
            'brightness_gray' => __UPLOAD_PATH__ . 'pdf/' . $fname . '_01_gray.jpg',
        ];
        return $n;
	}

	function get_entry_post_photo_crop_data( $fname, $effect, $force=null )
	{
		if( $fname == '' ){ return; }
        $dst_name = $this->get_entry_post_photo_name( $fname );
		...
		if( $effect != '' ){
			$p = json_decode( $effect, true );
			if( isset($p['crop']) ){
				if( isset($p['crop']['x']) ){
					$effect_param['crop']['x'] = $p['crop']['x'];
				}
				if( isset($p['crop']['y']) ){
					$effect_param['crop']['y'] = $p['crop']['y'];
				}
				if( isset($p['crop']['width']) ){
					$effect_param['crop']['width'] = $p['crop']['width'];
				}
				if( isset($p['crop']['height']) ){
					$effect_param['crop']['height'] = $p['crop']['height'];
				}
				if( isset($p['crop']['rotate']) ){
					$effect_param['crop']['rotate'] = $p['crop']['rotate'];
				}
            }
			if( isset($p['brightness']) ){
                $brightness = intval( $p['brightness'] );
				if( $brightness == 0 ){ $brightness = 100; }
                $effect_param['brightness'] = intval( ( $brightness - 100 ) * 255 / 100 );
			}
		}
		$degrees = -$effect_param['crop']['rotate'];
		if( $degrees != 0){
			$im = imagerotate($im, $degrees, 0);
		}
		if( $effect_param['crop']['width'] > __THUMB1_WIDTH__ ){
			$n_width = __THUMB1_WIDTH__;
			$n_height = (int)( $effect_param['crop']['height'] * __THUMB1_WIDTH__ / $effect_param['crop']['width'] );
		} else {
			$n_width = $effect_param['crop']['width'];
			$n_height = $effect_param['crop']['height'];
		}
		$n_im = imagecreatetruecolor( $n_width, $n_height );
        if( is_null($force) || $force['photo'] != 0 ){
            if( $force['photo'] == 1 || !file_exists($dst_name['photo']) ){
                imagecopyresampled(
                    $n_im, $im,
                    0, 0,
                    0, 0,
                ...
                imagejpeg( $n_im, $dst_name['photo'] );
                chmod( $dst_name['photo'], 0666 );
            }
        }
		imagecopyresampled(
			$n_im, $im,
			0, 0,
			$effect_param['crop']['x'], $effect_param['crop']['y'],
			$n_width, $n_height,
			$effect_param['crop']['width'], $effect_param['crop']['height']
        );
        if( is_null($force) || $force['trim'] != 0 ){
            if( $force['trim'] == 1 || !file_exists($dst_name['trim']) ){
                imagejpeg( $n_im, $dst_name['trim'] );
                chmod( $dst_name['trim'], 0666 );
            }
        }
        /*
		if( is_null($force) || $force['trim_gray'] != 0 ){
            if( $force['trim_gray'] == 1 || !file_exists($dst_name['trim_gray']) ){
                imagefilter( $n_im, IMG_FILTER_GRAYSCALE );
                imagejpeg( $n_im, $dst_name['trim_gray'] );
                chmod( $dst_name['trim_gray'], 0666 );
            }
        }
        */
		if( is_null($force) || $force['brightness'] != 0 ){
            if( $force['brightness'] == 1 || !file_exists($dst_name['brightness']) ){
                imagecopyresampled(
                    $n_im, $im,
                    0, 0,
                    $effect_param['crop']['x'], $effect_param['crop']['y'],
                ...
                imagefilter( $n_im, IMG_FILTER_BRIGHTNESS, $effect_param['brightness'] );
                imagejpeg( $n_im, $dst_name['brightness'] );
                chmod( $dst_name['brightness'], 0666 );
            }
		}
        /*
		if( is_null($force) || $force['brightness_gray'] != 0 ){
            if( $force['brightness_gray'] == 1 || !file_exists($dst_name['brightness_gray']) ){
                imagecopyresampled(
                    $n_im, $im,
                    0, 0,
                    $effect_param['crop']['x'], $effect_param['crop']['y'],
                ...
                imagefilter( $n_im, IMG_FILTER_BRIGHTNESS, $effect_param['brightness_gray'] );
                imagefilter( $n_im, IMG_FILTER_GRAYSCALE );
                imagejpeg( $n_im, $dst_name['brightness_gray'] );
                chmod( $dst_name['brightness_gray'], 0666 );
            }
        }
        */

		if( $effect_param['crop']['width'] > __THUMB2_WIDTH__ ){
			$n_width = __THUMB2_WIDTH__;
			$n_height = (int)( $effect_param['crop']['height'] * __THUMB2_WIDTH__ / $effect_param['crop']['width'] );
		} else {
			$n_width = $effect_param['crop']['width'];
			$n_height = $effect_param['crop']['height'];
		}
		$n_im = imagecreatetruecolor( $n_width, $n_height );
        if( is_null($force) || $force['photo2'] != 0 ){
            if( $force['photo2'] == 1 || !file_exists($dst_name['photo2']) ){
                imagecopyresampled(
                    $n_im, $im,
                    0, 0,
                    0, 0,
                ...
                imagejpeg( $n_im, $dst_name['photo2'] );
                chmod( $dst_name['photo2'], 0666 );
            }
        }
		imagecopyresampled(
			$n_im, $im,
		...
		if( is_null($force) || $force['trim2'] != 0 ){
            if( $force['trim2'] == 1 || !file_exists($dst_name['trim2']) ){
                imagejpeg( $n_im, $dst_name['trim2'] );
                chmod( $dst_name['trim2'], 0666 );
            }
        }
        /*
        if( is_null($force) || $force['trim2_gray'] != 0 ){
            if( $force['trim2_gray'] == 1 || !file_exists($dst_name['trim2_gray']) ){
                imagefilter( $n_im, IMG_FILTER_GRAYSCALE );
                imagejpeg( $n_im, $dst_name['trim2_gray'] );
                chmod( $dst_name['trim2_gray'], 0666 );
            }
        }
        */
	}
...
//echo $sql,"<br />\n";
	...
	function make_entry_sql_data_from_def( $sql, $def, $add1, $add2, $dbs )
	{
		foreach( $def as $dv ){
			if( $dv['def'] == 'text' ){
				$sql .= ( ' `' . $dv['field'] . "`='" . $dbs->real_escape_string( get_field_string( $_SESSION['p'], $add1.$dv['field'].$add2 ) ) . "'," );
...
//print_r($_SESSION['p'][$lv['field']]);
				...
				$datalist = array(
					$dv['field'] => $dbs->real_escape_string( get_field_string( $_SESSION['p'], $dv['field'] ) ),
					$dv['field'].'_effect' => $dbs->real_escape_string( get_field_string( $_SESSION['p'], $dv['field'].'_effect' ) )
				);
			} else if( $dv['kind'] == 'date' ){
				$datalist = array(
					$dv['field'].'_year' => $dbs->real_escape_string( get_field_string( $_SESSION['p'], $dv['field'].'_year' ) ),
					$dv['field'].'_month' => $dbs->real_escape_string( get_field_string( $_SESSION['p'], $dv['field'].'_month' ) ),
					$dv['field'].'_day' => $dbs->real_escape_string( get_field_string( $_SESSION['p'], $dv['field'].'_day' ) )
				);
			} else if( $dv['kind'] == 'year_month' || $dv['kind'] == 'year_month2' ){
				$datalist = array(
					$dv['field'].'_year' => $dbs->real_escape_string( get_field_string( $_SESSION['p'], $dv['field'].'_year' ) ),
...
//print_r($list );
		...
		foreach( $list as &$lv ){
			...
			if( isset( $lv['school_pref'] ) ){
				$lv['school_address_pref_name'] = $this->get_pref_name( $preftbl, intval($lv['school_pref']) );
			}
		}
...
echo "-->\n";
	...
	function output_entry_data_list_excel2( $sheet )
	{
		...
		foreach( $list as $lv ){
            ...
            $sheet->getCellByColumnAndRow( 7, $pos )->setValueExplicit( $fields['school_phone_tel'], PHPExcel_Cell_DataType::TYPE_STRING );
            $sheet->getCellByColumnAndRow( 8, $pos )->setValueExplicit( $fields['school_phone_fax'], PHPExcel_Cell_DataType::TYPE_STRING );
            ...
            $sheet->getCellByColumnAndRow( 11, $pos )->setValueExplicit( $fields['responsible_contact_mobile'], PHPExcel_Cell_DataType::TYPE_STRING );
	...
	function output_entry_data_list_excel3( $sheet )
	{
		...
		foreach( $list as $lv ){
            ...
            $sheet->getCellByColumnAndRow( $col++, $pos )->setValueExplicit( $fields['responsible_contact_tel'], PHPExcel_Cell_DataType::TYPE_STRING );
...
/**/
		if( $lt > 0 ){
			...
			if( count( $list ) > 0 ){
				foreach( $list as $lv ){
	    			$next_match = get_field_string_number( $lv, 'match', 0 );
		    		if( $next_match == 0 ){ continue; }
		    		$next_data = db_get_one_data( $dbs, 'dantai_match', '*', '`id`='.$next_match );
			    	$winner = get_field_string_number( $next_data, 'winner', 0 );
					if( $winner != 0 ){ continue; }
			    	$team1 = get_field_string_number( $next_data, 'team1', 0 );
			    	$team2 = get_field_string_number( $next_data, 'team2', 0 );
			    	$next_one_match_id = get_field_string_number( $next_data, 'match'.$match_no, 0 );
				    if( $next_one_match_id == 0 ){ continue; }
					if( $team1 == $team_id ){
...
//echo $sql,"<br />\n";
			...
			if( $league_match_index == 1 ){
                ...
                if( $team == 1 ){
                    ...
                    $next_matches[] = array( 'index' => 6, 'team' => 2 );
                }
            } else if( $league_match_index == 4 ){
                if( $team == 1 ){
                    $next_matches[] = array( 'index' => 5, 'team' => 1 );
                } else if( $team == 2 ){
                    $next_matches[] = array( 'index' => 6, 'team' => 1 );
                }
...
//echo $sql;
		...
		$sql = 'select * from `one_match_player_info`'
            . ' where `del`=0 and `navi_id`=' . $navi_id
            . ' and `match_id`=' . $one_match_id
            . ' and `team`=' . $team_id
            . ' and `match_no`=' . $match_no;
//            . ' and `player`=' . $player
//            . ( $player == 1000 ? ' `and player_change_name` like \'' . $name . '\'' : '' );
		$list = db_query_list( $dbs, $sql );
		if( count( $list ) > 0 ){
    	    $sql = 'update `one_match_player_info`'
				. ' set `player`=' . $player . ','
				. ' `player_change_name`=\'' . $name . '\''
				. ' where `id`=' . $list[0]['id'];
		} else {
            $sql = 'insert into `one_match_player_info` set `supporter`=0,'
                . ' `del`=0, `created`=NOW(), `navi_id`=' . $navi_id . ','
                . ' `match_id`=' . $one_match_id . ','
				. ' `team`=' . $team_id . ','
				. ' `match_no`=' . $match_no . ','
                . ' `player`=' . $player . ','
				. ' `player_change_name`=\'' . $name . '\'';
...
//echo $sql,"<br />\n";
	...
	function update_dantai_league_place_navi_data( $series, $series_info, $navi_id, $dbs )
	{
		$sql = 'select `dantai_league_match`.`id` as `id`,'
			...
			. ' `dantai_league`.`display_place_offset` as `display_place_offset`,'
			. ' `dantai_match`.`place_match_no` as `place_match_no`,'
			. ' `dantai_match`.`place_match_no_disp` as `place_match_no_disp`,'
			. ' `dantai_match`.`place` as `place`,'
			. ' `dantai_league_match`.`league_match_index` as `match_index`,'
			...
			. ' inner join `dantai_match` on `dantai_league_match`.`match`=`dantai_match`.`id`'
			. ' where `dantai_league`.`series`='.$series.' and `dantai_league`.`year`='.$_SESSION['auth']['year']
            . ' and `dantai_league_match`.`no_input`=0'
			. ' order by `dantai_match`.`place_match_no` asc';
		$list = db_query_list( $dbs, $sql );
//echo $sql;
//print_r($list);echo "<br />\n";
		foreach( $list as $lv ){
            ...
            if( $match_index <= $match_num ){
    			$lv['match_name'] = $series_info['dantai_'.$lv['series_mw'].'_name'];
				if( $lv['match_name'] == '' ){
					if( $lv['series_mw'] == 'm' ){
						$lv['match_name'] = '男子';
				...
				$lv['match_name'] .= '団体リーグ';
		    	$lv['series_lt'] = 'dl';
				$lv['place_name'] = '第' . ($lv['place']+$lv['display_place_offset']) . '試合場';
				$lv['place_match_no_name'] = '第'.$lv['place_match_no'].'試合';
				$lv['place_match_no_name_disp'] = '第'.$lv['place_match_no_disp'].'試合';
	...
	function update_dantai_tournament_place_navi_data( $series, $series_info, $navi_id, $dbs )
	{
		$sql = 'select `dantai_tournament_match`.`id` as `id`,'
			...
			. ' `dantai_tournament`.`match_num` as `match_num`,'
			. ' `dantai_tournament`.`extra_match_num` as `extra_match_num`,'
			. ' `dantai_tournament`.`display_place_offset` as `display_place_offset`,'
			. ' `dantai_match`.`place_match_no` as `place_match_no`,'
			. ' `dantai_match`.`place_match_no_disp` as `place_match_no_disp`,'
			. ' `dantai_match`.`place` as `place`,'
			. ' `dantai_tournament_match`.`match` as `match`,'
			...
			. ' where `dantai_tournament`.`series`='.$series
			. ' and `dantai_tournament`.`year`='.$_SESSION['auth']['year']
            . ' and `dantai_tournament_match`.`no_input`=0'
			. ' order by `dantai_match`.`place_match_no` asc';
		$list = db_query_list( $dbs, $sql );
...
//print_r($lv);echo "<br />\n";
	...
	function update_kojin_tournament_place_navi_data( $series, $series_info, $navi_id, $dbs )
	{
		$sql = 'select `kojin_tournament_match`.`id` as `id`,'
			...
			. ' `kojin_tournament`.`display_place_offset` as `display_place_offset`,'
			. ' `kojin_match`.`place_match_no` as `place_match_no`,'
			. ' `kojin_match`.`place_match_no_disp` as `place_match_no_disp`,'
            . ' `kojin_match`.`place` as `place`,'
			. ' `kojin_tournament_match`.`match` as `match`,'
			...
			. ' inner join `kojin_tournament_match` on `kojin_tournament_match`.`tournament`=`kojin_tournament`.`id`'
			. ' inner join `kojin_match` on `kojin_tournament_match`.`match`=`kojin_match`.`id`'
			. ' where `kojin_tournament`.`series`='.$series
            . ' and `kojin_tournament`.`year`='.$_SESSION['auth']['year']
            . ' and `kojin_tournament_match`.`no_input`=0'
		...
		foreach( $list as $lv ){
			...
			$lv['match_name'] .= '個人トーナメント';
			$lv['series_lt'] = 'kt';
			$lv['place_name'] = '第' . (intval($lv['place'])+intval($lv['display_place_offset'])) . '試合場';
			$lv['place_match_no_name'] = '第'.$lv['place_match_no'].'試合';
			$lv['place_match_no_name_disp'] = '第'.$lv['place_match_no_disp'].'試合';
            $lv['year'] = $_SESSION['auth']['year'];
            $lv['series_info_id'] = $series_info['series_info_id'];
	...
	function update_series_place_navi_data( $navi_id )
    {
		...
		$sql = 'select `series`.`dantai_league_m` as `dantai_league_m`,'
            ...
            . '`series`.`dantai_m_name` as `dantai_m_name`,'
            . '`series`.`dantai_w_name` as `dantai_w_name`,'
            . '`series`.`kojin_m_name` as `kojin_m_name`,'
            . '`series`.`kojin_w_name` as `kojin_w_name`,'
...
//echo $sql;
	...
	function get_series_place_navi_data( $navi_id, $place, $place_match_no )
    {
		...
		$sql = 'select * from `navi_info`'
            ...
            . ' order by `place_match_no` asc';
		$nlist = db_query_list( $dbs, $sql );
        if( count( $nlist ) == 0 ){ return []; }
	...
	function get_series_place_next_navi_data( $navi_id, $place, $place_match_no )
    {
		$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
		$sql = 'select * from `navi_info`'
            . ' where `del`=0 and `navi_id`=' . $navi_id
	...
	function get_series_place_prev_navi_data( $navi_id, $place, $place_match_no )
    {
		$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
		$sql = 'select * from `navi_info`'
            . ' where `del`=0 and `navi_id`=' . $navi_id
            . ' and `place_no`=' . $place . ' and `place_match_no`<=' . $place_match_no
            . ' order by `place_match_no` desc';
		$nlist = db_query_list( $dbs, $sql );
        if( count( $nlist ) == 0 ){ return array(); }
        if( $nlist[0]['series_lt'] === 'dl' || $nlist[0]['series_lt'] === 'dt' ){
            if( $place_match_no >= 1000 ){
	...
	function get_series_place_navi_max_match_no( $navi_id, $place )
    {
		$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
		$sql = 'select * from `navi_info`'
            . ' where `del`=0 and `navi_id`=' . $navi_id . ' and `place_no`=' . $place
            . ' and `place_match_no`<900 order by `place_match_no` desc';
		$nlist = db_query_list( $dbs, $sql );
        if( count( $nlist ) == 0 ){ return 0; }
        return intval( $nlist[0]['place_match_no'] );
    }
...
//print_r($nlist);
	...
	function get_series_place_top_navi_data( $navi_id, $place, $mode, $mw )
    {
		$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
		$sql = 'select * from `navi_info`'
            . ' where `del`=0 and `navi_id`=' . $navi_id
            . ' and `place_no`=' . $place . ' order by `place_match_no` asc';
		$nlist = db_query_list( $dbs, $sql );
        if( count( $nlist ) == 0 ){ return array(); }
        foreach( $nlist as $nv ){
            if( $nv['series_lt'] === 'dl' ){
                ...
                if( $mode == 'dt' ){
                    ...
                    if( $mw == $nv['series_mw'] && $nv['match_index'] < 8 ){
                        $nv['script'] = 'dantai_result.php';
                        return $nv;
                    }
                }
            } else if( $nv['series_lt'] === 'dtsub' ){
                if( $mode == 'dtsub' ){
                    if( $mw == $nv['series_mw'] ){
                        $nv['script'] = 'dantai_result.php';
                        return $nv;
                ...
                if( $mode == 'kt' ){
                    ...
                    if( $mw == $nv['series_mw'] && $nv['match_index'] < 16 ){
                        $nv['script'] = 'kojin_result.php';
                        return $nv;
                    }
                }
...
echo $sql,"<br />\n";
	...
	function get_dantai_league_one_result( $match )
	{
		...
		$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
		$sql = 'select `dantai_league_match`.*,'
			. ' `dantai_league`.`name` as `match_name`,'
			. ' `dantai_league`.`series` as `series`,'
			. ' `dantai_league`.`series_mw` as `series_mw`'
			. ' from `dantai_league_match` inner join `dantai_league` on `dantai_league`.`id`=`dantai_league_match`.`league` where `dantai_league_match`.`match`='.$match;
		$dantai_league_match = db_query_list( $dbs, $sql );
		...
		$dantai_match['series'] = $series;
		$dantai_match['series_mw'] = $series_mw;
		$dantai_match['match_name'] = $dantai_league_match[0]['match_name'];
		$dantai_match['entry1'] = $this->get_entry_one_data2( intval($dantai_match['team1']) );
		$dantai_match['entry2'] = $this->get_entry_one_data2( intval($dantai_match['team2']) );
...
//print_r($list);
	...
	function update_dantai_tournament_one_data( $series, $navi_id, $part, $mw, $post, $data, $dbs )
	{
...
/**/
			if( $match_index >= $macth1_level ){
				...
				$team1 = get_field_string_number( $dantai_match[0], 'team1', 0 );
				$team2 = get_field_string_number( $dantai_match[0], 'team2', 0 );
//echo print_r($dantai_match,TRUE),"<br />\n";
				if( $team1 != 0 && $team2 != 0 ){
					$sql = 'select * from `one_match_player_info` where `navi_id`=' . $navi_id . ' and `team`=' . $team1;
					$dantai_match_team = db_query_list( $dbs, $sql );
					if( count($dantai_match_team) > 0 ){
						foreach( $dantai_match_team as $mv ){
							$one_match_id = get_field_string_number( $dantai_match[0], 'match'.$mv['match_no'], 0 );
							$sql = 'update `one_match` set '
								. (is_null($mv['player']) ? '' : ('`player1`=' . $mv['player'] . ','))
								. (is_null($mv['player_change_name']) ? '' : (" `player1_change_name`='" . $mv['player_change_name'] . "',"))
								. (is_null($mv['supporter']) ? '' : (" `supporter1`='" . $mv['supporter'] . "',"))
								. ' `update_date`=NOW() where `id`='.$one_match_id;
//echo 'set:',$sql,"<br />\n";
							db_query( $dbs, $sql );
						}
					}
					$sql = 'select * from `one_match_player_info` where `navi_id`=' . $navi_id . ' and `team`=' . $team2;
					$dantai_match_team = db_query_list( $dbs, $sql );
					if( count($dantai_match_team) > 0 ){
						foreach( $dantai_match_team as $mv ){
							$one_match_id = get_field_string_number( $dantai_match[0], 'match'.$mv['match_no'], 0 );
							$sql = 'update `one_match` set '
								. (is_null($mv['player']) ? '' : ('`player2`=' . $mv['player'] . ','))
								. (is_null($mv['player_change_name']) ? '' : (" `player2_change_name`='" . $mv['player_change_name'] . "',"))
								. (is_null($mv['supporter']) ? '' : (" `supporter2`='" . $mv['supporter'] . "',"))
								. ' `update_date`=NOW() where `id`='.$one_match_id;
//echo 'set:',$sql,"<br />\n";
							db_query( $dbs, $sql );
						}
...
//print_r($data);
	...
	function update_dantai_tournament_data( $series, $mw, $navi_id, $post )
	{
		global $__dantai_league_team_last_match__;
        ...
        $part = 1;
		foreach( $list as $data ){
            $this->update_dantai_tournament_one_data( $series, $navi_id, $part, $mw, $post, $data, $dbs );
            $part++;
        }
		db_close( $dbs );
    }
...
//print_r($dantai_match);
	...
	function get_dantai_tournament_one_result( $match )
	{
		...
		$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
		$sql = 'select `dantai_tournament_match`.*,'
			. ' `dantai_tournament`.`team_num` as `team_num`,'
			. ' `dantai_tournament`.`series` as `series`,'
			. ' `dantai_tournament`.`series_mw` as `series_mw`'
			. ' from `dantai_tournament_match` inner join `dantai_tournament` on `dantai_tournament`.`id`=`dantai_tournament_match`.`tournament` where `dantai_tournament_match`.`match`='.$match;
		$dantai_tournament_match = db_query_list( $dbs, $sql );
		...
		$dantai_match['tournament'] = $tournament;
		$name = '決勝トーナメント ';
		if( $tournament_match_index == 1 ){
			$name .= '決勝';
		} else if( $tournament_match_index < 4 ){
			$name .= ('準決勝 第'.($tournament_match_index-1).'試合');
		} else if( $tournament_match_index < 8 ){
			$name .= ('準々決勝 第'.($tournament_match_index-3).'試合');
		} else {
			$name .= ('1回戦 第'.($tournament_match_index-7).'試合');
		}
		$dantai_match['match_name'] = $name;
		...
		for( $i1 = 1; $i1 <= 6; $i1++ ){
			...
			if( $match_id != 0 ){
                ...
                if( $one_match['player1'] == __PLAYER_NAME__ ){
					...
					if( $dantai_match['team1'] == 0 || $one_match['player1'] == 0 ){
	                    $one_match['player1_name'] = '';
					} else {
	                    $one_match['player1_name'] = $dantai_match['entry1']['player'.$one_match['player1'].'_sei'] . ' '. $dantai_match['entry1']['player'.$one_match['player1'].'_mei'];
					}
                }
				if( $one_match['player2'] == 0 && $i1 < 6 ){
					$one_match['player2'] = $i1;
                ...
                if( $one_match['player2'] == __PLAYER_NAME__ ){
					...
					if( $dantai_match['team2'] == 0 || $one_match['player2'] == 0 ){
	                    $one_match['player2_name'] = '';
					} else {
    	                ...
    	                $one_match['player2_name'] = $dantai_match['entry2']['player'.$one_match['player2'].'_sei'] . ' '. $dantai_match['entry2']['player'.$one_match['player2'].'_mei'];
					}
                }
			} else {
				if( $i1 < 6 ){
...
echo $sql,"<br />";
	...
	function get_kojin_tournament_one_result( $series, $series_mw, $id )
	{
		...
		$data['referee2_name'] = '';
		$data['referee3_name'] = '';
		$sql = 'select * from `referee` where `series_info_id`=' . $series_info['id'] . ' and `year`=' . $series_info['reg_year'];
		$referee_list = db_query_list( $dbs, $sql );
        foreach( $referee_list as $rv ){
            if( $rv['id'] == $data['referee1'] ){
                $data['referee1_name'] = $rv['sei'] . $rv['mei'];
            }
            if( $rv['id'] == $data['referee2'] ){
                $data['referee2_name'] = $rv['sei'] . $rv['mei'];
            }
            if( $rv['id'] == $data['referee3'] ){
                $data['referee3_name'] = $rv['sei'] . $rv['mei'];
            }
        }
...
//exit;
        ...
        $place_match_player_no = $this->get_navi_current_input_match_player_no_of_match( $series_info, $data['match'] );
		$team1_index = -1;
		$team2_index = -1;
...
//		$html .= '    <td class="td_name">学校名</td>'."\n";
		$html .= '    <td class="td_name"></td>'."\n";
		$html .= '    <td colspan="4" class="td_match';
        if( $place_match_player_no == 1 ){
            $html .= ' tb_frame_current_match';
        }
		$html .= '">先鋒</td>'."\n";
		$html .= '    <td colspan="4" class="td_match';
        if( $place_match_player_no == 2 ){
            $html .= ' tb_frame_current_match';
        }
		$html .= '">次鋒</td>'."\n";
		$html .= '    <td colspan="4" class="td_match';
        if( $place_match_player_no == 3 ){
            $html .= ' tb_frame_current_match';
        }
		$html .= '">中堅</td>'."\n";
		$html .= '    <td colspan="4" class="td_match';
        if( $place_match_player_no == 4 ){
            $html .= ' tb_frame_current_match';
        }
		$html .= '">副将</td>'."\n";
		$html .= '    <td colspan="4" class="td_match';
        if( $place_match_player_no == 5 ){
            $html .= ' tb_frame_current_match';
        }
		$html .= '">大将</td>'."\n";
		$html .= '    <td class="td_score">対戦結果</td>'."\n";
		$html .= '    <td colspan="3" class="td_match';
		...
		$school_name = '';
        if( $team1_index >= 0 ){
            if( isset( $entry_list[$team1_index]['school_name_ryaku'] ) ){
                $school_name = $entry_list[$team1_index]['school_name_ryaku'];
            } else if( isset( $entry_list[$team1_index]['school_name'] ) ){
                $school_name = $entry_list[$team1_index]['school_name'];
            }
            if( isset( $entry_list[$team1_index]['school_address_pref_name'] ) ){
//                $html .= ( '<br />(' . $entry_list[$team1_index]['school_address_pref_name'] . ')' );
            }
        }
		$html .= '    <td rowspan="2" class="tbnamecolor"'
			.  (mb_strlen($school_name)>12 ? ' style="font-size:12px;"': '')
			. '>' . $school_name . '</td>'."\n";
		for( $i1 = 1; $i1 <= 5; $i1++ ){
			$html .= '    <td colspan="4" class="tbname01">';
                    ...
                    if( $series_mw === '' ){
                        ...
                        if( $series_info['player_field_mode'] == 3 ){
            				$f = 'dantai_'.$series_mw.$data['matches'][$i1]['player1'];
         		            ...
         		            $f = sprintf( $series_info['dantai_'.$series_mw.'_field_head'], $data['matches'][$i1]['player1'] );
                ...
                if( $series_mw === '' ){
                    ...
                    if( $series_info['player_field_mode'] == 3 ){
    			        $f = 'dantai_'.$series_mw.$data['matches'][6]['player1'];
      		            ...
      		            $f = sprintf( $series_info['dantai_'.$series_mw.'_field_head'], $data['matches'][6]['player1'] );
		...
		for( $i1 = 1; $i1 <= 5; $i1++ ){
			for( $i2 = 1; $i2 <= 3; $i2++ ){
				...
				if( $data['matches'][$i1]['waza1_'.$i2] == 0 ){
					...
					$html .= '〇';
				} else if( $data['matches'][$i1]['waza1_'.$i2] == 6 ){
					$html .= 'ツ';
				}
				$html .= '</td>'."\n";
			}
            $html .= '    <td rowspan="2" class="td_waza_ippon">';
            if( $data['matches'][$i1]['end_match'] == 1 ){
                $fusen = false;
                for( $i2 = 1; $i2 <= 3; $i2++ ){
		...
		for( $i2 = 1; $i2 <= 3; $i2++ ){
			...
			if( $data['matches'][6]['waza1_'.$i2] == 0 ){
				...
				$html .= '〇';
			} else if( $data['matches'][6]['waza1_'.$i2] == 6 ){
				$html .= 'ツ';
			}
			$html .= '</td>'."\n";
		}
		...
		$school_name = '';
        if( $team2_index >= 0 ){
            if( isset( $entry_list[$team2_index]['school_name_ryaku'] ) ){
                $school_name = $entry_list[$team2_index]['school_name_ryaku'];
            } else if( isset( $entry_list[$team2_index]['school_name'] ) ){
                $school_name = $entry_list[$team2_index]['school_name'];
            }
            if( isset( $entry_list[$team2_index]['school_address_pref_name'] ) ){
//                $html .= ( '<br />(' . $entry_list[$team1_index]['school_address_pref_name'] . ')' );
            }
        }
		$html .= '    <td rowspan="2" class="tbnamecolor"'
		...
		for( $i1 = 1; $i1 <= 5; $i1++ ){
			for( $i2 = 1; $i2 <= 3; $i2++ ){
				...
				if( $data['matches'][$i1]['waza2_'.$i2] == 0 ){
					...
					$html .= '〇';
				} else if( $data['matches'][$i1]['waza2_'.$i2] == 6 ){
					$html .= 'ツ';
				}
				$html .= '</td>'."\n";
			}
		...
		for( $i2 = 1; $i2 <= 3; $i2++ ){
			...
			if( $data['matches'][6]['waza2_'.$i2] == 0 ){
				...
				$html .= '〇';
			} else if( $data['matches'][6]['waza2_'.$i2] == 6 ){
				$html .= 'ツ';
			}
			$html .= '</td>'."\n";
		}
                    ...
                    if( $series_mw === '' ){
                        ...
                        if( $series_info['player_field_mode'] == 3 ){
    	    			    $f = 'dantai_'.$series_mw.$data['matches'][$i1]['player2'];
         		            ...
         		            $f = sprintf( $series_info['dantai_'.$series_mw.'_field_head'], $data['matches'][$i1]['player2'] );
                ...
                if( $series_mw === '' ){
                    ...
                    if( $series_info['player_field_mode'] == 3 ){
        			    $f = 'dantai_'.$series_mw.$data['matches'][6]['player2'];
         		        ...
         		        $f = sprintf( $series_info['dantai_'.$series_mw.'_field_head'], $data['matches'][6]['player2'] );
...
/*
		...
		for( $i1 = 1; $i1 <= 5; $i1++ ){
			$html .= '    <td colspan="4" class="tbname01">';
			$html .= $data['matches'][$i1]['match_time'];
			$html .= '    </td>'."\n";
        }
        $html .= '    <td>&nbsp;</td>'."\n";
		$html .= '    <td colspan="3" class="tbname01">';
		$html .= $data['matches'][6]['match_time'];
		$html .= '    </td>'."\n";
		$html .= '  </tr>'."\n";
*/
		$html .= '</table>'."\n";
    ...
    function output_one_match_for_excel( $sheet, $col, $row, $series_info, $data, $entry_list, $series_mw, $result_width, $result_hight )
    {
        ...
        $result_x = -4; //-intval( $result_width - $result_size ) / 3;   //=8
        $result_y = 5;
            ...
            if( $data['matches'][$i1]['end_match'] == 1 ){
                ...
                if( !$fusen ){
                    ...
                    for( $i2 = 1; $i2 <= 3; $i2++ ){
                     ...
                     if( ( $hon1 == 1 && $hon2 == 0 ) || ( $hon1 == 0 && $hon2 == 1 ) ){
                        if( $data['matches'][$i1]['extra'] == 0 ){
                            $sheet->setCellValueByColumnAndRow( $col+$i1*4, $row+1, '一本勝' );
                        }
                    } else if( $hon1 == $hon2 ){
                        $sheet->setCellValueByColumnAndRow( $col+$i1*4, $row+1, '引き分け' );
                    }
                }
            }
            $sheet->setCellValueByColumnAndRow( $col+$i1*4-3, $row+4, $data['matches'][$i1]['match_time'] );
		}
		$sheet->setCellValueByColumnAndRow( $col+25, $row+4, $data['matches'][6]['match_time'] );
        ...
        if( $team1_index >= 0 ){
	    	...
	    	for( $i1 = 1; $i1 <= 5; $i1++ ){
                ...
                $sheet->setCellValueByColumnAndRow( $col+$i1*4-3, $row, $player );
    		}
    		if( $end_match == 5 || $data['fusen'] == 1 ){
                if( $data['fusen'] == 0 ){
					if( $data['winner'] == 1 ){
    	                ...
    	                $objDrawing->setCoordinates( $this->get_excel_coordinates($col+22,$row) );
                ...
                if( $data['fusen'] == 1 ){
                    $sheet->setCellValueByColumnAndRow( $col+22, $row, '' );
                    $sheet->setCellValueByColumnAndRow( $col+22, $row+1, '' );
                } else {
                    $sheet->setCellValueByColumnAndRow( $col+22, $row, $data['hon1'] );
                    $sheet->setCellValueByColumnAndRow( $col+22, $row+1, $data['win1'] );
                }
    		} else {
                $sheet->setCellValueByColumnAndRow( $col+22, $row, '' );
                $sheet->setCellValueByColumnAndRow( $col+22, $row+1, '' );
    		...
    		$sheet->setCellValueByColumnAndRow( $col+25, $row, $player );
    		...
    		for( $i1 = 1; $i1 <= 5; $i1++ ){
                for( $i2 = 1; $i2 <= 3; $i2++ ){
                    ...
                    $sheet->setCellValueByColumnAndRow( $col+($i1-1)*4+$i2, $row+1, $waza );
                }
    		}
    		for( $i2 = 1; $i2 <= 3; $i2++ ){
                $waza = '';
                ...
                $sheet->setCellValueByColumnAndRow( $col+$i2+24, $row+1, $waza );
    		}
        }
        if( $team2_index >= 0 ){
	    	...
	    	for( $i1 = 1; $i1 <= 5; $i1++ ){
                for( $i2 = 1; $i2 <= 3; $i2++ ){
                    ...
                    $sheet->setCellValueByColumnAndRow( $col+($i1-1)*4+$i2, $row+2, $waza );
                }
    		}
	    	for( $i2 = 1; $i2 <= 3; $i2++ ){
                $waza = '';
                ...
                $sheet->setCellValueByColumnAndRow( $col+$i2+24, $row+2, $waza );
		    }
    		for( $i1 = 1; $i1 <= 5; $i1++ ){
                $player = '';
                if( $data['matches'][$i1]['player2'] == 0 ){
                ...
                $sheet->setCellValueByColumnAndRow( $col+$i1*4-3, $row+3, $player );
    		}
	    	if( $end_match == 5 || $data['fusen'] == 1 ){
                if( $data['fusen'] == 0 ){
					if( $data['winner'] == 2 ){
                    	...
                    	$objDrawing->setHeight($result_size);
	                    $objDrawing->setWorksheet($sheet);
    	                $objDrawing->setCoordinates( $this->get_excel_coordinates($col+22,$row+2) );
        	            $objDrawing->setOffsetX($result_x);
            	        $objDrawing->setOffsetY($result_y);
                	} else if( $data['winner']==1 ){
                    	$objDrawing = new PHPExcel_Worksheet_Drawing();
                ...
                if( $data['fusen'] == 1 ){
                    $sheet->setCellValueByColumnAndRow( $col+22, $row+2, '' );
                    $sheet->setCellValueByColumnAndRow( $col+22, $row+3, '' );
                } else {
                    $sheet->setCellValueByColumnAndRow( $col+22, $row+2, $data['hon2'] );
                    $sheet->setCellValueByColumnAndRow( $col+22, $row+3, $data['win2'] );
                }
		    } else {
                $sheet->setCellValueByColumnAndRow( $col+22, $row+2, '' );
                $sheet->setCellValueByColumnAndRow( $col+22, $row+3, '' );
    		...
    		$sheet->setCellValueByColumnAndRow( $col+25, $row+3, $player );
        }
	}
...
//echo $sql,"<br />\n";
        db_query( $dbs, $sql );
        $sql = 'select `entry_info`.`id` as `id`, `entry_field`.`data` as `school_name` from `entry_info`'
            . ' inner join `entry_field` on  `entry_field`.`info`=`entry_info`.`id` and `entry_field`.`field`=\'school_name\''
            . ' where `entry_info`.`series`=' . $series
                . ' and `entry_info`.`year`=' . $_SESSION['auth']['year']
            . ' order by `entry_info`.`id` asc';
        $entry_list = db_query_list( $dbs, $sql );
		$entry_list_num = count( $entry_list );
		$entry_index = 0;
//echo $sql,"<br />\n";
//print_r($entry_list);
...
//print_r($line);
			...
			$set_id = 0;
			if( $entry_list_num > 0 ){
				for( $i1 = 0; $i1 < $entry_list_num; $i1++ ){
                    if( $entry_list[$i1]['school_name'] == $line[0] ){
                        $set_id = $entry_list[$i1]['id'];
                        break;
                    }
                }
			}
//echo 'set_id: ',$set_id,"<br />\n";
            if( $set_id != 0 ){
                $sql = 'update `entry_info` set `disp_order`=' . $order . ', `del`=0 where `id`=' . $set_id;
//echo '<!-- ',$sql," --><br />\n";
//echo $sql,"<br />\n";
                db_query( $dbs, $sql );
                $entry_index++;
...
//echo $sql,"<br />\n";
                db_query( $dbs, $sql );
                $set_id = db_query_insert_id( $dbs );
            }

            $pref_tbl = $this->get_pref_array2();
            $this->update_entry_field_data( $set_id, 'join', '1', $dbs );
                    ...
                    if( $fields[$findex] == 'school_pref' || $fields[$findex] == 'kojin_address_pref' ){
                        foreach( $pref_tbl as $pv ){
                            if( mb_strpos( $value, $pv['title'] ) !== false ){
                                $this->update_entry_field_data( $set_id, $fields[$findex], $pv['value'], $dbs );
                                break;
                            }
                        }
                    } else {
                        $this->update_entry_field_data( $set_id, $fields[$findex], $value, $dbs );
                    }
                }
            }
        ...
        foreach( $tournament_list as $tv ){
            ...
            for( $level = $tv['match_level']; $level >= 1; $level-- ){
                ...
                if( $split == 1 ){
                    if( $level == 4 ){
                        $html .= $footer;
                        $file = 'dtm_' . $navi_info['result_prefix'] . $mw . $tindex . '_1';
						...
						$this->update_realtime_queue( $data );
                        $html = $header;
                        if( count( $tournament_list ) > 1 ){
                            $html .= '    <h2>ブロック' . $tindex . '</h2>'."\n";
                        }
                        ...
                        $html .= $footer;
                        $file = 'dtm_' . $navi_info['result_prefix'] . $mw . $tindex . '_2';
                        $path = $navi_info['result_path'] . '/' . $file . '.html';
                        $fp = fopen( $path, 'w' );
                        fwrite( $fp, $html );
                        fclose( $fp );
            ...
            if( $split == 0 ){
                $html .= $footer;
                $file = 'dtm_' . $navi_info['result_prefix'] . $mw . $tindex;
                $path = $navi_info['result_path'] . '/' . $file . '.html';
                $fp = fopen( $path, 'w' );
                fwrite( $fp, $html );
                fclose( $fp );
...
break;
	...
	function output_entry_data_list_all_1_excel( $sheet, $series, $series_mw, $start_pos, $series_name )
	{
		$field_def = $this->get_entry_field_def( $series, true );
		$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
		$sql = 'select * from `entry_info`'
			.' where `series`='.$series.' and `year`='.$_SESSION['auth']['year'].' and `del`=0'
			.' order by `disp_order` asc';
        ...
        foreach( $field_def as $dv ){
            for( $i1 = 1; $i1 <= 3; $i1++ ){
                if( !is_null( $dv['excel_title'.$i1] ) ){
                    $sheet->setCellValueByColumnAndRow( $col, $i1, $dv['excel_title'.$i1] );
                    if( $dv['kind'] == 'tel_fax' ){
                        $sheet->setCellValueByColumnAndRow( $col, $i1+1, 'TEL' );
                        $sheet->setCellValueByColumnAndRow( $col+1, $i1+1, 'FAX' );
                    } else if( $dv['kind'] == 'mobile_tel' ){
                        $sheet->setCellValueByColumnAndRow( $col, $i1+1, '携帯' );
                        $sheet->setCellValueByColumnAndRow( $col+1, $i1+1, 'TEL' );
                    }
                }
            }
            if( $dv['kind'] == 'address' ){
		...
		foreach( $list as $lv ){
			...
			$col = 0;
			$sheet->setCellValueByColumnAndRow( $col++, $pos, $id );
            if( $series_name != '' ){
                $sheet->setCellValueByColumnAndRow( $col++, $pos, $series_name );
            }
            foreach( $field_def as $dv ){
                $field = $dv['field'];
                if( $dv['kind'] == 'pref_select' ){
                    $sheet->setCellValueByColumnAndRow(
                        $col++, $pos,
                        $this->get_pref_name( $pref_array, get_field_string( $fields, $field ) )
                    );
                } else if( $dv['kind'] == 'check' ){
                    $str = array();
                    $datas = explode( '|', get_field_string( $fields, $field ) );
                    if( count( $datas ) > 0 && counT( $dv['sel'] ) > 0 ){
                        foreach( $datas as $d ){
                            if( isset( $dv['sel'][$d] ) ){
                                $str[] = $dv['sel'][$d];
                            }
                    ...
                    $sheet->setCellValueByColumnAndRow( $col++, $pos, implode( ',', $str ) );
                } else if( $dv['kind'] == 'select' || $dv['kind'] == 'radio' || $dv['kind'] == 'check_one' ){
                    $str = '';
                    $data = get_field_string( $fields, $field );
                    if( $data != '' && counT( $dv['sel'] ) > 0 ){
                        if( isset( $dv['sel'][$data] ) ){
                            $str = $dv['sel'][$data];
                        }
                    }
                    $sheet->setCellValueByColumnAndRow( $col++, $pos, $str );
                } else if( $dv['kind'] == 'address_pref' ){
                    $sheet->setCellValueByColumnAndRow(
                        $col++, $pos,
                        $this->get_pref_name( $pref_array, get_field_string_number( $fields, $field.'_pref', 0 ) )
                    );
                } else if( $dv['kind'] == 'address' ){
                    $sheet->setCellValueByColumnAndRow(
                        $col++, $pos,
                        get_field_string( $fields, $field.'_zip1' ).'-'.get_field_string( $fields, $field.'_zip2' )
                    );
                    $sheet->setCellValueByColumnAndRow(
                        $col++, $pos,
                        $this->get_pref_name( $pref_array, get_field_string_number( $fields, $field.'_pref', 0 ) )
                    );
                    $sheet->setCellValueByColumnAndRow( $col++, $pos, get_field_string( $fields, $field ) );
                } else if( $dv['kind'] == 'name' || $dv['kind'] == 'name_kana' ){
                    $sheet->setCellValueByColumnAndRow( $col++, $pos, get_field_string( $fields, $field.'_sei' ) );
                    $sheet->setCellValueByColumnAndRow( $col++, $pos, get_field_string( $fields, $field.'_mei' ) );
                } else if( $dv['kind'] == 'name3' ){
                    $sheet->setCellValueByColumnAndRow( $col++, $pos, get_field_string( $fields, $field.'_sei' ) );
                    $sheet->setCellValueByColumnAndRow( $col++, $pos, get_field_string( $fields, $field.'_mei' ) );
                    $sheet->setCellValueByColumnAndRow(
                        $col++, $pos, 
                        get_field_string_number( $fields, $field.'_add' ) == 1 ? 'あり' : 'なし'
                    );
                } else if( $dv['kind'] == 'name4' ){
                    $sheet->setCellValueByColumnAndRow( $col++, $pos, get_field_string( $fields, $field.'_sei' ) );
                    $sheet->setCellValueByColumnAndRow( $col++, $pos, get_field_string( $fields, $field.'_mei' ) );
                    ...
                    $sheet->setCellValueByColumnAndRow( $col++, $pos, get_field_string( $fields, $field.'_disp' ) );
                } else if( $dv['kind'] == 'name5' ){
                    $sheet->setCellValueByColumnAndRow( $col++, $pos, get_field_string( $fields, $field.'_sei' ) );
                    $sheet->setCellValueByColumnAndRow( $col++, $pos, get_field_string( $fields, $field.'_mei' ) );
                    $sheet->setCellValueByColumnAndRow(
                    ...
                    $sheet->getCellByColumnAndRow( $col++, $pos )->setValueExplicit( get_field_string( $fields, $field.'_tel' ), PHPExcel_Cell_DataType::TYPE_STRING );
                    $sheet->getCellByColumnAndRow( $col++, $pos )->setValueExplicit( get_field_string( $fields, $field.'_fax' ), PHPExcel_Cell_DataType::TYPE_STRING );
                    //$sheet->setCellValueByColumnAndRow( $col++, $pos, "'".get_field_string( $fields, $field.'_tel' ) );
                    //$sheet->setCellValueByColumnAndRow( $col++, $pos, "'".get_field_string( $fields, $field.'_fax' ) );
                } else if( $dv['kind'] == 'mobile_tel' ){
                    $sheet->getCellByColumnAndRow( $col++, $pos )->setValueExplicit( get_field_string( $fields, $field.'_mobile' ), PHPExcel_Cell_DataType::TYPE_STRING );
                    $sheet->getCellByColumnAndRow( $col++, $pos )->setValueExplicit( get_field_string( $fields, $field.'_fax' ), PHPExcel_Cell_DataType::TYPE_STRING );
                    //$sheet->setCellValueByColumnAndRow( $col++, $pos, "'".get_field_string( $fields, $field.'_mobile' ) );
                    //$sheet->setCellValueByColumnAndRow( $col++, $pos, "'".get_field_string( $fields, $field.'_tel' ) );
                } else if( $dv['kind'] == 'school_org' ){
                    $sheet->setCellValueByColumnAndRow( $col++, $pos, get_field_string( $fields, $field.'_school_name' ) );
                    $sheet->setCellValueByColumnAndRow(
                        $col++, $pos,
                        $this->get_org_name( $org_array, get_field_string( $fields, $field ) )
                    );
                } else if( $dv['kind'] == 'school_name' ){
                    $sheet->setCellValueByColumnAndRow( $col++, $pos, get_field_string( $fields, $field.'_org' ) );
                    $sheet->setCellValueByColumnAndRow( $col++, $pos, get_field_string( $fields, $field ) );
                } else if( $dv['kind'] == 'school_kana' ){
                    $sheet->setCellValueByColumnAndRow( $col++, $pos, get_field_string( $fields, $field ) );
                } else if( $lv['kind'] == 'grade_j' ){
                    $sheet->setCellValueByColumnAndRow(
                        $col++, $pos,
                        $this->get_grade_junior_name( $grade_junior_array, get_field_string_number( $fields, $field, 0 ) )
                    );
                } else if( $lv['kind'] == 'grade_e' ){
                    $sheet->setCellValueByColumnAndRow(
                        $col++, $pos,
                        $this->get_grade_elementary_name( $grade_elementary_array, get_field_string_number( $fields, $field, 0 ) )
                    );
                } else if( $dv['kind'] == 'gakunen_dan_j' ){
                    $sheet->setCellValueByColumnAndRow(
                        $col++, $pos,
                        $this->get_grade_junior_name( $grade_junior_array, get_field_string_number( $fields, $field.'_gakunen', 0 ) )
                    );
                    $sheet->setCellValueByColumnAndRow( $col++, $pos, get_field_string( $fields, $field.'_dan' ) );
                } else if( $dv['kind'] == 'gakunen_dan_e' ){
                    $sheet->setCellValueByColumnAndRow(
                        $col++, $pos,
                        $this->get_grade_elementary_name( $grade_elementary_array, get_field_string_number( $fields, $field.'_gakunen', 0 ) )
                    );
                    $sheet->setCellValueByColumnAndRow( $col++, $pos, get_field_string( $fields, $field.'_dan' ) );
                } else if( $dv['kind'] == 'photo' ){
                    //$data[$field] = get_field_string( $fields, $field );
                } else if( $dv['kind'] == 'yosen_rank' ){
                    $sheet->setCellValueByColumnAndRow(
                        $col++, $pos,
                        $this->get_yosen_rank_name( $yosen_rank_array, get_field_string_number( $fields, $field, 0 ) )
                    );
                } else if( $dv['kind'] == 'date' ){
                    $sheet->setCellValueByColumnAndRow(
                        $col++, $pos,
                        $this->get_birth_year_name( $gengou_array, get_field_string_number( $fields, $field.'_year', 0 ), get_field_string_number( $fields, $field.'_month', 0 ), get_field_string_number( $fields, $field.'_day', 0 ) )
                    );
                    $sheet->setCellValueByColumnAndRow(
                        $col++, $pos, get_field_string_number( $fields, $field.'_month', 0 ) . '月'
                    );
                    $sheet->setCellValueByColumnAndRow(
                        $col++, $pos, get_field_string_number( $fields, $field.'_day', 0 ) . '日'
                    );
                } else if( $dv['kind'] == 'year_month' || $dv['kind'] == 'year_month2' ){
                    $year = get_field_string_number( $fields, $field.'_year', 0 );
                    $month = get_field_string_number( $fields, $field.'_month', 0 );
                    if( $year > 1900 ){
                        $sheet->setCellValueByColumnAndRow(
                            $col, $pos,
                            $this->get_birth_year_name( $gengou_array, $year, $month, 0 )
                        );
                    }
                    $col++;
                    if( $month > 0 ){
                        $sheet->setCellValueByColumnAndRow( $col, $pos, $month . '月' );
                    }
                    $col++;
                } else if( $dv['kind'] == 'include' ){
                    $func = 'get_entry_db_data_from_def_'.$series.'_'.$dv['field'];
                    $data = $func( $data, $fields, $dv, $add1, $add2 );
                } else {
                    if( $dv['data'] == 'text' ){
                        //$sheet->setCellValueByColumnAndRow( $col++, $pos, get_field_string( $fields, $field ) );
                        $sheet->getCellByColumnAndRow( $col++, $pos )->setValueExplicit( get_field_string( $fields, $field ), PHPExcel_Cell_DataType::TYPE_STRING );
                    } else if( $dv['data'] == 'integer' ){
                        $sheet->setCellValueByColumnAndRow( $col++, $pos, get_field_string_number( $fields, $field, 0 ) );
                    }
                }
            }
			$pos++;
		}
        $sheet->getStyle( 'A1:'.PHPExcel_Cell::stringFromColumnIndex($colmax).($pos-1) )->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        $colwidth = array();
        for( $i1 = 0; $i1 < $colmax; $i1++ ){
            $colwidth = 4;
            for( $i2 = 1; $i2 < $pos; $i2++ ){
                $str = $sheet->getCellByColumnAndRow( $i1, $i2 )->getValue();
                $bytes = strlen( mb_convert_encoding( $str, 'SJIS', 'UTF-8' ) );
                if( $colwidth < $bytes ){
                    $colwidth = $bytes;
                }
            }
            $sheet->getColumnDimension( PHPExcel_Cell::stringFromColumnIndex($i1) )->setWidth( $colwidth + 1 );
            //$sheet->setCellValueByColumnAndRow( $i1, $pos, $colwidth );
        }
		db_close( $dbs );
		return $pos;
	}
	...
	//
	//---------------------------------------------------------------
    function fetch_template_for_pdf( $data, $template, $fname )
    {
		include( dirname(dirname(__FILE__))."/mpdf60/mpdf.php" );

		$this->template = 'admin/' . $template . '.html';
		$this->smarty_assign['list'] = $data;
		$html = $this->fetch();
		$fp = fopen( dirname(dirname(__FILE__)).'/output/output.'.$fname.'.html', 'w' );
		$out_html = "<html><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\"></head><body>"
			. $html
			. "</body></html>";
		fwrite( $fp, $out_html );
		fclose( $fp );
...
/**/
        ...
        $mpdf->Output( dirname(dirname(__FILE__)).'/output/'.$fname, 'F' );
	}

	function __embed_PDF_string_raw( $pdf, $x, $y, $size, $text )
    {
        $pdf->SetFont( 'ipamjm', '', $size );
        //$pdf->SetFont( 'kozminproregular', '', $size );
        $pdf->Text( $x, $y, $text );
    }

    function __embed_PDF_string( $pdf, $tbl, $field, $text )
    {
        $this->__embed_PDF_string_raw( $pdf, $tbl[$field]['x'], $tbl[$field]['y'], $tbl[$field]['size'], $text );
    }

    function __embed_PDF_field_string( $pdf, $tbl )
    {
        if( $tbl['mode'] == 'name' ){
        ...
        $this->__embed_PDF_string_raw( $pdf, $tbl['x'], $tbl['y'], $tbl['size'], $text );

//        echo $tbl['x'],':',$tbl['y'],':',$tbl['size'],':',$tbl['field'],':',$text,"<br />\n";
    }

	function embed_data_for_PDF( $data, $fname )
	{
		require_once dirname(dirname(dirname(dirname(__FILE__)))).'/zenchu/admin/tcpdf/tcpdf.php';
		require_once dirname(dirname(dirname(dirname(__FILE__)))).'/zenchu/admin/tcpdf/fpdi/autoload.php';
        $pdf = new setasign\Fpdi\Tcpdf\Fpdi();
        $pdf->setPrintHeader( false );
        $pdf->setPrintFooter( false );
		foreach( $data as $tv ){
	        $pdf->setSourceFile(dirname(dirname(__FILE__)).'/templates/'.$tv['template']);
	        $tpl = $pdf->importPage(1);
			foreach( $tv['data'] as $page ){
		        $pdf->AddPage();
    		    $pdf->useTemplate($tpl);
				foreach( $page as $d ){
					$j = json_decode( $d, true );
					if( $j['kind'] == 'text' ){
						$pdf->SetFont( 'ipamjm', '', $j['size'] );
						//$pdf->SetFont( 'kozminproregular', '', $j['size'] );
						$pdf->Text( $j['x'], $j['y'], $j['text'] );
					} else if( $j['kind'] == 'cell' ){
						$pdf->SetFont( 'ipamjm', '', $j['size'] );
						//$pdf->SetFont( 'kozminproregular', '', $j['size'] );
						$x = $pdf->GetX();
						$y = $pdf->GetY();
						$pdf->SetXY( $j['x'], $j['y'] );
						$pdf->Cell( $j['width'], $j['height'], $j['text'], $j['border'], 0, $j['align'], false, '', 1 );
						$pdf->SetXY( $x, $y );
					} else if( $j['kind'] == 'jpeg' ){
						$pdf->Image(
							$j['image'], $j['x'], $j['y'], 0, $j['height'],
							'JPG', '', '', true, 300
						);
					}
				}
        ...
        $pdf->Output(dirname(dirname(__FILE__)).'/output/' . $fname, "F");
		return dirname(dirname(__FILE__)).'/output/'.$fname;
	}
    ...
    function update_realtime_queue( $data )
    {
		$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
		$sql = 'SELECT * FROM `realtime_queue` WHERE `deleted_at` is not null ORDER BY `id` ASC limit 1';
		$list = db_query_list( $dbs, $sql );
		$sqlset = 'set `deleted_at`=null,'
			. ' `created_at`=now(),'
			. ' `mode`=' . get_field_string_number( $data, 'mode', 0 ) . ','
			. ' `navi`=' . get_field_string_number( $data, 'navi', 0 ) . ','
			. ' `place`=\'' . $dbs->real_escape_string(get_field_string( $data, 'place' )) . '\','
			. ' `file`=\'' . $dbs->real_escape_string(get_field_string( $data, 'file' )) . '\','
			. ' `series`=\'' . $dbs->real_escape_string(get_field_string( $data, 'series' )) . '\'';
		if( count($list) > 0 ){
			$sql = 'update `realtime_queue` ' . $sqlset . ' where `id`='.$list[0]['id'];
		} else {
			$sql = 'insert into `realtime_queue` ' . $sqlset;
		}
		db_query( $dbs, $sql );
		db_close( $dbs );
		//$cmd = 'nohup /usr/bin/php /var/www/cgi-bin/realtime_queue.php > /dev/null 2>&1 &';
		//echo $cmd;
		//passthru( $cmd );
        //$context = new ZMQContext();
        //$socket = $context->getSocket(ZMQ::SOCKET_PUSH, 'my pusher');
        //$socket->connect("tcp://localhost:5558");
        //$socket->send("hello there, using socket 1");
    }

    function update_pdf_queue( $data, $exec )
    {
		if( !is_null($data) ){
			$navi_id = get_field_string_number( $data, 'navi_id', 0 );
			$place = get_field_string_number( $data, 'place', -1 );
			$place_match_no = get_field_string_number( $data, 'place_match_no', -1 );
			if( $navi_id == 0 || $place == -1 || $place_match_no >= 1000 ){
				return;
			}
			$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
			$sql = 'SELECT * FROM `pdf_queue` WHERE `deleted_at` is not null ORDER BY `id` ASC limit 1';
			$list = db_query_list( $dbs, $sql );
			$sqlset = 'set `deleted_at`=null,'
				. ' `created_at`=now(),'
				. ' `navi_id`=' . $navi_id . ','
			...
			if( count($list) > 0 ){
				$sql = 'update `pdf_queue` ' . $sqlset . ' where `id`='.$list[0]['id'];
			} else {
				$sql = 'insert into `pdf_queue` ' . $sqlset;
			}
			db_query( $dbs, $sql );
			db_close( $dbs );
		}
		if( $exec ){
			$cmd = 'nohup /usr/bin/php '. dirname(dirname(dirname(__FILE__))) .'/result/pdf_queue.php > /dev/null 2>&1 &';
			//$cmd = 'nohup /usr/bin/php '. dirname(dirname(dirname(__FILE__))) .'/result/pdf_queue.php &';
			$output=null;
			$retval=null;
			//echo $cmd,"¥n = ";
    ...
    function update_result_queue( $data, $exec )
    {
		if( !is_null($data) ){
			$series = get_field_string_number( $data, 'series', 0 );
			$mw = get_field_string( $data, 'mw' );
			$lt = get_field_string( $data, 'lt' );
			$output_result = get_field_string_number( $data, 'output_result', 0 );
			$output_match = get_field_string_number( $data, 'output_match', 0 );
			if( $series == 0 || $lt == '' ){
				return;
			}
			$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
			$sql = 'SELECT * FROM `result_queue` WHERE `deleted_at` is not null ORDER BY `id` ASC limit 1';
			$list = db_query_list( $dbs, $sql );
			$sqlset = 'set `deleted_at`=null,'
				. ' `created_at`=now(),'
				. ' `series`=' . $series . ','
			...
			if( count($list) > 0 ){
				$sql = 'update `result_queue` ' . $sqlset . ' where `id`='.$list[0]['id'];
			} else {
				$sql = 'insert into `result_queue` ' . $sqlset;
			}
			db_query( $dbs, $sql );
			db_close( $dbs );
		}
		if( $exec ){
			$cmd = 'nohup /usr/bin/php7.0 '. dirname(dirname(dirname(__FILE__))) .'/result/result_queue.php'
				. ' > '	. dirname(dirname(__FILE__)) . '/log/result_queue.log 2>&1 &';
			echo $cmd;
			exec( $cmd );
		}
	}
