...
//print_r($_SESSION);
		if( isset($_SESSION['auth']) && $series != 0 && $_SESSION['auth']['series'] != $series ){
		...
		if( $mode == 'auth' ){
			...
			if(
				!isset($_SESSION['auth'])
				|| ( ( $series == 3 || $series == 4 || $series == 5 || $series == 6 ) && $edit == 0 && $_SESSION['auth']['login'] != 2 )
				|| ( ( $series == 3 || $series == 4 || $series == 5 || $series == 6 ) && $edit == 1 && $_SESSION['auth']['login'] != 1 )
				|| ( $series != 3 && $series != 4 && $series != 5 && $series != 6 && $_SESSION['auth']['login'] != 1 )
			){
...
//print_r($_SESSION);
			...
			if( isset( $_POST['exec'] ) ){
				$info_id = $this->update_entry_data();
				$pid = get_field_string_number( $_SESSION['p'], 'id', 0 );
				if( $pid == 0 ){
					if( $_SESSION['auth']['series'] == 4 ){
/*
						...
						$header_info = "From: ".mb_encode_mimeheader('全中剣道長野大会事務局')." <zenchu@kendo-nagano.com>\n".
						...
						$body = $school_name . " 様\n\n"
							...
							. "エントリー内容の変更は http://nagano-zenchu.jp/regedit/ から\n"
...
/*
						...
						mb_language("Japanese");
						mb_internal_encoding("UTF-8");
						$header_info = "From: ".mb_encode_mimeheader('第13回松代藩文武学校旗争奪全国中学校選抜剣道大会 事務局')." <zenchu@kendo-nagano.com>\n".
							"Content-Type: text/plain;charset=ISO-2022-JP\n".
							"X-Mailer: PHP/".phpversion();
						$title = $school_name . "様 エントリーのお知らせ";
						$body = $school_name . " 様\n\n"
