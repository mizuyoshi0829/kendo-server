<?php
	...
	function __get_entry_data_58_59_list_for_PDF( $series, $series_mw, $kaisai_rev )
	{
		$c = new common();
...
*/
	...
	{
		return __get_entry_data_58_59_list_for_PDF( 16, 'm', 1 );
	}

	function get_entry_data_59_list_for_PDF()
	{
		return __get_entry_data_58_59_list_for_PDF( 17, 'w', 1 );
	}
...
//							$html .= $league_list[$league_data]['match'][$match_no_index-1]['hon2'].'-'.$league_list[$league_data]['match'][$match_no_index-1]['win2'];
			...
			if( ( $league_data % 4 ) == 3 ){
				$html .= $footer;
				if( $league_data == 3 ){
                    $file = $navi_info['result_path'] . '/dl_' . $mw . 'a.html';
				} else if( $league_data == 7 ){
                    $file = $navi_info['result_path'] . '/dl_' . $mw . 'e.html';
				} else if( $league_data == 11 ){
                    $file = $navi_info['result_path'] . '/dl_' . $mw . 'i.html';
				} else if( $league_data == 15 ){
                    $file = $navi_info['result_path'] . '/dl_' . $mw . 'm.html';
				}
				$fp = fopen( $file, 'w' );
				fwrite( $fp, $html );
				fclose( $fp );
...
//print_r($league_list);
	...
	function __output_league_58_59_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, $mw )
	{
//print_r($league_list);
		if( $mw == 'm' ){
			$mwstr = '男子';
		...
		$file_name = 'H30kantoDantaiLeagueResults_' . $mw . '.xls';
		$file_path = $series_info['output_path'] . '/' . $file_name;
		$reader = PHPExcel_IOFactory::createReader('Excel5');
		$excel = $reader->load(dirname(__FILE__).'/dantaiLeagueChartBase_'.$mw.'.xls');
		$excel->setActiveSheetIndex( 0 );		//何番目のシートに有効にするか
		$sheet = $excel->getActiveSheet();	//有効になっているシートを取得
		$sheet->setCellValueByColumnAndRow( 1, 1, '平成30年度関東大会' );
	...
	{
		return __output_league_58_59_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, 'm' );
	}

	function output_league_59_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_58_59_for_Excel( $objPage, $series_info, $league_param, $league_list, $entry_list, 'w' );
	}

	//---------------------------------------------------------------
	//
	//---------------------------------------------------------------
	function __output_league_match_for_HTML2_58_59( $objPage, $navi_info, $league_list, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
		...
		for( $league_data = 0; $league_data < count( $league_list ); $league_data++ ){
			...
			for( $match_index = 0; $match_index < count( $league_list[$league_data]['match'] ); $match_index++ ){
				...
				$html .= $objPage->output_one_match_for_HTML2( $navi_info, $league_list[$league_data]['match'][$match_index], $entry_list, $mw );
			...
			if( ( $league_data % 4 ) == 3 ){
				$html .= $footer;
				if( $league_data == 3 ){
					$file = $navi_info['result_path'] . '/dlm_'.$mw.'a.html';
				} else if( $league_data == 7 ){
					$file = $navi_info['result_path'] . '/dlm_'.$mw.'e.html';
				} else if( $league_data == 11 ){
					$file = $navi_info['result_path'] . '/dlm_'.$mw.'i.html';
				} else if( $league_data == 15 ){
					$file = $navi_info['result_path'] . '/dlm_'.$mw.'m.html';
				}
				$fp = fopen( $file, 'w' );
				fwrite( $fp, $html );
				fclose( $fp );
	...
	{
        $objPage = new form_page();
		__output_league_match_for_HTML2_58_59( $objPage, $navi_info, $league_list, $entry_list, 'm' );
	...
	{
        $objPage = new form_page();
		__output_league_match_for_HTML2_58_59( $objPage, $navi_info, $league_list, $entry_list, 'w' );
	...
	function __output_league_match_for_excel_58_59( $objPage, $series_info, $league_param, $league_list, $entry_list, $mw )
	{
		if( $mw == 'm' ){
			$mwstr = '男子';
		} else {
...
//print_r($league_list);
//exit;
		$file_name = 'H30kantoDantaiLeagueMatchResults_'.$mw.'.xls';
		$file_path = $series_info['output_path'] . '/' . $file_name;
		$reader = PHPExcel_IOFactory::createReader('Excel5');
		$excel = $reader->load(dirname(__FILE__).'/leagueResultsBase.xls');
		$excel->setActiveSheetIndex( 0 );		//何番目のシートに有効にするか
		$sheet = $excel->getActiveSheet();	//有効になっているシートを取得
		$sheet->setCellValueByColumnAndRow( 0, 1, '平成30年度関東大会' );
		$sheet->setCellValueByColumnAndRow( 0, 2, $mwstr.'団体戦　予選リーグ結果' );
		...
		for( $league_data = 0; $league_data < count( $league_list ); $league_data++ ){
			for( $match = 0; $match < $league_list[$league_data]['match_num']; $match++ ){
				$objPage->output_one_match_for_excel( $sheet, $col, $row, $series_info, $league_list[$league_data]['match'][$league_param['place_match_info'][$match]], $entry_list, $mw, 46, 26 );
				$row += 6;
			}
			if( ( $league_data % 4 == 3 ) ){
				$col += 24;
	...
	{
		return __output_league_match_for_excel_58_59( $objPage, $series_info, $league_param, $league_list, $entry_list, 'm' );
	}

	function output_league_match_for_excel_59( $objPage, $series_info, $league_param, $league_list, $entry_list )
	{
		return __output_league_match_for_excel_58_59( $objPage, $series_info, $league_param, $league_list, $entry_list, 'w' );
	}
	...
	function __output_tournament_match_58_59_for_excel( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw )
	{
		...
		$reader = PHPExcel_IOFactory::createReader('Excel5');
		$excel = $reader->load(dirname(__FILE__).'/dantaiTournamentMatchResultsBase2.xls');
		$excel->setActiveSheetIndex( 0 );		//何番目のシートに有効にするか
		$sheet = $excel->getActiveSheet();	//有効になっているシートを取得
...
//exit;
        $file_name = 'H30kantoDantaiTounamentResults.xls';
        $file_path = $series_info['output_path'] . '/' . $file_name;
        $excel = new PHPExcel();
        $reader = PHPExcel_IOFactory::createReader('Excel5');
		...
		$objPage->output_one_match_for_excel( $sheet, 0, 53, $series_info, $tournament_data['match'][7], $entry_list, $mw, 46, 36 );
		$objPage->output_one_match_for_excel( $sheet, 0, 59, $series_info, $tournament_data['match'][8], $entry_list, $mw, 46, 36 );
		$objPage->output_one_match_for_excel( $sheet, 0, 65, $series_info, $tournament_data['match'][9], $entry_list, $mw, 46, 36 );
		$objPage->output_one_match_for_excel( $sheet, 0, 71, $series_info, $tournament_data['match'][10], $entry_list, $mw, 46, 36 );
		$objPage->output_one_match_for_excel( $sheet, 0, 77, $series_info, $tournament_data['match'][11], $entry_list, $mw, 46, 36 );

		$objPage->output_one_match_for_excel( $sheet, 24, 53, $series_info, $tournament_data['match'][12], $entry_list, $mw, 46, 36 );
		$objPage->output_one_match_for_excel( $sheet, 24, 59, $series_info, $tournament_data['match'][13], $entry_list, $mw, 46, 36 );
		$objPage->output_one_match_for_excel( $sheet, 24, 65, $series_info, $tournament_data['match'][14], $entry_list, $mw, 46, 36 );
		$objPage->output_one_match_for_excel( $sheet, 24, 71, $series_info, $tournament_data['match'][3], $entry_list, $mw, 46, 36 );
		$objPage->output_one_match_for_excel( $sheet, 24, 77, $series_info, $tournament_data['match'][4], $entry_list, $mw, 46, 36 );

		$objPage->output_one_match_for_excel( $sheet, 48, 53, $series_info, $tournament_data['match'][5], $entry_list, $mw, 46, 36 );
		$objPage->output_one_match_for_excel( $sheet, 48, 59, $series_info, $tournament_data['match'][6], $entry_list, $mw, 46, 36 );
		$objPage->output_one_match_for_excel( $sheet, 48, 65, $series_info, $tournament_data['match'][1], $entry_list, $mw, 46, 36 );
		$objPage->output_one_match_for_excel( $sheet, 48, 71, $series_info, $tournament_data['match'][2], $entry_list, $mw, 46, 36 );
		$objPage->output_one_match_for_excel( $sheet, 48, 77, $series_info, $tournament_data['match'][0], $entry_list, $mw, 46, 36 );
	...
	function __output_tournament_chart_58_59_for_excel( $objPage, $series_info, $tournament_param, $tournament_list, $league_data, $entry_list, $mw )
	{
		...
		require_once dirname(dirname(dirname(__FILE__))).'/phpExcel/Classes/PHPExcel.php';
		require_once dirname(dirname(dirname(__FILE__))).'/phpExcel/Classes/PHPExcel/IOFactory.php';
		$file_name = 'H30kantoDantaiTournamentChartResults_'.$mw.'.xlsx';
		$file_path = $series_info['output_path'] . '/' . $file_name;
		$reader = PHPExcel_IOFactory::createReader('Excel2007');
		$excel = $reader->load(dirname(__FILE__).'/dantaiTournamentChartBase58.xlsx');
		$excel->setActiveSheetIndex( 0 );		//何番目のシートに有効にするか
		$sheet = $excel->getActiveSheet();	//有効になっているシートを取得
		$sheet->setCellValue( 'S2', '平成30年度関東大会' );
...
//exit;
	...
	function __output_tournament_match_for_HTML2_58_59( $objPage, $series_info, $tournament_list, $entry_list, $mw )
	{
		...
		$html = $header . '    <h2>' . $mwstr . '団体決勝トーナメント結果(一回戦)</h2>'."\n";
		for( $i1 = 1; $i1 <= 8; $i1++ ){
		...
		$html .= $footer;
		$file = $series_info['result_path'] . '/dtm_'.$mw.'1.html';
		$fp = fopen( $file, 'w' );
		fwrite( $fp, $html );
		fclose( $fp );
		...
		$html .= $objPage->output_one_match_for_HTML2( $series_info, $tournament_list[0]['match'][0], $entry_list, $mv );
		$html .= $footer;
		$file = $series_info['result_path'] . '/dtm_'.$mw.'2.html';
		$fp = fopen( $file, 'w' );
		fwrite( $fp, $html );
		fclose( $fp );
	}
...
//echo $pdf;
//exit;
        $file = $series_info['result_path'] . '/dt_' . $mw . '.html';
		$fp = fopen( $file, 'w' );
		fwrite( $fp, $pdf );
		fclose( $fp );
	...
	function __output_entry_data_list_all_1_excel58_59( $sheet, $series, $mw, $start_pos, $series_name, $kaisai_rev )
	{
		...
		foreach( $list as $lv ){
			...
			for( $player = 1; $player <= 7; $player++ ){
				...
				if( $fields['dantai_'.$mw.$player.'_add'] == '1' ){
					$sheet->setCellValueByColumnAndRow( $col, $pos, 'あり' );
				}
				$col++;
				...
				$sheet->setCellValueByColumnAndRow( $col++, $pos, $c->get_grade_junior_name( $gradetbl, intval($fields['dantai_gakunen_dan_'.$mw.$player.'_gakunen']) ) );
				$sheet->setCellValueByColumnAndRow( $col++, $pos, $fields['dantai_gakunen_dan_'.$mw.$player.'_dan'] );
			}
			$sheet->setCellValueByColumnAndRow( $col++, $pos, $fields['entry_num_'.$mw] );
	...
	function output_entry_data_list_all_1_excel58( $sheet )
	{
		...
		__output_entry_data_list_all_1_excel16_59( $sheet, 59, 'w', 4+48, '団体戦女子', 1 );
		return false;
	}
