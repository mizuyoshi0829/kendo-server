				...
				if( $team1 != 0 && $team2 != 0 ){
					...
					if( count($dantai_match_team) > 0 ){
						foreach( $dantai_match_team as $mv ){
							...
							$sql = 'update `one_match`'
								. ' set `player1`=' . $mv['player'] . ','
								. " `player1_change_name`='" . $mv['player_change_name'] . "',"
								. " `supporter1`='" . $mv['supporter'] . "',"
...
//echo 'set:',$sql,"<br />\n";
					...
					if( count($dantai_match_team) > 0 ){
						foreach( $dantai_match_team as $mv ){
							$one_match_id = get_field_string_number( $dantai_match[0], 'match'.$mv['match_no'], 0 );
							$sql = 'update `one_match`'
								. ' set `player2`=' . $mv['player'] . ','
								. " `player2_change_name`='" . $mv['player_change_name'] . "',"
								. " `supporter2`='" . $mv['supporter'] . "',"
								. ' `update_date`=NOW() where `id`='.$one_match_id;
//echo 'set:',$sql,"<br />\n";
							db_query( $dbs, $sql );
						}
...
//        echo $tbl['x'],':',$tbl['y'],':',$tbl['size'],':',$tbl['field'],':',$text,"<br />\n";
    ...
    function update_realtime_queue( $data )
    {
		...
		db_query( $dbs, $sql );
		db_close( $dbs );
		$cmd = 'nohup /usr/bin/php7.0 '. dirname(dirname(dirname(__FILE__))) .'/result/realtime_queue.php > /dev/null 2>&1 &';
		//echo $cmd;
		exec( $cmd );
    }
    ...
    function update_pdf_queue( $data, $exec )
    {
		if( !is_null($data) ){
			...
			if( $navi_id == 0 || $place == -1 || $place_match_no == -1 || $place_match_no >= 1000 ){
				return;
			}
			$dbs = db_connect( DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME );
