...
class form_page_admin_dantai_league extends form_page
{
	...
	//	if( $_SESSION['auth'] != 1 ){ exit; }
		...
		if( $mode == 'new' ){
			...
			$this->clear_match_info( 5 );
			$this->clear_match_info( 6 );
			...
			$this->update_dantai_league_list( $series, $mw, $_POST );
		} else if( $mode == 'delete' ){
			$id = get_field_string_number( $_POST, 'id', 0 );
			if( $id > 0 ){
		...
		$list = $this->get_dantai_league_list( $series, $mw );
		$this->smarty_assign['league_list'] = $list;
		$this->smarty_assign['place_array'] = $this->get_place_array_for_smarty(null,$list[0]);
...
/**/
			...
			$league_data = $this->get_dantai_league_list( $series, $mw );
